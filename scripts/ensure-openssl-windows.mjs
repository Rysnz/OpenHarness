 ...
 * - Cached under .openharness/cache/firedaemon-openssl-<version>/x64 (gitignored).
 * - Skips download if OPENSSL_DIR already points at a valid tree, or cache hit, or OPENHARNESS_SKIP_OPENSSL_BOOTSTRAP=1.
...
export const CACHE_ROOT = join(ROOT, '.openharness', 'cache', `firedaemon-openssl-${OPENSSL_VERSION}`);
...
export async function ensureOpenSslWindows() {
  ...
  if (process.env.OPENHARNESS_SKIP_OPENSSL_BOOTSTRAP === '1') {
    console.log(`${LOG} OPENHARNESS_SKIP_OPENSSL_BOOTSTRAP=1, skipping bootstrap`);
    return;
  }
