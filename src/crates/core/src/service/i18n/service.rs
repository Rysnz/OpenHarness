...
impl I18nService {
    ...
    pub async fn initialize(&self) -> OpenHarnessResult<()> {
    ...
    async fn load_all_bundles(&self) -> OpenHarnessResult<()> {
    ...
    pub async fn set_locale(&self, locale: LocaleId) -> OpenHarnessResult<()> {
        let old_locale = {
            let mut current = self.current_locale.write().await;
...
pub async fn initialize_global_i18n_service(
    config_service: Option<Arc<ConfigService>>,
) -> OpenHarnessResult<Arc<I18nService>> {
