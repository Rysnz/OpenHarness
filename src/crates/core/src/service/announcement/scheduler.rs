...
impl AnnouncementScheduler {
    ...
    pub async fn run(&self, locale: &str) -> OpenHarnessResult<Vec<AnnouncementCard>> {
    ...
    pub async fn mark_seen(&self, id: &str) -> OpenHarnessResult<()> {
        let mut state = self.state.write().await;
    ...
    pub async fn dismiss(&self, id: &str) -> OpenHarnessResult<()> {
        let mut state = self.state.write().await;
    ...
    pub async fn never_show(&self, id: &str) -> OpenHarnessResult<()> {
        let mut state = self.state.write().await;
