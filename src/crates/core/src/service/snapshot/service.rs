...
impl SnapshotService {
    pub fn new(workspace_dir: PathBuf, config: Option<SnapshotConfig>) -> Self {
        ...
        let snapshot_system = FileSnapshotSystem::new(&openharness_dir);
        let snapshot_core = Arc::new(RwLock::new(SnapshotCore::new(
            &openharness_dir,
            snapshot_system,
        )));
        let file_lock_manager = Arc::new(FileLockManager::new(openharness_dir.clone()));
    ...
    pub async fn initialize(&mut self) -> SnapshotResult<()> {
        ...
        info!(
            "Snapshot service initialized: git_isolated={} openharness_dir={}",
            isolation_status,
            self.openharness_dir.display()
        );
    ...
    pub async fn get_system_stats(&self) -> SnapshotResult<SystemStats> {
        ...
        Ok(SystemStats {
            ...
            openharness_dir: self.openharness_dir.clone(),
        })
    }
