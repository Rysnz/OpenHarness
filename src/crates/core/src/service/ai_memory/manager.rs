...
impl AIMemoryManager {
    ...
    pub async fn new(path_manager: Arc<PathManager>) -> OpenHarnessResult<Self> {
        ...
        if let Some(parent) = storage_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                OpenHarnessError::io(format!("Failed to create memory storage directory: {}", e))
            })?;
        }
    ...
    pub async fn new_project(
        ...
        let storage_path = workspace_path.join(".openharness").join("ai_memories.json");
    ...
    async fn load_storage(path: &PathBuf) -> OpenHarnessResult<MemoryStorage> {
        let content = fs::read_to_string(path).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to read memory storage file: {}", e))
        })?;

        let storage: MemoryStorage = serde_json::from_str(&content).map_err(|e| {
            OpenHarnessError::Deserialization(format!(
                "Failed to deserialize memory storage: {}",
                e
            ))
        })?;
    ...
    async fn save_storage(&self) -> OpenHarnessResult<()> {
        let storage = self.storage.read().await;
        let content = serde_json::to_string_pretty(&*storage).map_err(|e| {
            OpenHarnessError::serialization(format!("Failed to serialize memory storage: {}", e))
        })?;

        fs::write(&self.storage_path, content).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to write memory storage file: {}", e))
        })?;
    ...
    /// Adds a memory point.
    pub async fn add_memory(&self, memory: AIMemory) -> OpenHarnessResult<AIMemory> {
    ...
    pub async fn delete_memory(&self, id: &str) -> OpenHarnessResult<bool> {
        let mut storage = self.storage.write().await;
    ...
    pub async fn update_memory(&self, memory: AIMemory) -> OpenHarnessResult<bool> {
        let mut storage = self.storage.write().await;
    ...
    pub async fn get_all_memories(&self) -> OpenHarnessResult<Vec<AIMemory>> {
        let storage = self.storage.read().await;
        Ok(storage.memories.clone())
    }

    /// Returns enabled memory points.
    pub async fn get_enabled_memories(&self) -> OpenHarnessResult<Vec<AIMemory>> {
        let storage = self.storage.read().await;
        Ok(storage
    ...
    pub async fn get_memories_for_prompt(&self) -> OpenHarnessResult<Option<String>> {
    ...
    pub async fn toggle_memory(&self, id: &str) -> OpenHarnessResult<bool> {
        let mut storage = self.storage.write().await;
