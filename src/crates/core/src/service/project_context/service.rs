...
impl ProjectContextService {
    ...
    pub async fn get_document_statuses(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<Vec<ContextDocumentStatus>> {
    ...
    pub async fn create_document(
        ...
        let doc = find_builtin_document(doc_id)
            .ok_or_else(|| OpenHarnessError::service(format!("Unknown document id: {}", doc_id)))?;

        let relative_path = doc.possible_paths.first().ok_or_else(|| {
            OpenHarnessError::service(format!("No possible path for document: {}", doc_id))
        })?;
        ...
        if let Some(parent) = full_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create directory: {}", e))
            })?;
        }
        ...
        fs::write(&full_path, template)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to create document: {}", e)))?;
    ...
    pub async fn generate_document(
        ...
        let result = match result {
            Ok(r) => r,
            Err(e) => {
                if matches!(e, OpenHarnessError::Cancelled(_)) {
                    debug!(
    ...
    pub async fn cancel_generate_document(&self, doc_id: &str) -> OpenHarnessResult<()> {
    ...
    /// Loads configuration.
    pub async fn load_config(&self, workspace: &Path) -> OpenHarnessResult<ProjectContextConfig> {
        ...
        let content = fs::read_to_string(&config_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to read config: {}", e)))?;

        serde_json::from_str(&content)
            .map_err(|e| OpenHarnessError::service(format!("Failed to parse config: {}", e)))
    }
    ...
    async fn load_config_and_cleanup(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<ProjectContextConfig> {
        let mut config = self.load_config(workspace).await.unwrap_or_default();
    ...
    pub async fn save_config(
        ...
        if let Some(parent) = config_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create .openharness directory: {}", e))
            })?;
        }

        let content = serde_json::to_string_pretty(config)
            .map_err(|e| OpenHarnessError::service(format!("Failed to serialize config: {}", e)))?;

        fs::write(&config_path, content)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to write config: {}", e)))?;
    ...
    fn get_config_path(&self, workspace: &Path) -> PathBuf {
        workspace.join(".openharness").join(CONFIG_FILE_NAME)
    }
    ...
    pub async fn create_category(
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .map_err(|e| OpenHarnessError::service(format!("Failed to get timestamp: {}", e)))?
            .as_secs();
    ...
    pub async fn update_category(
        ...
        let category = config
            ...
            .find(|cat| cat.id == category_id)
            .ok_or_else(|| {
                OpenHarnessError::service(format!("Custom category not found: {}", category_id))
            })?;
    ...
    pub async fn get_all_categories(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<Vec<CategoryInfo>> {
        let config = self.load_config_and_cleanup(workspace).await?;
    ...
    ) -> OpenHarnessResult<ImportedDocument> {
    ...
    pub async fn delete_imported_document(
        ...
        let doc_index = config
            ...
            .ok_or_else(|| {
                OpenHarnessError::service(format!("Imported document not found: {}", doc_id))
            })?;
        ...
        if doc_path.exists() {
            fs::remove_file(&doc_path).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to remove document file: {}", e))
            })?;
    ...
    pub async fn get_imported_documents(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<Vec<ImportedDocument>> {
        let config = self.load_config(workspace).await.unwrap_or_default();
