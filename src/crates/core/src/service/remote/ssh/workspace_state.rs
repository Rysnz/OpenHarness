...
/// This returns a dedicated stub under `~/.openharness/remote_ssh/_unresolved/.../sessions` that is
...
impl RemoteWorkspaceStateManager {
    ...
    pub async fn lookup_connection(
        ...
        // Partner sessions use client-local paths under ~/.openharness/partners.
        ...
        let is_local_assistant_path = get_path_manager_arc().is_local_partner_workspace_path(path);
    ...
    pub async fn is_remote_path(&self, path: &str) -> bool {
        if get_path_manager_arc().is_local_partner_workspace_path(path) {
            return false;
        }
        let path_norm = normalize_remote_workspace_path(path);
        let guard = self.registrations.read().await;
    ...
    /// Local mirror directory for persisted sessions (`~/.openharness/remote_ssh/.../sessions`).
...
mod tests {
    ...
    #[tokio::test]
    async fn local_partner_path_not_remote_without_connection_id() {
        let pm = PathManager::default();
        let partner_path = pm
            .partner_workspace_dir("d3726520", None)
            .to_string_lossy()
            .to_string();
        ...
        .await;
        assert!(
            m.lookup_connection(&partner_path, None).await.is_none(),
            "partner workspace must not bind to SSH when remote_connection_id is omitted"
        );
        assert!(
            m.lookup_connection(&partner_path, Some("conn"))
                .await
                .is_some(),
    ...
    fn local_stable_id_is_deterministic_and_prefixed() {
        let id1 = super::local_workspace_stable_storage_id("/Users/foo/OpenHarness");
        let id2 = super::local_workspace_stable_storage_id("/Users/foo/OpenHarness");
        assert_eq!(id1, id2);
