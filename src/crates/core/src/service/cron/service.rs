...
impl CronService {
    ...
    pub async fn create_job(&self, request: CreateCronJobRequest) -> OpenHarnessResult<CronJob> {
    ...
    ) -> OpenHarnessResult<CronJob> {
        let _guard = self.mutation_lock.lock().await;
        let mut jobs = self.jobs.write().await;
        let current_ms = now_ms();
        let job = jobs.get_mut(job_id).ok_or_else(|| {
            OpenHarnessError::NotFound(format!("Scheduled job not found: {}", job_id))
        })?;
    ...
    pub async fn set_job_enabled(&self, job_id: &str, enabled: bool) -> OpenHarnessResult<CronJob> {
        self.update_job(
            job_id,
    ...
    pub async fn delete_job(&self, job_id: &str) -> OpenHarnessResult<bool> {
        let _guard = self.mutation_lock.lock().await;
        let mut jobs = self.jobs.write().await;
    ...
    pub async fn delete_jobs_for_session(&self, session_id: &str) -> OpenHarnessResult<usize> {
    ...
    pub async fn run_job_now(&self, job_id: &str) -> OpenHarnessResult<CronJob> {
        {
            let _guard = self.mutation_lock.lock().await;
            let mut jobs = self.jobs.write().await;
            let current_ms = now_ms();
        ...
        self.get_job(job_id).await.ok_or_else(|| {
            OpenHarnessError::NotFound(format!("Scheduled job not found after run: {}", job_id))
        })
    }

    pub async fn handle_turn_started(&self, turn_id: &str) -> OpenHarnessResult<()> {
    ...
    pub async fn handle_turn_failed(&self, turn_id: &str, error: &str) -> OpenHarnessResult<()> {
        self.handle_turn_state_change(turn_id, |job, now_ms| {
            job.state.active_turn_id = None;
    ...
    pub async fn handle_turn_cancelled(&self, turn_id: &str) -> OpenHarnessResult<()> {
        self.handle_turn_state_change(turn_id, |job, now_ms| {
            job.state.active_turn_id = None;
    ...
    async fn handle_turn_state_change<F>(&self, turn_id: &str, update: F) -> OpenHarnessResult<()>
    where
    ...
    async fn process_due_jobs(&self) -> OpenHarnessResult<()> {
        let current_ms = now_ms();
        let due_job_ids = {
            let jobs = self.jobs.read().await;
            let mut due = jobs
    ...
    async fn process_job(&self, job_id: &str) -> OpenHarnessResult<()> {
        let _guard = self.mutation_lock.lock().await;
        let mut jobs = self.jobs.write().await;
        let current_ms = now_ms();
    ...
    async fn persist_snapshot(&self) -> OpenHarnessResult<()> {
        let jobs = self.jobs.read().await;
        self.persist_jobs_locked(&jobs).await
    }

    async fn persist_jobs_locked(&self, jobs: &HashMap<String, CronJob>) -> OpenHarnessResult<()> {
        self.store
...
fn reconcile_loaded_job(job: &mut CronJob, now_ms: i64) -> OpenHarnessResult<bool> {
