...
pub fn validate_schedule(schedule: &CronSchedule, created_at_ms: i64) -> OpenHarnessResult<()> {
    let _ = compute_next_run_after_ms(schedule, created_at_ms, created_at_ms - 1)?;
    Ok(())
}

pub fn compute_initial_next_run_at_ms(
    job: &CronJob,
    now_ms: i64,
) -> OpenHarnessResult<Option<i64>> {
    match &job.schedule {
        CronSchedule::At { .. } => {
...
pub fn parse_at_timestamp_ms(schedule: &CronSchedule) -> OpenHarnessResult<i64> {
    ...
    let parsed = DateTime::parse_from_rfc3339(at).map_err(|error| {
        OpenHarnessError::validation(format!("Invalid ISO-8601 timestamp '{}': {}", at, error))
    })?;
...
fn compute_every_next_run_ms(
    ...
    i64::try_from(next).map_err(|_| {
        OpenHarnessError::service("Recurring schedule next run timestamp overflowed i64")
    })
}

fn compute_cron_next_run_ms(expr: &str, tz: Option<&str>, after_ms: i64) -> OpenHarnessResult<i64> {
    let normalized_expr = normalize_cron_expr(expr)?;
    let schedule = Schedule::from_str(&normalized_expr).map_err(|error| {
        OpenHarnessError::validation(format!("Invalid cron expression '{}': {}", expr, error))
    })?;
...
fn parse_timezone(tz_name: &str) -> OpenHarnessResult<Tz> {
    Tz::from_str(tz_name).map_err(|error| {
        OpenHarnessError::validation(format!("Invalid timezone '{}': {}", tz_name, error))
    })
}

fn normalize_cron_expr(expr: &str) -> OpenHarnessResult<String> {
    let fields = expr.split_whitespace().collect::<Vec<_>>();
    match fields.len() {
        5 => Ok(format!("0 {}", expr)),
        6 | 7 => Ok(expr.to_string()),
        other => Err(OpenHarnessError::validation(format!(
