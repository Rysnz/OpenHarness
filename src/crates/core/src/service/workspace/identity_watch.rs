...
impl WorkspaceIdentityWatchService {
    ...
    pub async fn set_event_emitter(&self, emitter: Arc<dyn EventEmitter>) -> OpenHarnessResult<()> {
        {
    ...
    pub async fn sync_watched_workspaces(&self) -> OpenHarnessResult<()> {
        let partner_workspaces = self.workspace_service.get_partner_workspaces().await;
        let next_paths: HashMap<PathBuf, String> = partner_workspaces
            .into_iter()
    ...
    async fn create_watcher(&self) -> OpenHarnessResult<()> {
        ...
        let mut watcher = RecommendedWatcher::new(tx, Config::default()).map_err(|e| {
            OpenHarnessError::service(format!("Failed to create identity watcher: {}", e))
        })?;
