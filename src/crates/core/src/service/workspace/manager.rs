...
pub struct WorkspaceOpenOptions {
    ...
    /// Local/partner workspaces use a stable `local_*` id from `localhost` + canonical root path.
...
impl WorkspaceInfo {
    ...
    pub async fn new(root_path: PathBuf, options: WorkspaceOpenOptions) -> OpenHarnessResult<Self> {
        ...
        let partner_id = if workspace_kind == WorkspaceKind::Partner {
            options.partner_id.clone()
        } else {
            None
        };
        ...
        let (id, resolved_root_path) = if is_remote {
            ...
            let (canonical_pb, norm_str) =
                canonicalize_local_workspace_root(&root_path).map_err(OpenHarnessError::service)?;
    ...
    async fn load_identity(&mut self) {
        ...
        if self.workspace_kind == WorkspaceKind::Partner {
    ...
    /// Scans the workspace.
    async fn scan_workspace(&mut self, options: ScanOptions) -> OpenHarnessResult<()> {
    ...
    fn scan_directory<'a>(
        ...
        depth: usize,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = OpenHarnessResult<()>> + 'a + Send>>
    {
        Box::pin(async move {
            ...
            let mut read_dir = fs::read_dir(dir).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to read directory: {}", e))
            })?;

            while let Some(entry) = read_dir.next_entry().await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to read directory entry: {}", e))
            })? {
                let path = entry.path();
                ...
                let metadata = entry.metadata().await.map_err(|e| {
                    OpenHarnessError::service(format!("Failed to read metadata: {}", e))
                })?;
...
pub struct WorkspaceManager {
    ...
    recent_partner_workspaces: Vec<String>,
...
impl WorkspaceManager {
    ...
    pub fn rekey_workspace_id(&mut self, old_id: &str, new_id: String) -> OpenHarnessResult<()> {
        ...
        for rid in &mut self.recent_partner_workspaces {
            if rid.as_str() == old_id {
                *rid = new_id.clone();
            }
        }
    ...
    /// Migrates persisted local/partner workspaces from legacy UUID ids to `local_*` stable ids.
    ...
    pub fn migrate_local_workspace_ids_to_stable_storage(&mut self) -> HashMap<String, String> {
        ...
        for old_id in old_ids {
            ...
            if self.workspaces.contains_key(&new_id) {
                ...
                self.recent_partner_workspaces.retain(|x| x != &old_id);
    ...
    /// Opens a workspace.
    pub async fn open_workspace(&mut self, path: PathBuf) -> OpenHarnessResult<WorkspaceInfo> {
    ...
    pub async fn open_workspace_with_options(
        ...
        let existing_workspace_id = if is_remote {
            ...
            let canon_norm = match normalize_local_workspace_root_for_stable_id(&path) {
                Ok(n) => n,
                Err(e) => return Err(OpenHarnessError::service(e)),
            };
        ...
        if let Some(workspace_id) = existing_workspace_id {
            if let Some(workspace) = self.workspaces.get_mut(&workspace_id) {
                ...
                workspace.partner_id = if options.workspace_kind == WorkspaceKind::Partner {
                    options.partner_id.clone()
                } else {
                    None
                };
    ...
    pub fn close_current_workspace(&mut self) -> OpenHarnessResult<()> {
    ...
    pub fn close_workspace(&mut self, workspace_id: &str) -> OpenHarnessResult<()> {
    ...
    pub fn set_active_workspace(&mut self, workspace_id: &str) -> OpenHarnessResult<()> {
        if !self
    ...
    pub fn set_current_workspace(&mut self, workspace_id: String) -> OpenHarnessResult<()> {
    ...
    /// Returns recently accessed partner workspace records.
    pub fn get_recent_partner_workspace_infos(&self) -> Vec<&WorkspaceInfo> {
        self.recent_partner_workspaces
            .iter()
            .filter_map(|id| self.workspaces.get(id))
            .collect()
    }
    ...
    /// Removes a workspace.
    pub fn remove_workspace(&mut self, workspace_id: &str) -> OpenHarnessResult<()> {
    ...
    pub async fn cleanup_invalid_workspaces(&mut self) -> OpenHarnessResult<usize> {
    ...
    fn update_recent_workspaces(&mut self, workspace_id: String) {
        ...
        let is_partner = self
            .workspaces
            .get(&workspace_id)
            .map(|workspace| workspace.workspace_kind == WorkspaceKind::Partner)
            .unwrap_or(false);
        let target_list = if is_partner {
            &mut self.recent_partner_workspaces
        } else {
    ...
    pub fn remove_from_recent_workspaces_only(&mut self, workspace_id: &str) -> bool {
        ...
        let before_a = self.recent_partner_workspaces.len();
        self.recent_partner_workspaces
            .retain(|id| id != workspace_id);
        if self.recent_partner_workspaces.len() != before_a {
            changed = true;
        }
        changed
    }
    ...
    /// Returns a reference to the recent partner-workspaces list.
    pub fn get_recent_partner_workspaces(&self) -> &Vec<String> {
        &self.recent_partner_workspaces
    }

    /// Sets the recent partner-workspaces list.
    pub fn set_recent_partner_workspaces(&mut self, recent: Vec<String>) {
        self.recent_partner_workspaces = recent
            .into_iter()
            .filter(|id| {
                self.workspaces
                    .get(id)
