...
pub enum RemoteResponse {
    WorkspaceInfo {
        ...
        /// `"normal"` | `"partner"` | `"remote"` — mirrors [`crate::service::workspace::WorkspaceKind`].
        #[serde(skip_serializing_if = "Option::is_none")]
...
fn turns_to_chat_messages(turns: &[crate::service::session::DialogTurnData]) -> Vec<ChatMessage> {
    ...
    for turn in turns {
        ...
        // Skip partner message for in-progress turns.  The active turn's
        // content is delivered via the real-time overlay, not the historical
        // list.  Including an empty / partial partner message here would
        ...
        result.push(ChatMessage {
            id: format!("{}_partner", turn.turn_id),
...
impl RemoteSessionStateTracker {
    ...
    fn handle_event(&self, event: &crate::agentic::events::AgenticEvent) {
        use openharness_events::AgenticEvent as AE;
...
impl crate::agentic::events::EventSubscriber for Arc<RemoteSessionStateTracker> {
    ...
    ) -> crate::util::errors::OpenHarnessResult<()> {
...
impl RemoteServer {
    ...
    pub async fn dispatch(&self, cmd: &RemoteCommand) -> RemoteResponse {
        match cmd {
            ...
            | RemoteCommand::ListPartners
            | RemoteCommand::SetPartner { .. } => self.handle_workspace_command(cmd).await,
    ...
    async fn handle_poll_command(&self, cmd: &RemoteCommand) -> RemoteResponse {
        ...
        let has_partner_msg = new_messages.iter().any(|m| m.role == "partner");

        let active_turn = if turn_finished && has_partner_msg {
        ...
        let (send_msgs, send_total) = if turn_finished && !has_partner_msg {
            // Turn is finished but disk doesn't have the completed partner
        ...
        match cmd {
                ...
                if let Some(ws_service) = get_global_workspace_service() {
                    if let Some(ws) = ws_service.get_current_workspace().await {
                        ...
                        return RemoteResponse::WorkspaceInfo {
                            ...
                            partner_id: ws.partner_id.clone(),
                        };
                    }
                }
                ...
                let entries = recent
                    ...
                    .map(|w| {
                        let kind_str = match w.workspace_kind {
                            ...
                            crate::service::workspace::WorkspaceKind::Partner => "partner",
            ...
            RemoteCommand::ListPartners => {
                let ws_service = match get_global_workspace_service() {
                    Some(s) => s,
                    None => {
                        return RemoteResponse::PartnerList { partners: vec![] };
                    }
                };
                ...
                let entries = partners
                    ...
                    .map(|w| PartnerEntry {
                        path: w.root_path.to_string_lossy().to_string(),
                        name: w.name.clone(),
                        partner_id: w.partner_id.clone(),
                    })
                    .collect();
                RemoteResponse::PartnerList { partners: entries }
            }
            RemoteCommand::SetPartner { path } => {
                let ws_service = match get_global_workspace_service() {
                    Some(s) => s,
                    None => {
                        return RemoteResponse::PartnerUpdated {
                            success: false,
                            path: None,
                            name: None,
                            error: Some("Workspace service not available".into()),
                ...
                match ws_service.open_workspace(path_buf).await {
                    Ok(info) => {
                        if let Err(e) =
                            ...
                            .await
                        {
                            error!("Failed to initialize snapshot after remote partner set: {e}");
                        }
                        RemoteResponse::PartnerUpdated {
                            success: true,
                            path: Some(info.root_path.to_string_lossy().to_string()),
                    ...
                    Err(e) => RemoteResponse::PartnerUpdated {
                        success: false,
                        path: None,
                        name: None,
                        error: Some(e.to_string()),
        ...
        match cmd {
            ...
            RemoteCommand::CreateSession {
                ...
                let session_name =
                    custom_name
                        ...
                        .unwrap_or(match agent {
                            ...
                            "Partner" => "Remote Partner Session",
                            _ => "Remote Code Session",
                        });

                let binding_ws_str = if is_partner {
                    // For Partner sessions, get or create default partner workspace
                    use crate::service::workspace::get_global_workspace_service;
                    ...
                    if let Some(default_ws) =
                        workspaces.into_iter().find(|w| w.partner_id.is_none())
                    {
                        ...
                        match ws_service.create_partner_workspace(None).await {
                            ...
                            Err(e) => {
                                return RemoteResponse::Error {
                                    message: format!("Failed to create partner workspace: {}", e),
                                };
                            }
                        }
                    }
                ...
                let Some(binding_ws_str) = binding_ws_str else {
                    return RemoteResponse::Error {
                        message: if is_partner {
                            "Failed to get or create partner workspace".to_string()
                        } else {
