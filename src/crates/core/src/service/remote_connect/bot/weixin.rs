...
//! `@tencent-weixin/openpartner-weixin`. Login is QR-based; after login the same 6-digit
...
/// Weixin CDN host for encrypted upload (same as `@tencent-weixin/openpartner-weixin`).
...
/// CDN download URL (same as `@tencent-weixin/openpartner-weixin` `buildCdnDownloadUrl`).
...
/// `CDNMedia.aes_key`: base64(raw 16 bytes) or base64(32-char hex) — OpenPartner `parseAesKey`.
...
// ── QR login session store (in-memory, same role as OpenPartner installer) ─────
...
impl WeixinBot {
    ...
    /// Decrypt one inbound `image_item` (CDN download + AES-128-ECB), matching OpenPartner `downloadMediaFromItem`.
    ...
    /// Read plaintext → encrypt → getuploadurl → POST to CDN (same pipeline as OpenPartner weixin plugin).
    ...
    async fn send_message_with_items(
        ...
        let client_id = format!("openharness-wx-{}", uuid::Uuid::new_v4());
        let msg = json!({
            "from_user_id": "",
            "to_user_id": to_user_id,
            "client_id": client_id,
            ...
            if let Some(msgs) = resp["msgs"].as_array() {
                for msg in msgs {
                    ...
                    if text.len() == 6 && text.chars().all(|c| c.is_ascii_digit()) {
                        ...
                        let err = if language.is_chinese() {
                            "请输入 OpenHarness 桌面端远程连接中显示的 6 位配对码。"
                        } else {
                            "Please send the 6-digit pairing code from OpenHarness Desktop Remote Connect."
                        };
                        let _ = self.send_text(&peer, err).await;
                    } else if Self::has_inbound_image_items(msg) {
                        let err = if language.is_chinese() {
                            "配对请直接发送 6 位数字配对码；完成配对后再发送图片与伙伴对话。"
                        } else {
