...
/// Display mode for bot sessions - Professional or Partner
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default)]
pub enum BotDisplayMode {
    /// Professional mode: can create Code/Cowork sessions
    #[serde(rename = "pro")]
    Pro,
    /// Partner mode: can create Partner sessions
...
pub struct BotChatState {
    ...
    pub current_partner: Option<String>,
    pub current_session_id: Option<String>,
    /// Display mode: Professional (Pro) or Partner
    #[serde(default)]
    ...
    /// Not persisted — used so numeric replies work like OpenPartner menu numbers.
    #[serde(skip, default)]
...
impl BotChatState {
    pub fn new(chat_id: String) -> Self {
        Self {
            ...
            display_mode: BotDisplayMode::Partner,
            pending_action: None,
...
pub fn parse_command(text: &str) -> BotCommand {
    ...
    match trimmed {
        ...
        "/switch_partner" => BotCommand::SwitchPartner,
        "/pro" => BotCommand::SwitchMode(BotDisplayMode::Pro),
        "/partner" => BotCommand::SwitchMode(BotDisplayMode::Partner),
        ...
        "/new_partner_session" => BotCommand::NewPartnerSession,
...
要连接你的 OpenHarness 桌面端，请发送 OpenHarness Remote Connect 面板里显示的 6 位配对码。

如果你还没有配对码，请打开 OpenHarness Desktop -> Remote Connect -> Telegram/飞书/微信机器人，复制 6 位配对码并发送到这里。"
    } else {
        "\
Welcome to OpenHarness!

To connect your OpenHarness desktop app, please enter the 6-digit pairing code shown in your OpenHarness Remote Connect panel.

Need a pairing code? Open OpenHarness Desktop -> Remote Connect -> Telegram/Feishu/WeChat bot -> copy the 6-digit code and send it here."
    }
}
...
/switch_partner - List and switch partners (Partner mode)
/pro - Switch to Expert mode (can create Code/Cowork sessions)
/partner - Switch to Partner mode (can create Partner sessions)
...
/new_partner_session - Create a new partner session (Partner mode)
...
pub fn paired_success_message(language: BotLanguage) -> String {
    if language.is_chinese() {
        format!(
            "配对成功！OpenHarness 已连接。\n\n{}",
            help_message(language)
        )
    } else {
        format!(
            "Pairing successful! OpenHarness is now connected.\n\n{}",
            help_message(language)
        )
    }
}

/// After IM pairing: partner mode, default partner workspace, resume latest Partner (else any) session or create Partner.
/// Mutates `state` (`display_mode`, `current_partner`, `current_session_id`). Does not set `paired`.
pub async fn bootstrap_im_chat_after_pairing(state: &mut BotChatState) -> String {
    ...
    state.display_mode = BotDisplayMode::Partner;
    ...
    let ws_service = match get_global_workspace_service() {
        ...
        None => {
            return if language.is_chinese() {
                "自动准备未能完成：工作区服务不可用。请稍后在 OpenHarness 桌面端打开工作区后再试。"
                    .to_string()
            } else {
                "Auto-setup incomplete: workspace service unavailable. Open a workspace in OpenHarness Desktop and try again."
                    .to_string()
            };
        }
    };

    let mut partners = ws_service.get_partner_workspaces().await;
    if partners.is_empty() {
        match ws_service.create_partner_workspace(None).await {
            Ok(w) => partners.push(w),
            Err(e) => {
                return if language.is_chinese() {
                    format!("自动准备未能完成：无法创建伙伴工作区（{e}）。请使用 /switch_partner。")
                } else {
                    format!(
                        "Auto-setup incomplete: could not create partner workspace ({e}). Use /switch_partner."
                    )
                };
            }
        }
    ...
    let picked = partners
        .iter()
        .find(|w| w.partner_id.is_none())
        .cloned()
        .or_else(|| partners.first().cloned());

    let Some(ws_info) = picked else {
        return if language.is_chinese() {
            "自动准备未能完成：没有可用伙伴。请使用 /switch_partner。".to_string()
        } else {
            "Auto-setup incomplete: no partner found. Use /switch_partner.".to_string()
        };
    };
    ...
    if let Err(e) = ws_service.open_workspace(path_buf.clone()).await {
        return if language.is_chinese() {
            format!("自动准备未能完成：无法打开伙伴工作区（{e}）。")
        } else {
            format!("Auto-setup incomplete: failed to open partner workspace ({e}).")
        };
    }
    if let Err(e) =
    ...
    state.current_partner = Some(path_str.clone());
    ...
    if let Some(m) = latest {
        ...
        return if language.is_chinese() {
            format!("已为你进入伙伴模式，并恢复最近会话「{name}」。直接发送消息即可继续对话。")
        } else {
            format!(
                "Partner mode is on; resumed your latest session \"{name}\". Send a message to continue."
            )
        };
    }

    let create_res = handle_new_session(state, "Partner").await;
    if state.current_session_id.is_none() {
        return if language.is_chinese() {
            ...
            format!(
                "Partner mode is on, but session creation failed: {}",
    ...
    if language.is_chinese() {
        "已进入伙伴模式；尚无历史会话，已为你新建伙伴会话。直接发送消息即可开始。".to_string()
    } else {
        "Partner mode is on; no prior sessions were found, so a new partner session was created. Send a message to start."
            .to_string()
    }
}

/// Mark chat paired, run partner/session bootstrap, return first user-visible message + main menu actions.
...
fn label_new_partner_session(language: BotLanguage) -> &'static str {
    if language.is_chinese() {
        "新建伙伴会话"
    } else {
        "New Partner Session"
...
fn label_switch_partner(language: BotLanguage) -> &'static str {
    if language.is_chinese() {
        "切换伙伴"
    } else {
        "Switch Partner"
...
fn label_switch_partner_mode(language: BotLanguage) -> &'static str {
    if language.is_chinese() {
        "伙伴模式"
    } else {
        "Partner Mode"
...
pub fn main_menu_actions(language: BotLanguage, display_mode: BotDisplayMode) -> Vec<BotAction> {
    ...
    if is_pro {
        ...
        vec![
            ...
            BotAction::secondary(label_switch_partner_mode(language), "/partner"),
        ...
        // Partner mode: show partner switch (not workspace)
        vec![
            BotAction::primary(label_switch_partner(language), "/switch_partner"),
            BotAction::secondary(label_resume_session(language), "/resume_session"),
            BotAction::secondary(label_switch_pro_mode(language), "/pro"),
            BotAction::secondary(label_new_partner_session(language), "/new_partner_session"),
            BotAction::secondary(label_help(language), "/help"),
        ]
    }
}
...
fn partner_mode_actions(language: BotLanguage) -> Vec<BotAction> {
    vec![
        BotAction::primary(label_new_partner_session(language), "/new_partner_session"),
        BotAction::secondary(label_switch_partner(language), "/switch_partner"),
        BotAction::secondary(label_switch_pro_mode(language), "/pro"),
        BotAction::secondary(label_help(language), "/help"),
    ]
}
...
fn partner_required_actions(language: BotLanguage) -> Vec<BotAction> {
    vec![BotAction::primary(
        label_switch_partner(language),
        "/switch_partner",
    )]
}
...
async fn dispatch_im_bot_command_inner(
    ...
    match cmd {
        ...
        BotCommand::PairingCode(_) => HandleResult {
            reply: if language.is_chinese() {
                "配对码会自动处理。如果你需要重新配对，请在 OpenHarness Desktop 中重新启动连接。"
                    .to_string()
            } else {
                "Pairing codes are handled automatically. If you need to re-pair, please restart the connection from OpenHarness Desktop."
                    .to_string()
            },
            actions: vec![],
            forward_to_session: None,
        ...
        BotCommand::SwitchPartner => {
            if !state.paired {
                return not_paired(language);
            }
            handle_switch_partner(state).await
        }
        ...
        BotCommand::NewPartnerSession => {
            if !state.paired {
                return not_paired(language);
            }
            // Partner session only available in Partner mode
            if state.display_mode != BotDisplayMode::Partner {
                return wrong_mode_for_partner(language);
            }
            // Partner sessions don't need workspace
            handle_new_session(state, "Partner").await
        }
...
fn not_paired(language: BotLanguage) -> HandleResult {
    HandleResult {
        reply: if language.is_chinese() {
            "尚未连接到 OpenHarness Desktop。请先发送 6 位配对码。".to_string()
        } else {
            "Not connected to OpenHarness Desktop. Please enter the 6-digit pairing code first."
                .to_string()
        },
        actions: vec![],
        forward_to_session: None,
...
fn need_partner(language: BotLanguage) -> HandleResult {
    HandleResult {
        reply: if language.is_chinese() {
            "尚未选择伙伴。请先使用 /switch_partner。".to_string()
        } else {
            "No partner selected. Use /switch_partner first.".to_string()
        },
        actions: partner_required_actions(language),
        forward_to_session: None,
    }
}
...
fn wrong_mode_for_partner(language: BotLanguage) -> HandleResult {
    HandleResult {
        reply: if language.is_chinese() {
            "该会话只能在伙伴模式下创建。请先发送 /partner 切换到伙伴模式。".to_string()
        } else {
            "This session can only be created in Partner mode. Please send /partner to switch to Partner mode.".to_string()
        },
        actions: partner_mode_actions(language),
        forward_to_session: None,
    }
}
...
async fn handle_switch_workspace(state: &mut BotChatState) -> HandleResult {
    ...
    if workspaces.is_empty() {
        return HandleResult {
            reply: if language.is_chinese() {
                "未找到工作区。请先在 OpenHarness Desktop 中打开一个项目。".to_string()
            } else {
                "No workspaces found. Please open a project in OpenHarness Desktop first."
                    .to_string()
            },
            actions: vec![],
            forward_to_session: None,
...
async fn handle_switch_partner(state: &mut BotChatState) -> HandleResult {
    use crate::service::workspace::get_global_workspace_service;
    let language = current_bot_language().await;
    ...
    let partners = ws_service.get_partner_workspaces().await;
    if partners.is_empty() {
        return HandleResult {
            reply: if language.is_chinese() {
                "未找到伙伴。请先在 OpenHarness Desktop 中创建伙伴。".to_string()
            } else {
                "No partners found. Please create an partner in OpenHarness Desktop first."
                    .to_string()
            },
            actions: partner_mode_actions(language),
            forward_to_session: None,
    ...
    let effective_current: Option<&str> = state.current_partner.as_deref();

    let mut text = if language.is_chinese() {
        String::from("请选择伙伴：\n\n")
    } else {
        String::from("Select an partner:\n\n")
    };
    let mut options: Vec<(String, String)> = Vec::new();
    for (i, ws) in partners.iter().enumerate() {
        let path = ws.root_path.to_string_lossy().to_string();
        let is_current = effective_current == Some(path.as_str());
        let marker = if is_current {
            if language.is_chinese() {
    ...
    text.push_str(if language.is_chinese() {
        ...
        "\nReply with the partner number."
    });

    let action_labels: Vec<String> = options.iter().map(|(_, name)| name.clone()).collect();
    state.pending_action = Some(PendingAction::SelectPartner { options });
    HandleResult {
        reply: text,
        actions: numbered_actions(&action_labels),
        forward_to_session: None,
...
async fn handle_resume_session(state: &mut BotChatState, page: usize) -> HandleResult {
    ...
    let ws_path = if state.display_mode == BotDisplayMode::Pro {
        ...
        match &state.current_partner {
            Some(p) => std::path::PathBuf::from(p),
            None => return need_partner(language),
        }
    };
    ...
    if all_meta.is_empty() {
        let reply = if language.is_chinese() {
            if state.display_mode == BotDisplayMode::Pro {
                ...
                "当前工作区没有会话。请使用 /new_partner_session 创建一个。".to_string()
            }
        } else {
            if state.display_mode == BotDisplayMode::Pro {
                "No sessions found in this workspace. Use /new_code_session or /new_cowork_session to create one.".to_string()
            } else {
                "No sessions found in this workspace. Use /new_partner_session to create one."
                    .to_string()
            }
        };
        return HandleResult {
...
async fn handle_new_session(state: &mut BotChatState, agent_type: &str) -> HandleResult {
    ...
    let language = current_bot_language().await;
    let is_partner = agent_type == "Partner";
    ...
    let coordinator = match get_global_coordinator() {
        ...
        None => {
            return HandleResult {
                reply: if language.is_chinese() {
                    "OpenHarness 会话系统尚未就绪。".to_string()
                } else {
                    "OpenHarness session system not ready.".to_string()
                },
                actions: vec![],
                forward_to_session: None,
            };
    ...
    let ws_path = if is_partner {
        // For Partner sessions, prefer current_partner, or get/create default
        if let Some(ref partner_path) = state.current_partner {
            Some(partner_path.clone())
        } else {
            let ws_service = match get_global_workspace_service() {
                Some(s) => s,
                None => {
            ...
            // Get or create default partner workspace
            let workspaces = ws_service.get_partner_workspaces().await;
            let resolved =
                if let Some(default_ws) = workspaces.into_iter().find(|w| w.partner_id.is_none()) {
                        ...
                        Err(e) => {
                            return HandleResult {
                                reply: if language.is_chinese() {
                                    ...
                                    format!("Failed to create partner workspace: {}", e)
                                },
                                actions: vec![],
                                forward_to_session: None,
                            };
            ...
            if let Some(ref path) = resolved {
                state.current_partner = Some(path.clone());
            }
            resolved
        }
    } else {
...
async fn handle_number_selection(state: &mut BotChatState, n: usize) -> HandleResult {
    ...
    match pending {
        ...
        Some(PendingAction::SelectPartner { options }) => {
            if n < 1 || n > options.len() {
                state.pending_action = Some(PendingAction::SelectPartner { options });
                return HandleResult {
                    reply: if language.is_chinese() {
                        format!(
                            ...
                            state
                                ...
                                .as_ref()
                                .map(|a| match a {
                                    PendingAction::SelectPartner { options } => options.len(),
                                    _ => 0,
                                })
                                .unwrap_or(0)
                        )
            ...
            let (path, name) = options[n - 1].clone();
            select_partner(state, &path, &name).await
        }
...
async fn select_partner(state: &mut BotChatState, path: &str, name: &str) -> HandleResult {
    use crate::service::workspace::get_global_workspace_service;
    let language = current_bot_language().await;
    ...
    match ws_service.open_workspace(path_buf).await {
        Ok(info) => {
            ...
            .await
            {
                error!("Failed to init snapshot after bot partner switch: {e}");
            }
            state.current_partner = Some(path.to_string());
            state.current_session_id = None;
            info!("Bot switched partner to: {path}");
        ...
        Err(e) => HandleResult {
            reply: if language.is_chinese() {
                ...
                format!("Failed to switch partner: {e}")
            },
            actions: vec![],
            forward_to_session: None,
        },
...
fn build_workspace_switched_reply(
    ...
    if is_pro {
        ...
        if language.is_chinese() {
            reply.push_str(
                "/resume_session - 恢复已有会话\n\
                 /new_partner_session - 开始新的伙伴会话\n\
                 /pro - 切换到专业模式",
            );
        } else {
            reply.push_str(
                "/resume_session - Resume an existing session\n\
                 /new_partner_session - Start a new partner session\n\
...
async fn select_session(
    ...
    if let Some((user_text, partner_text)) = last_pair {
        reply.push_str(if language.is_chinese() {
            "— 最近一次对话 —\n"
        } else {
            "— Last conversation —\n"
        ...
        reply.push_str(&format!("{}: {partner_text}\n\n", "AI"));
        reply.push_str(if language.is_chinese() {
            "你可以继续对话。"
        } else {
...
/// Load the last user/partner dialog pair from the unified project session store,
...
async fn submit_question_answers(tool_id: &str, answers: &[Value]) -> HandleResult {
    ...
    match manager.send_answer(tool_id, Value::Object(payload)) {
        Ok(_) => HandleResult {
            reply: "Answers submitted. Waiting for the partner to continue...".to_string(),
            actions: vec![],
            forward_to_session: None,
        },
        Err(e) => HandleResult {
...
async fn handle_question_reply(
    ...
    if language.is_chinese()
        && result.reply == "Answers submitted. Waiting for the partner to continue..."
    {
        result.reply = "答案已提交，等待伙伴继续...".to_string();
    }
    result
}
...
async fn handle_chat_message(
    ...
    if let Some(pending) = state.pending_action.clone() {
        return match pending {
            ...
            PendingAction::SelectPartner { .. } => HandleResult {
                reply: if language.is_chinese() {
                    "请回复伙伴编号。".to_string()
                } else {
                    "Please reply with the partner number.".to_string()
                },
                actions: vec![],
                forward_to_session: None,
            },
    ...
    if state.current_session_id.is_none() {
        let reply = if language.is_chinese() {
            if state.display_mode == BotDisplayMode::Pro {
                ...
                "当前没有活动会话。请使用 /resume_session 恢复已有会话，或使用 /new_partner_session 创建新会话。"
                    .to_string()
            }
        } else {
            if state.display_mode == BotDisplayMode::Pro {
                ...
                "No active session. Use /resume_session to resume one or /new_partner_session to create a new one."
                    .to_string()
            }
        };
        return HandleResult {
    ...
    if session_busy {
        return HandleResult {
            reply: if language.is_chinese() {
                "消息已加入队列，将在当前伙伴步骤结束后自动处理。".to_string()
            } else {
                "Your message was queued and will run after the current partner step finishes."
                    .to_string()
            },
            actions: vec![],
