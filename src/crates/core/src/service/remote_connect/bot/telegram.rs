...
//! in OpenHarness settings.  The desktop polls for updates via the Telegram Bot
...
impl TelegramBot {
    ...
    fn enter_pairing_code_message(language: BotLanguage) -> &'static str {
        if language.is_chinese() {
            "请输入 OpenHarness Desktop 中显示的 6 位配对码。"
        } else {
            "Please enter the 6-digit pairing code from OpenHarness Desktop."
        }
    }
    ...
    pub async fn set_bot_commands(&self) -> Result<()> {
        ...
        let commands = serde_json::json!({
            "commands": [
                ...
                { "command": "partner", "description": "Switch to Partner mode (Partner)" },
                ...
                { "command": "new_partner_session", "description": "Create partner session (Partner)" },
