...
type ConfigMigrationFn = fn(Value) -> OpenHarnessResult<Value>;
...
impl ConfigManager {
    ...
    pub async fn new(settings: ConfigManagerSettings) -> OpenHarnessResult<Self> {
    ...
    async fn load_or_create_config(&mut self) -> OpenHarnessResult<()> {
    ...
    async fn load_and_migrate_config(&mut self) -> OpenHarnessResult<()> {
        let content = fs::read_to_string(&self.config_file)
            .await
            .map_err(|e| OpenHarnessError::config(format!("Failed to read config file: {}", e)))?;

        let mut config_value: Value = serde_json::from_str(&content).map_err(|e| {
            OpenHarnessError::config(format!("Failed to parse config file as JSON: {}", e))
        })?;
    ...
    async fn smart_merge_config_from_value(&mut self, user_value: Value) -> OpenHarnessResult<()> {
        let base_config = self.providers.get_default_config();

        let base_value = serde_json::to_value(&base_config).map_err(|e| {
            OpenHarnessError::config(format!("Failed to serialize default config: {}", e))
        })?;
        ...
        let mut config: GlobalConfig = serde_json::from_value(merged_value).map_err(|e| {
            OpenHarnessError::config(format!("Failed to deserialize merged config: {}", e))
        })?;
    ...
    async fn save_config(&self) -> OpenHarnessResult<()> {
        let content = serde_json::to_string_pretty(&self.config)
            .map_err(|e| OpenHarnessError::config(format!("Config serialization failed: {}", e)))?;

        if let Some(parent) = self.config_file.parent() {
            if !parent.exists() {
                fs::create_dir_all(parent).await.map_err(|e| {
                    OpenHarnessError::config(format!(
    ...
    pub fn get<T>(&self, path: &str) -> OpenHarnessResult<T>
    where
    ...
    pub async fn set<T>(&mut self, path: &str, value: T) -> OpenHarnessResult<()>
    where
        T: serde::Serialize,
    {
        let old_config = self.config.clone();
        let json_value = serde_json::to_value(value).map_err(|e| {
            OpenHarnessError::config(format!("Failed to serialize config value: {}", e))
        })?;
    ...
    pub async fn reset(&mut self, path: Option<&str>) -> OpenHarnessResult<()> {
        let old_config = self.config.clone();
    ...
    pub async fn validate_config(&self) -> OpenHarnessResult<ConfigValidationResult> {
        self.providers.validate_config(&self.config).await
    }

    /// Exports configuration.
    pub fn export_config(&self) -> OpenHarnessResult<serde_json::Value> {
        serde_json::to_value(&self.config)
            .map_err(|e| OpenHarnessError::config(format!("Failed to export config: {}", e)))
    }

    /// Imports configuration.
    pub async fn import_config(&mut self, config_data: serde_json::Value) -> OpenHarnessResult<()> {
        let old_config = self.config.clone();

        let imported_config: GlobalConfig = serde_json::from_value(config_data).map_err(|e| {
            OpenHarnessError::config(format!("Failed to parse imported config: {}", e))
        })?;
    ...
    pub async fn create_backup(&self) -> OpenHarnessResult<PathBuf> {
        let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S");
        let backup_dir = self.config_dir.join("backups");

        if !backup_dir.exists() {
            fs::create_dir_all(&backup_dir).await.map_err(|e| {
                OpenHarnessError::config(format!("Failed to create backup directory: {}", e))
            })?;
        }
        ...
        let content = serde_json::to_string_pretty(&self.config)
            .map_err(|e| OpenHarnessError::config(format!("Failed to serialize backup: {}", e)))?;

        fs::write(&backup_file, content)
            .await
            .map_err(|e| OpenHarnessError::config(format!("Failed to write backup: {}", e)))?;
    ...
    fn get_value_by_path(&self, path: &str) -> OpenHarnessResult<serde_json::Value> {
    ...
    fn get_value_by_path_from_config(
        ...
        let config_value = serde_json::to_value(config)
            .map_err(|e| OpenHarnessError::config(format!("Failed to serialize config: {}", e)))?;
        ...
        for key in keys {
            current = current.get(key).ok_or_else(|| {
                OpenHarnessError::config(format!("Config path '{}' not found", path))
            })?;
        }
    ...
    fn set_value_by_path(&mut self, path: &str, value: serde_json::Value) -> OpenHarnessResult<()> {
        if path.is_empty() {
            self.config = serde_json::from_value(value).map_err(|e| {
                OpenHarnessError::config(format!("Failed to deserialize config: {}", e))
            })?;
            return Ok(());
        }
        ...
        let last_key = keys.last().ok_or_else(|| {
            OpenHarnessError::config(format!("Config path '{}' does not contain any keys", path))
        })?;
        ...
        self.config = serde_json::from_value(config_value).map_err(|e| {
            OpenHarnessError::config(format!("Failed to deserialize updated config: {}", e))
        })?;
...
pub(crate) fn migrate_0_0_0_to_1_0_0(mut config: Value) -> OpenHarnessResult<Value> {
