...
impl ConfigService {
    ...
    pub async fn with_settings(settings: ConfigManagerSettings) -> OpenHarnessResult<Self> {
    ...
    pub async fn get_config<T>(&self, path: Option<&str>) -> OpenHarnessResult<T>
    where
    ...
    pub async fn set_config<T>(&self, path: &str, value: T) -> OpenHarnessResult<()>
    where
        T: serde::Serialize,
    {
    ...
    pub async fn reset_config(&self, path: Option<&str>) -> OpenHarnessResult<()> {
        let mut manager = self.manager.write().await;
    ...
    pub async fn export_config(&self) -> OpenHarnessResult<ConfigExport> {
        let manager = self.manager.read().await;
    ...
    pub async fn import_config(
        &self,
        export: ConfigExport,
    ) -> OpenHarnessResult<ConfigImportResult> {
        let mut manager = self.manager.write().await;
    ...
    /// Runs a health check.
    pub async fn health_check(&self) -> OpenHarnessResult<ConfigHealthStatus> {
        let manager = self.manager.read().await;
    ...
    pub async fn reload(&self) -> OpenHarnessResult<()> {
        let settings = ConfigManagerSettings::default();
    ...
    pub async fn create_backup(&self) -> OpenHarnessResult<std::path::PathBuf> {
        let manager = self.manager.read().await;
    ...
    pub async fn get_ai_models(&self) -> OpenHarnessResult<Vec<AIModelConfig>> {
        let config: GlobalConfig = self.get_config(None).await?;
        Ok(config.ai.models)
    }

    /// Adds an AI model configuration.
    pub async fn add_ai_model(&self, model: AIModelConfig) -> OpenHarnessResult<()> {
    ...
    pub async fn update_ai_model(
        ...
        if let Some(existing_model) = config.ai.models.iter_mut().find(|m| m.id == model_id) {
            *existing_model = model;
            ...
            Err(OpenHarnessError::config(format!(
                "AI model '{}' not found",
                model_id
            )))
        }
    ...
    pub async fn delete_ai_model(&self, model_id: &str) -> OpenHarnessResult<()> {
        let mut config: GlobalConfig = self.get_config(None).await?;
