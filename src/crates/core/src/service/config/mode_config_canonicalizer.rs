...
fn canonicalize_mode_config(
    ...
    default_tools: &[String],
    valid_tools: &HashSet<String>,
) -> OpenHarnessResult<Option<ModeConfig>> {
...
pub async fn get_mode_config_views() -> OpenHarnessResult<HashMap<String, ModeConfigView>> {
...
pub async fn get_mode_config_view(mode_id: &str) -> OpenHarnessResult<ModeConfigView> {
    let views = get_mode_config_views().await?;
    views
        .get(mode_id)
        .cloned()
        .ok_or_else(|| OpenHarnessError::config(format!("Mode does not exist: {}", mode_id)))
}

pub async fn persist_mode_config_from_value(mode_id: &str, config: Value) -> OpenHarnessResult<()> {
    let config_service = GlobalConfigManager::get_service().await?;
...
pub async fn reset_mode_config_to_default(mode_id: &str) -> OpenHarnessResult<()> {
    let config_service = GlobalConfigManager::get_service().await?;
    let mut stored_configs: HashMap<String, ModeConfig> = config_service
        .get_config(Some("ai.mode_configs"))
        .await
...
pub async fn canonicalize_mode_configs() -> OpenHarnessResult<ModeConfigCanonicalizationReport> {
    let config_service = GlobalConfigManager::get_service().await?;
    let valid_tools = get_valid_tool_names().await;
    let mode_defaults = get_mode_defaults().await;
    ...
    let ai_object = ai_value
        .as_object_mut()
        .ok_or_else(|| OpenHarnessError::config("AI config must be a JSON object".to_string()))?;
...
mod tests {
    ...
    fn normalize_skill_override_lists_removes_duplicates_and_conflicts() {
        let (disabled, enabled) = normalize_skill_override_lists(
            vec![
                "user::openharness::pdf".to_string(),
                "user::openharness::pdf".to_string(),
            ],
            vec![
                "user::openharness::pdf".to_string(),
                "user::openharness::docx".to_string(),
                "user::openharness::docx".to_string(),
            ],
        );

        assert_eq!(disabled, vec!["user::openharness::pdf".to_string()]);
        assert_eq!(enabled, vec!["user::openharness::docx".to_string()]);
    }
    ...
    fn stored_mode_from_overrides_keeps_enabled_user_skills() {
        ...
        let stored = stored_mode_from_overrides(
            ...
            Vec::new(),
            Vec::new(),
            vec!["user::openharness::pdf".to_string()],
            &[],
            &valid_tools,
        )
