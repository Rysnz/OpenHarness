...
impl GlobalConfigManager {
    ...
    pub async fn initialize() -> OpenHarnessResult<()> {
        ...
        CONFIG_UPDATE_SENDER.set(sender).map_err(|_| {
            OpenHarnessError::config("Failed to initialize config update sender".to_string())
        })?;
        ...
        GLOBAL_CONFIG_SERVICE.set(service_wrapper).map_err(|_| {
            OpenHarnessError::config("Failed to initialize global config service".to_string())
        })?;
    ...
    pub async fn get_service() -> OpenHarnessResult<Arc<ConfigService>> {
        let service_wrapper = GLOBAL_CONFIG_SERVICE.get().ok_or_else(|| {
            OpenHarnessError::config("Global config service not initialized".to_string())
        })?;

        let service_guard = service_wrapper.read().await;
        service_guard
            .as_ref()
            .ok_or_else(|| OpenHarnessError::config("Global config service is None".to_string()))
            .map(Arc::clone)
    }

    /// Updates the global configuration service instance (used for configuration reload).
    pub async fn update_service(new_service: Arc<ConfigService>) -> OpenHarnessResult<()> {
        let service_wrapper = GLOBAL_CONFIG_SERVICE.get().ok_or_else(|| {
            OpenHarnessError::config("Global config service not initialized".to_string())
        })?;
    ...
    pub async fn reload() -> OpenHarnessResult<()> {
    ...
    pub async fn update_theme(&self, theme_id: &str) -> OpenHarnessResult<()> {
        let service = Self::get_service().await?;
