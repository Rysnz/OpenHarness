...
impl Default for ThemesConfig {
    fn default() -> Self {
        Self {
            current: "openharness-light".to_string(),
            custom: None,
        }
    }
}
...
pub struct DebugModeConfig {
    /// Custom log path (relative to the workspace; default: `.openharness/debug.log`).
    pub log_path: String,
...
impl Default for DebugModeConfig {
    fn default() -> Self {
        Self {
            log_path: ".openharness/debug.log".to_string(),
            ingest_port: 7242,
...
pub trait ConfigProvider: Send + Sync {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> OpenHarnessResult<Vec<String>>;
