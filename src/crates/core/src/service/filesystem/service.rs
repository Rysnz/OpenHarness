...
impl FileSystemService {
    ...
    /// Builds a file tree.
    pub async fn build_file_tree(&self, root_path: &str) -> OpenHarnessResult<Vec<FileTreeNode>> {
    ...
    ) -> OpenHarnessResult<Vec<FileTreeNode>> {
    ...
    pub async fn scan_directory(&self, root_path: &str) -> OpenHarnessResult<DirectoryScanResult> {
    ...
    pub async fn get_directory_contents(&self, path: &str) -> OpenHarnessResult<Vec<FileTreeNode>> {
    ...
    ) -> OpenHarnessResult<Vec<FileSearchResult>> {
    ...
    ) -> OpenHarnessResult<FileSearchOutcome> {
    ...
    /// Gets the file size.
    pub async fn get_file_size(&self, file_path: &str) -> OpenHarnessResult<u64> {
        let info = self.get_file_info(file_path).await?;
        Ok(info.size)
    }

    /// Reads a text file quickly.
    pub async fn read_text_file(&self, file_path: &str) -> OpenHarnessResult<String> {
    ...
    pub async fn write_text_file(&self, file_path: &str, content: &str) -> OpenHarnessResult<()> {
        self.write_file(file_path, content).await.map(|_| ())
    }

    /// Lists all files in a directory (recursive).
    pub async fn list_all_files(&self, root_path: &str) -> OpenHarnessResult<Vec<String>> {
    ...
    pub async fn calculate_directory_size(&self, dir_path: &str) -> OpenHarnessResult<u64> {
    ...
    pub async fn get_directory_stats(&self, dir_path: &str) -> OpenHarnessResult<DirectoryStats> {
        let scan_result = self.scan_directory(dir_path).await?;
