...
pub async fn initialize_global_ai_rules_service() -> OpenHarnessResult<()> {
    ...
    let path_manager = try_get_path_manager_arc()
        .map_err(|e| OpenHarnessError::service(format!("Failed to create PathManager: {}", e)))?;
    ...
    GLOBAL_AI_RULES_SERVICE.set(wrapper).map_err(|_| {
        OpenHarnessError::service("Failed to initialize global AIRulesService".to_string())
    })?;
...
pub async fn get_global_ai_rules_service() -> OpenHarnessResult<Arc<AIRulesService>> {
    let wrapper = GLOBAL_AI_RULES_SERVICE.get()
        .ok_or_else(|| OpenHarnessError::service(
    ...
    guard
        .as_ref()
        .ok_or_else(|| OpenHarnessError::service("Global AIRulesService is None".to_string()))
        .map(Arc::clone)
}
...
impl AIRulesService {
    ...
    pub async fn set_workspace(&self, workspace_path: PathBuf) -> OpenHarnessResult<()> {
    ...
    pub async fn get_user_rules(&self) -> OpenHarnessResult<Vec<AIRule>> {
        Ok(self.user_rules.read().await.clone())
    }

    /// Returns a single user-level rule.
    pub async fn get_user_rule(&self, name: &str) -> OpenHarnessResult<Option<AIRule>> {
        let rules = self.user_rules.read().await;
        Ok(rules.iter().find(|r| r.name == name).cloned())
    }

    /// Creates a user-level rule.
    pub async fn create_user_rule(&self, request: CreateRuleRequest) -> OpenHarnessResult<AIRule> {
        ...
        self.get_user_rule(&rule_name)
            .await?
            .ok_or_else(|| OpenHarnessError::service("Failed to create rule".to_string()))
    }
    ...
    pub async fn update_user_rule(
        ...
        self.get_user_rule(new_name)
            .await?
            .ok_or_else(|| OpenHarnessError::service("Failed to update rule".to_string()))
    }

    /// Deletes a user-level rule.
    pub async fn delete_user_rule(&self, name: &str) -> OpenHarnessResult<bool> {
        let rules_dir = self.path_manager.user_rules_dir();
    ...
    pub async fn reload_user_rules(&self) -> OpenHarnessResult<()> {
        let rules_dir = self.path_manager.user_rules_dir();
        let rules = self
    ...
    pub async fn get_user_rules_stats(&self) -> OpenHarnessResult<RuleStats> {
        let rules = self.user_rules.read().await;
        Ok(Self::calculate_stats(&rules))
    }

    /// Returns all project-level rules.
    pub async fn get_project_rules(&self) -> OpenHarnessResult<Vec<AIRule>> {
    ...
    pub async fn get_project_rules_for_workspace(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<Vec<AIRule>> {
        self.load_project_rules_for_workspace(workspace).await
    }

    /// Returns a single project-level rule.
    pub async fn get_project_rule(&self, name: &str) -> OpenHarnessResult<Option<AIRule>> {
    ...
    pub async fn get_project_rule_for_workspace(
        ...
        workspace: &Path,
        name: &str,
    ) -> OpenHarnessResult<Option<AIRule>> {
    ...
    pub async fn create_project_rule(
        &self,
    ...
    /// Supports rules from both the OpenHarness and Cursor directories.
    ...
    pub async fn update_project_rule_for_workspace(
        ...
        let rule = self
            ...
            .await?
            .ok_or_else(|| OpenHarnessError::service(format!("Rule '{}' not found", name)))?;

        let rules_dir = rule
            .file_path
            .parent()
            .ok_or_else(|| OpenHarnessError::service("Invalid rule file path".to_string()))?;
    ...
    /// Supports rules from both the OpenHarness and Cursor directories.
    pub async fn delete_project_rule(&self, name: &str) -> OpenHarnessResult<bool> {
        let workspace_path = self.require_workspace_path().await?;
    ...
    /// Loads OpenHarness rules first, then Cursor rules; for duplicates, the first loaded wins.
    pub async fn reload_project_rules(&self) -> OpenHarnessResult<()> {
    ...
    pub async fn reload_project_rules_for_workspace(
        &self,
        workspace: &Path,
    ) -> OpenHarnessResult<()> {
        let rules = self.load_project_rules_for_workspace(workspace).await?;
    ...
    pub async fn get_project_rules_stats(&self) -> OpenHarnessResult<RuleStats> {
        let rules = self.project_rules.read().await;
        Ok(Self::calculate_stats(&rules))
    }
    ...
    async fn load_project_rules_for_workspace(
        ...
        let openharness_rules_dir = self.path_manager.project_rules_dir(workspace);
        let openharness_rules = self
            .load_rules_from_dir(&openharness_rules_dir, RuleLevel::Project)
            .await?;
        ...
        if cursor_rules_dir.exists() {
            ...
            for rule in cursor_rules {
                if !loaded_names.contains(&rule.name) {
                    ...
                    debug!(
                        "Skipping Cursor rule '{}' (already loaded from OpenHarness)",
                        rule.name
                    );
                }
            }
...
"#);
    ...
    pub async fn build_system_prompt(&self) -> OpenHarnessResult<String> {
        let user_rules = self.user_rules.read().await.clone();
    ...
    async fn load_rule_from_file(
        &self,
        path: &Path,
        level: RuleLevel,
    ) -> OpenHarnessResult<AIRule> {
        let content = tokio::fs::read_to_string(path).await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to read file {:?}: {}", path, e))
        })?;
        ...
        let name = path
            ...
            .and_then(|s| s.to_str())
            .map(|s| s.to_string())
            .ok_or_else(|| OpenHarnessError::service("Invalid file name".to_string()))?;

        AIRule::from_mdc(name, level, path.to_path_buf(), &content)
            .map_err(OpenHarnessError::service)
    }
    ...
    async fn create_rule_internal(
        ...
        tokio::fs::create_dir_all(dir)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to create directory: {}", e)))?;
        ...
        tokio::fs::write(&file_path, mdc_content)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to write file: {}", e)))?;
    ...
    async fn update_rule_internal(
        ...
        let content = tokio::fs::read_to_string(&old_file_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to read file: {}", e)))?;

        let (mut frontmatter, mut body) =
            parse_mdc_content(&content).map_err(OpenHarnessError::service)?;
        ...
        let new_file_path = if let Some(new_name) = &request.name {
            if new_name != name {
                ...
                tokio::fs::remove_file(&old_file_path).await.map_err(|e| {
                    OpenHarnessError::service(format!("Failed to delete old file: {}", e))
                })?;
                new_path
            } else {
                old_file_path
    ...
    /// Deletes a rule file.
    async fn delete_rule_internal(&self, dir: &Path, name: &str) -> OpenHarnessResult<bool> {
        ...
        tokio::fs::remove_file(&file_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to delete file: {}", e)))?;
    ...
    pub async fn toggle_user_rule(&self, name: &str) -> OpenHarnessResult<AIRule> {
        let rule = self
            .get_user_rule(name)
            .await?
            .ok_or_else(|| OpenHarnessError::service(format!("Rule '{}' not found", name)))?;
    ...
    pub async fn toggle_project_rule(&self, name: &str) -> OpenHarnessResult<AIRule> {
        let workspace_path = self.require_workspace_path().await?;
    ...
    async fn require_workspace_path(&self) -> OpenHarnessResult<PathBuf> {
        self.workspace_path
            .read()
            .await
            .clone()
            .ok_or_else(|| OpenHarnessError::service("No workspace set".to_string()))
    }
