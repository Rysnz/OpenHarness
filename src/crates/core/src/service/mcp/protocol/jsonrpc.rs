...
pub fn create_initialize_request(
    ...
    let params = InitializeParams {
        ...
        client_info: MCPServerInfo {
            ...
            description: Some("OpenHarness MCP Client".to_string()),
            vendor: Some("OpenHarness".to_string()),
        },
    };
...
pub fn parse_response_result<T>(response: &MCPResponse) -> crate::util::errors::OpenHarnessResult<T>
where
    T: serde::de::DeserializeOwned,
{
    if let Some(error) = &response.error {
        return Err(crate::util::errors::OpenHarnessError::MCPError(format!(
            "MCP Error {}: {}",
    ...
    let result = response.result.as_ref().ok_or_else(|| {
        crate::util::errors::OpenHarnessError::MCPError(
            "Missing result in MCP response".to_string(),
        )
    })?;

    serde_json::from_value(result.clone()).map_err(|e| {
        crate::util::errors::OpenHarnessError::Deserialization(format!(
