...
use super::types::{
    InitializeResult as OpenHarnessInitializeResult, MCPAnnotations, MCPCapability, MCPPrompt,
...
#[derive(Clone)]
struct OpenHarnessRmcpClientHandler {
    info: ClientInfo,
}

impl ClientHandler for OpenHarnessRmcpClientHandler {
...
enum ClientState {
    Connecting {
        transport: Option<StreamableHttpClientTransport<OpenHarnessStreamableHttpClient>>,
    },
    Ready {
        service: Arc<RunningService<RoleClient, OpenHarnessRmcpClientHandler>>,
    },
}

#[derive(Clone)]
struct OpenHarnessStreamableHttpClient {
    client: reqwest::Client,
    oauth_manager: Option<Arc<Mutex<AuthorizationManager>>>,
}

impl OpenHarnessStreamableHttpClient {
...
impl StreamableHttpClient for OpenHarnessStreamableHttpClient {
...
impl RemoteMCPTransport {
    ...
    fn build_default_headers(headers: &HashMap<String, String>) -> HeaderMap {
        ...
        if !header_map.contains_key(USER_AGENT) {
            header_map.insert(
                USER_AGENT,
                HeaderValue::from_static("OpenHarness-MCP-Client/1.0"),
            );
        }
    ...
    pub async fn new(
        ...
        let transport = StreamableHttpClientTransport::with_client(
            OpenHarnessStreamableHttpClient {
                client: http_client,
    ...
    async fn service(
        &self,
    ) -> OpenHarnessResult<Arc<RunningService<RoleClient, OpenHarnessRmcpClientHandler>>> {
        let guard = self.state.lock().await;
        match &*guard {
            ClientState::Ready { service } => Ok(Arc::clone(service)),
            ClientState::Connecting { .. } => Err(OpenHarnessError::MCPError(
    ...
    pub async fn initialize(
        ...
        client_name: &str,
        client_version: &str,
    ) -> OpenHarnessResult<OpenHarnessInitializeResult> {
        let mut guard = self.state.lock().await;
        match &mut *guard {
            ClientState::Ready { service } => {
                let info = service.peer().peer_info().ok_or_else(|| {
                    OpenHarnessError::MCPError(
                        "Handshake succeeded but server info missing".to_string(),
                    )
                })?;
            ...
            ClientState::Connecting { transport } => {
                let Some(transport) = transport.take() else {
                    return Err(OpenHarnessError::MCPError(
                        "Remote MCP client already initializing".to_string(),
                    ));
                };

                let handler = OpenHarnessRmcpClientHandler {
                ...
                let service = tokio::time::timeout(self.request_timeout, transport_fut)
                    ...
                    .map_err(|e| OpenHarnessError::MCPError(format!("Handshake failed: {}", e)))?;
    ...
    pub async fn ping(&self) -> OpenHarnessResult<()> {
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP ping timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP ping failed: {}", e)))?;

        match result {
            rmcp::model::ServerResult::EmptyResult(_) => Ok(()),
            other => Err(OpenHarnessError::MCPError(format!(
    ...
    pub async fn list_resources(
    ...
    ) -> OpenHarnessResult<ResourcesListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP resources/list timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP resources/list failed: {}", e)))?;
    ...
    pub async fn read_resource(&self, uri: &str) -> OpenHarnessResult<ResourcesReadResult> {
        let service = self.service().await?;
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP resources/read timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP resources/read failed: {}", e)))?;
    ...
    pub async fn list_prompts(
        &self,
        cursor: Option<String>,
    ) -> OpenHarnessResult<PromptsListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP prompts/list timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP prompts/list failed: {}", e)))?;
    ...
    pub async fn get_prompt(
    ...
    ) -> OpenHarnessResult<PromptsGetResult> {
        let service = self.service().await?;
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP prompts/get timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP prompts/get failed: {}", e)))?;
    ...
    pub async fn list_tools(&self, cursor: Option<String>) -> OpenHarnessResult<ToolsListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP tools/list timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP tools/list failed: {}", e)))?;
    ...
    pub async fn call_tool(
    ...
    ) -> OpenHarnessResult<MCPToolResult> {
        let service = self.service().await?;
        ...
        let result = tokio::time::timeout(self.request_timeout, fut)
            .await
            .map_err(|_| OpenHarnessError::Timeout("MCP tools/call timeout".to_string()))?
            .map_err(|e| OpenHarnessError::MCPError(format!("MCP tools/call failed: {}", e)))?;

        Ok(map_tool_result(result))
    }
}

fn map_initialize_result(info: &rmcp::model::ServerInfo) -> OpenHarnessInitializeResult {
    OpenHarnessInitializeResult {
...
mod tests {
    ...
    fn build_client_info_declares_supported_client_capabilities() {
        let info = RemoteMCPTransport::build_client_info("OpenHarness", "1.0.0");
