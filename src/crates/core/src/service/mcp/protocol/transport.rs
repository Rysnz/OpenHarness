...
impl MCPTransport {
    ...
    /// Sends a response.
    pub async fn send_response(&self, id: Value, result: Value) -> OpenHarnessResult<()> {
        let response = MCPResponse::success(id, result);
        self.send_message(MCPMessage::Response(response)).await
    }

    /// Sends an error response.
    pub async fn send_error(&self, id: Value, error: MCPError) -> OpenHarnessResult<()> {
        let response = MCPResponse::error(id, error);
        self.send_message(MCPMessage::Response(response)).await
    }

    /// Sends a message.
    async fn send_message(&self, message: MCPMessage) -> OpenHarnessResult<()> {
        let json = serde_json::to_string(&message).map_err(|e| {
            OpenHarnessError::serialization(format!("Failed to serialize MCP message: {}", e))
        })?;

        let mut stdin = self.stdin.lock().await;
        stdin.write_all(json.as_bytes()).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to write to MCP server stdin: {}", e))
        })?;
        stdin.write_all(b"\n").await.map_err(|e| {
        ...
        stdin.flush().await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to flush MCP server stdin: {}", e))
        })?;
