...
pub fn map_auth_error(error: impl ToString) -> OpenHarnessError {
    OpenHarnessError::MCPError(format!("OAuth error: {}", error.to_string()))
}

pub async fn has_stored_oauth_credentials(server_id: &str) -> OpenHarnessResult<bool> {
...
pub async fn clear_stored_oauth_credentials(server_id: &str) -> OpenHarnessResult<()> {
...
pub async fn build_authorization_manager(
    server_id: &str,
    server_url: &str,
) -> OpenHarnessResult<(AuthorizationManager, bool)> {
...
fn effective_oauth_config(config: &MCPServerConfig) -> MCPServerOAuthConfig {
    ...
    if oauth.client_name.is_none() {
        oauth.client_name = Some(format!("OpenHarness MCP Client ({})", config.name));
    }
    oauth
}
...
) -> OpenHarnessResult<PreparedMCPRemoteOAuthAuthorization> {
