    ...
    fn copy(self) -> OAuthCallbackPageCopy {
        match self {
            Self::ZhCN => OAuthCallbackPageCopy {
                html_lang: "zh-CN",
                page_title: "OpenHarness OAuth 回调",
                brand_label: "OpenHarness Desktop",
                badge_success: "已收到授权",
                badge_warning: "回调参数不完整",
                badge_error: "授权失败",
                success_title: "OpenHarness 已收到 OAuth 回调",
                success_message: "可以返回 OpenHarness。应用正在交换授权码并重新连接 MCP 服务器。",
                success_detail_title: "接下来会发生什么",
                success_detail_body:
                    "这个页面可以直接关闭。如果 OpenHarness 没有自动完成重连，请回到 MCP 设置页后重试 OAuth。",
                warning_title: "OpenHarness 收到的 OAuth 回调缺少必要参数",
                warning_message:
                    "OAuth 提供方已跳转回来，但缺少必须的参数。请返回 OpenHarness 重新发起登录流程。",
                warning_detail_title: "缺少的参数",
                error_title: "OpenHarness 未能完成 OAuth 授权",
                error_message:
                    "请返回 OpenHarness，并根据下面的提供方返回信息检查问题后重新发起 OAuth。",
                error_detail_title: "提供方返回",
                close_hint: "处理完成后，这个页面可以直接关闭。",
            },
            Self::EnUS => OAuthCallbackPageCopy {
                html_lang: "en-US",
                page_title: "OpenHarness OAuth Callback",
                brand_label: "OpenHarness Desktop",
                badge_success: "Authorization received",
                badge_warning: "Callback incomplete",
                badge_error: "Authorization failed",
                success_title: "OpenHarness received the OAuth callback",
                success_message:
                    "You can return to OpenHarness now. The app is exchanging the authorization code and reconnecting the MCP server.",
                success_detail_title: "What happens next",
                success_detail_body:
                    "This page can be closed now. If OpenHarness does not finish reconnecting automatically, return to MCP settings and retry OAuth.",
                warning_title: "OpenHarness received an OAuth callback with missing parameters",
                warning_message:
                    "The provider redirected back, but required OAuth parameters were missing. Return to OpenHarness and start the sign-in flow again.",
                warning_detail_title: "Missing parameters",
                error_title: "OpenHarness could not finish the OAuth authorization",
                error_message:
                    "Return to OpenHarness and review the provider response below before retrying OAuth.",
...
impl MCPServerManager {
    ...
    pub async fn start_remote_oauth_authorization(
        &self,
        server_id: &str,
    ) -> OpenHarnessResult<MCPRemoteOAuthSessionSnapshot> {
        let config = self
            .config_service
            .get_server_config(server_id)
            .await?
            .ok_or_else(|| {
                OpenHarnessError::NotFound(format!("MCP server config not found: {}", server_id))
            })?;
        ...
        let callback_path = Url::parse(&prepared.redirect_uri)
            .map_err(|error| {
                OpenHarnessError::MCPError(format!(
        ...
        tokio::spawn(async move {
            let _ = MCPServerManager::update_oauth_snapshot(&callback_session, |snapshot| {
                ...
                snapshot.message = Some(
                    "Waiting for the OAuth provider to redirect back to OpenHarness.".to_string(),
                );
            })
            .await;
    ...
    pub async fn cancel_remote_oauth_authorization(
        &self,
        server_id: &str,
    ) -> OpenHarnessResult<()> {
    ...
    pub async fn clear_remote_oauth_credentials(&self, server_id: &str) -> OpenHarnessResult<()> {
