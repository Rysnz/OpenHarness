...
impl MCPServerManager {
    ...
    pub async fn initialize_all(&self) -> OpenHarnessResult<()> {
    ...
    pub async fn initialize_non_destructive(&self) -> OpenHarnessResult<()> {
    ...
    pub async fn ensure_registered(&self, server_id: &str) -> OpenHarnessResult<()> {
    ...
    /// Starts a server.
    pub async fn start_server(&self, server_id: &str) -> OpenHarnessResult<()> {
        self.start_reconnect_monitor_if_needed();
        ...
        if !config.enabled {
            ...
            return Err(OpenHarnessError::Configuration(format!(
        ...
        let process = self.registry.get_process(server_id).await.ok_or_else(|| {
            ...
            OpenHarnessError::NotFound(format!("MCP server not registered: {}", server_id))
        })?;
        ...
        match config.server_type {
            super::super::MCPServerType::Local => {
                ...
                let resolved = runtime_manager.resolve_command(command).ok_or_else(|| {
                    OpenHarnessError::ProcessError(format!(
                        "MCP server command '{}' not found in system PATH or OpenHarness managed runtimes at {}",
                        command,
            ...
            super::super::MCPServerType::Remote => {
                ...
                if transport != crate::service::mcp::server::MCPServerTransport::StreamableHttp {
                    ...
                    return Err(OpenHarnessError::NotImplemented(format!(
    ...
    /// Stops a server.
    pub async fn stop_server(&self, server_id: &str) -> OpenHarnessResult<()> {
        info!("Stopping MCP server: id={}", server_id);

        self.stop_connection_event_listener(server_id).await;

        let process = self.registry.get_process(server_id).await.ok_or_else(|| {
            OpenHarnessError::NotFound(format!("MCP server not found: {}", server_id))
        })?;
    ...
    /// Restarts a server.
    pub async fn restart_server(&self, server_id: &str) -> OpenHarnessResult<()> {
        ...
        match config.server_type {
            super::super::MCPServerType::Local => {
                ...
                let command = config.command.as_ref().ok_or_else(|| {
                    OpenHarnessError::Configuration("Missing command".to_string())
                })?;
    ...
    pub async fn get_server_status(&self, server_id: &str) -> OpenHarnessResult<MCPServerStatus> {
        if !self.registry.contains(server_id).await {
    ...
    /// Adds a server.
    pub async fn add_server(&self, config: MCPServerConfig) -> OpenHarnessResult<()> {
        config.validate()?;
    ...
    /// Removes a server.
    pub async fn remove_server(&self, server_id: &str) -> OpenHarnessResult<()> {
    ...
    pub async fn update_server_config(&self, config: MCPServerConfig) -> OpenHarnessResult<()> {
        config.validate()?;
    ...
    pub async fn clear_remote_server_auth(&self, server_id: &str) -> OpenHarnessResult<()> {
        self.clear_remote_oauth_credentials(server_id).await?;
        self.config_service
    ...
    pub async fn shutdown(&self) -> OpenHarnessResult<()> {
