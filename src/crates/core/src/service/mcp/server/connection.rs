...
impl MCPConnection {
        ...
        match &self.transport {
            TransportType::Local(transport) => {
                ...
                match tokio::time::timeout(self.request_timeout, rx).await {
                    ...
                    Ok(Err(_)) => Err(OpenHarnessError::MCPError(format!(
                        "Request channel closed for method: {}",
                        method
                    ))),
                    Err(_) => Err(OpenHarnessError::Timeout(format!(
                        "Request timeout for method: {}",
                        method
                    ))),
                }
            }
            TransportType::Remote(_transport) => Err(OpenHarnessError::NotImplemented(
    ...
    pub async fn initialize(
        ...
        client_name: &str,
        client_version: &str,
    ) -> OpenHarnessResult<InitializeResult> {
        match &self.transport {
    ...
    pub async fn send_response(&self, request_id: Value, result: Value) -> OpenHarnessResult<()> {
        match &self.transport {
            TransportType::Local(transport) => transport.send_response(request_id, result).await,
            TransportType::Remote(_) => Err(OpenHarnessError::NotImplemented(
    ...
    pub async fn send_error(&self, request_id: Value, error: MCPError) -> OpenHarnessResult<()> {
        match &self.transport {
