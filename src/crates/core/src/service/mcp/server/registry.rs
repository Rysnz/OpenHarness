...
impl MCPServerRegistry {
    ...
    pub async fn unregister(&self, server_id: &str) -> crate::util::errors::OpenHarnessResult<()> {
        let mut servers = self.servers.write().await;
        ...
        if let Some(process) = servers.remove(server_id) {
            ...
            Err(crate::util::errors::OpenHarnessError::NotFound(format!(
    ...
    /// Clears the registry.
    pub async fn clear(&self) -> crate::util::errors::OpenHarnessResult<()> {
        let mut servers = self.servers.write().await;
