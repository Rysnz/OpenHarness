...
impl MCPServerProcess {
    fn is_auth_error(error: &OpenHarnessError) -> bool {
    ...
    pub async fn start(
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| OpenHarnessError::ProcessError("Failed to capture stdin".to_string()))?;
        let stdout = child.stdout.take().ok_or_else(|| {
            OpenHarnessError::ProcessError("Failed to capture stdout".to_string())
        })?;
    ...
    pub async fn start_remote(&mut self, config: &MCPServerConfig) -> OpenHarnessResult<()> {
        let url = config.url.as_deref().ok_or_else(|| {
            OpenHarnessError::Configuration(format!(
        ...
        if !merged_headers.contains_key("Authorization")
            ...
            && !merged_headers.contains_key("AUTHORIZATION")
        {
            // Backward compatibility: older OpenHarness configs store `Authorization` under `env`.
    ...
    async fn handshake(&mut self) -> OpenHarnessResult<()> {
        let connection = self
            .connection
            .as_ref()
            .ok_or_else(|| OpenHarnessError::MCPError("Connection not established".to_string()))?;
        ...
        let result: InitializeResult = connection
            .initialize("OpenHarness", env!("CARGO_PKG_VERSION"))
            .await?;
    ...
    pub async fn stop(&mut self) -> OpenHarnessResult<()> {
    ...
    pub async fn restart(
        ...
        if self.restart_count >= self.max_restarts {
            ...
            .await;
            return Err(OpenHarnessError::MCPError(format!(
                "Max restart attempts ({}) reached",
                self.max_restarts
            )));
        }
...
mod tests {
    ...
    fn detect_auth_error_patterns() {
        let unauthorized =
            OpenHarnessError::MCPError("Handshake failed: Unauthorized (401)".to_string());
        assert!(MCPServerProcess::is_auth_error(&unauthorized));

        let oauth_refresh = OpenHarnessError::MCPError(
        ...
        let generic = OpenHarnessError::MCPError("Handshake failed: connection reset".to_string());
