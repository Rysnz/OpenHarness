...
impl MCPConfigService {
    ...
    pub fn new(config_service: Arc<ConfigService>) -> OpenHarnessResult<Self> {
        Ok(Self { config_service })
    }

    /// Loads all MCP server configurations.
    pub async fn load_all_configs(&self) -> OpenHarnessResult<Vec<MCPServerConfig>> {
    ...
    async fn load_builtin_configs(&self) -> OpenHarnessResult<Vec<MCPServerConfig>> {
        Ok(Vec::new())
    }

    /// Loads user-level configuration (supports Cursor format `{ "mcpServers": { "id": {..} } }`
    /// and array format `[{..}]`).
    async fn load_user_configs(&self) -> OpenHarnessResult<Vec<MCPServerConfig>> {
        match self
            .config_service
    ...
    async fn load_project_configs(&self) -> OpenHarnessResult<Vec<MCPServerConfig>> {
        match self
            .config_service
    ...
    pub async fn get_server_config(
        &self,
        server_id: &str,
    ) -> OpenHarnessResult<Option<MCPServerConfig>> {
        let all_configs = self.load_all_configs().await?;
        Ok(all_configs.into_iter().find(|c| c.id == server_id))
    }

    /// Saves a server configuration.
    pub async fn save_server_config(&self, config: &MCPServerConfig) -> OpenHarnessResult<()> {
        match config.location {
            ConfigLocation::BuiltIn => Err(OpenHarnessError::Configuration(
    ...
    pub async fn set_remote_authorization(
        ...
        let normalized =
            Self::normalize_authorization_value(authorization_value).ok_or_else(|| {
                OpenHarnessError::Validation("Authorization value cannot be empty".to_string())
            })?;
    ...
    async fn save_user_config(&self, config: &MCPServerConfig) -> OpenHarnessResult<()> {
    ...
    async fn save_project_config(&self, config: &MCPServerConfig) -> OpenHarnessResult<()> {
        ...
        let value = serde_json::to_value(&configs).map_err(|e| {
            OpenHarnessError::serialization(format!("Failed to serialize MCP config: {}", e))
        })?;
    ...
    pub async fn delete_server_config(&self, server_id: &str) -> OpenHarnessResult<()> {
        let current_value = self
            .config_service
            .get_config::<serde_json::Value>(Some("mcp_servers"))
            .await
