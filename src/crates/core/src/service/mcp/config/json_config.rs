...
impl MCPConfigService {
    ...
    pub async fn load_mcp_json_config(&self) -> OpenHarnessResult<String> {
        match self
            .config_service
    ...
    pub async fn save_mcp_json_config(&self, json_config: &str) -> OpenHarnessResult<()> {
        ...
        let config_value: serde_json::Value = serde_json::from_str(json_config).map_err(|e| {
            ...
            error!("{}", error_msg);
            OpenHarnessError::validation(error_msg)
        })?;

        if config_value.get("mcpServers").is_none() {
            let error_msg = "Config missing 'mcpServers' field";
            error!("{}", error_msg);
            return Err(OpenHarnessError::validation(error_msg.to_string()));
        }
