...
use crate::agentic::core::{
    has_prompt_markup, Message, MessageContent, MessageRole, ProcessingPhase, PromptEnvelope,
    Session, SessionConfig, SessionKind, SessionState, SessionSummary, TurnStats,
};
...
pub struct SubagentResult {
    /// AI text response
    pub text: String,
    /// Subagent session ID (can be queried for transcript details)
    pub session_id: Option<String>,
}

#[derive(Debug, Clone)]
pub struct SubagentExecutionOverrides {
    pub model_id: Option<String>,
    pub max_turns: Option<u32>,
    pub allowed_tools: Vec<String>,
    pub cleanup_session_on_finish: bool,
}

impl Default for SubagentExecutionOverrides {
    fn default() -> Self {
        Self {
            model_id: None,
            max_turns: None,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PartnerBootstrapSkipReason {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PartnerBootstrapBlockReason {
    ModelUnavailable,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PartnerBootstrapEnsureOutcome {
    Started {
        session_id: String,
        turn_id: String,
    },
    Skipped {
        session_id: String,
        reason: PartnerBootstrapSkipReason,
    },
    Blocked {
        session_id: String,
        reason: PartnerBootstrapBlockReason,
        detail: String,
    },
}

const PARTNER_BOOTSTRAP_AGENT_TYPE: &str = "Partner";
...
pub struct ConversationCoordinator {
    ...
    agent_task_supervisor: Arc<AgentTaskSupervisor>,
...
impl ConversationCoordinator {
    ...
    fn partner_bootstrap_kickoff_query(is_chinese: bool) -> &'static str {
        if is_chinese {
            "请开始初始化"
        } else {
    ...
    fn partner_bootstrap_system_reminder(
        kickoff_query: &str,
        expected_reply_language: &str,
    ) -> String {
        format!(
            "This is an automatic bootstrap kickoff generated by the system because this partner workspace still contains BOOTSTRAP.md. \
...
Update the persona files and delete BOOTSTRAP.md as soon as bootstrap is complete."
    ...
    pub fn new(
        ...
        let agent_task_supervisor = Arc::new(AgentTaskSupervisor::new(
            4,
            Some(crate::agentic::runtime::AgentTaskRegistry::default_snapshot_file()),
        ));
        let agent_task_registry = agent_task_supervisor.registry();
        tool_pipeline.set_agent_task_registry(Arc::clone(&agent_task_registry));
        tool_pipeline.set_agent_patch_store(agent_task_supervisor.patch_store());
        execution_engine.set_agent_task_registry(agent_task_registry);
    ...
    async fn wrap_user_input(
        ...
        let current_agent = agent_registry
            ...
            .ok_or_else(|| {
                OpenHarnessError::NotFound(format!("Agent not found: {}", agent_type))
            })?;
    ...
    pub async fn ensure_partner_bootstrap(
        ...
        session_id: String,
        workspace_path: String,
    ) -> OpenHarnessResult<PartnerBootstrapEnsureOutcome> {
        let workspace_root = PathBuf::from(&workspace_path);
        // Empty or partial partner dirs may never have run create_partner_workspace; fill only
        ...
        if !bootstrap_pending {
            return Ok(PartnerBootstrapEnsureOutcome::Skipped {
                session_id,
                reason: PartnerBootstrapSkipReason::BootstrapNotRequired,
            });
        }
        ...
        if turn_count > 0 {
            return Ok(PartnerBootstrapEnsureOutcome::Skipped {
                session_id,
                reason: PartnerBootstrapSkipReason::SessionHasExistingTurns,
            });
        }
        ...
        if !matches!(session.state, SessionState::Idle) {
            return Ok(PartnerBootstrapEnsureOutcome::Skipped {
                session_id,
                reason: PartnerBootstrapSkipReason::SessionNotIdle,
            });
        }

        let is_chinese = Self::is_chinese_locale().await;
        let kickoff_query = Self::partner_bootstrap_kickoff_query(is_chinese);
        ...
        let ai_client_factory =
            match crate::infrastructure::ai::get_global_ai_client_factory().await {
                Ok(factory) => factory,
                Err(error) => {
                    return Ok(PartnerBootstrapEnsureOutcome::Blocked {
                        session_id,
                        reason: PartnerBootstrapBlockReason::ModelUnavailable,
        ...
        let mut envelope = PromptEnvelope::new();
        envelope.push_system_reminder(Self::partner_bootstrap_system_reminder(
            kickoff_query,
            expected_reply_language,
        ));
        envelope.push_user_query(kickoff_query.to_string());

        let turn_id = format!("partner-bootstrap-{}", uuid::Uuid::new_v4());
        ...
        self.start_dialog_turn_internal(
            ...
            None,
            Some(turn_id.clone()),
            PARTNER_BOOTSTRAP_AGENT_TYPE.to_string(),
        ...
        Ok(PartnerBootstrapEnsureOutcome::Started {
            session_id,
            turn_id,
        })
    }
    ...
    pub async fn compact_session_manually(&self, session_id: String) -> OpenHarnessResult<()> {
        let session = self
            .session_manager
            .get_session(&session_id)
            .ok_or_else(|| {
                OpenHarnessError::NotFound(format!("Session not found: {}", session_id))
            })?;
    ...
    async fn start_dialog_turn_internal(
        ...
        // - Sessions with multiple turns should have > 1 messages (at least system + user + partner)
        let needs_restore = if context_messages.is_empty() {
            debug!(
        ...
        tokio::spawn(async move {
                ...
                .await
            {
                ...
                Err(e) => {
                    let is_cancellation = matches!(&e, OpenHarnessError::Cancelled(_));
    ...
    ) -> OpenHarnessResult<Option<String>> {
    ...
    pub async fn list_sessions(
        &self,
        workspace_path: &Path,
    ) -> OpenHarnessResult<Vec<SessionSummary>> {
        self.session_manager.list_sessions(workspace_path).await
    }

    /// Get a best-effort message view for a session.
    pub async fn get_messages(&self, session_id: &str) -> OpenHarnessResult<Vec<Message>> {
    ...
    ) -> OpenHarnessResult<(Vec<Message>, bool)> {
        self.session_manager
    ...
    fn extract_agent_task_id_from_context(
    ...
    async fn append_agent_task_observability_from_turn(
        &self,
        task_id: &AgentTaskId,
        session_id: &str,
        dialog_turn_id: &str,
    ) {
        let messages = match self.session_manager.get_messages(session_id).await {
            Ok(messages) => messages,
            Err(error) => {
                warn!(
                    "Failed to collect subagent messages for task transcript: task_id={}, session_id={}, error={}",
                    task_id,
                    session_id,
                    error
                );
        ...
        let transcript_store = self.agent_task_supervisor.transcript_store();

        for message in messages {
            if message.metadata.turn_id.as_deref() != Some(dialog_turn_id) {
                continue;
            }

            match message.content {
                MessageContent::Text(text) => {
                    if matches!(message.role, MessageRole::Assistant) && !text.trim().is_empty() {
                        transcript_store
                            .append_entry(task_id, AgentTranscriptEntry::ModelMessage { text })
                            .await;
                    }
                }
                MessageContent::Mixed {
                    ...
                    if !text.trim().is_empty() {
                        transcript_store
                            .append_entry(
                                task_id,
                                AgentTranscriptEntry::ModelMessage { text: text.clone() },
                            )
                            .await;
                    }
                ...
                MessageContent::ToolResult {
                    ...
                    transcript_store
                        .append_entry(
                            task_id,
                            AgentTranscriptEntry::ToolResult {
    ...
    pub async fn spawn_agent_task(
        ...
        let executor: AgentTaskExecutor = Arc::new(
            move |task_config: AgentTaskConfig,
                  task_id: AgentTaskId,
                  cancellation_token: CancellationToken| {
                Box::pin(async move {
                    let coordinator = get_global_coordinator().ok_or_else(|| {
                        OpenHarnessError::service("coordinator not initialized".to_string())
                    })?;

                    let parent_session_id = task_config
                        .session_id
                        .clone()
                        .unwrap_or_else(|| format!("agent-task-parent-{}", uuid::Uuid::new_v4()));
                    let parent_info = SubagentParentInfo {
                        tool_call_id: format!("agent-task-tool-{}", task_id),
                        session_id: parent_session_id,
                        dialog_turn_id: format!("agent-task-turn-{}", task_id),
                    };

                    let mut context = std::collections::HashMap::new();
                    context.insert("agent_task_id".to_string(), task_id.to_string());
                    if let Some(parent_task_id) = task_config.parent_task_id.as_ref() {
                        context.insert("parent_task_id".to_string(), parent_task_id.to_string());
                    }
                    if matches!(task_config.fork_context, ForkContextMode::InheritParent) {
                        context.insert("fork_context".to_string(), "inherit".to_string());
                    }

                    let workspace_path = task_config
                        .workspace_binding
                    ...
                    let result = coordinator
                        .execute_subagent(
                            task_config.agent_name.clone(),
                            task_config.prompt.clone(),
                            parent_info,
                            Some(workspace_path),
                            Some(context),
                            Some(&cancellation_token),
                            Some(SubagentExecutionOverrides {
                                model_id: task_config.model.clone(),
                                max_turns: task_config.max_turns,
                                allowed_tools: task_config.allowed_tools.clone(),
                                cleanup_session_on_finish: false,
                            }),
                        )
                        .await?;

                    Ok(AgentTaskExecutionOutput {
                        summary: result.text,
                        transcript_ref: result.session_id,
        ...
        self.agent_task_supervisor
            .spawn_task(config, kind, executor)
            .await
    }

    pub async fn wait_agent_task(&self, task_id: &str) -> OpenHarnessResult<AgentTaskSnapshot> {
        self.agent_task_supervisor
            .wait_task(&AgentTaskId::from(task_id))
            .await
    }
    ...
    pub async fn cancel_agent_task(
        ...
        self.agent_task_supervisor
            .cancel_task(&AgentTaskId::from(task_id), reason)
            .await
    }
    ...
    pub async fn list_agent_tasks(
        &self,
        filter: Option<&AgentTaskFilter>,
    ) -> Vec<AgentTaskSnapshot> {
        self.agent_task_supervisor.list_tasks(filter).await
    }

    pub async fn agent_task_events(
        &self,
        task_id: &str,
    ) -> OpenHarnessResult<Vec<crate::agentic::runtime::AgentTaskEvent>> {
        self.agent_task_supervisor
            .events(&AgentTaskId::from(task_id))
            .await
    }
    ...
    pub async fn agent_task_transcript(
        &self,
        task_id: &str,
    ) -> Option<crate::agentic::runtime::AgentTranscript> {
        self.agent_task_supervisor
            .transcript_store()
            .get(&AgentTaskId::from(task_id))
            .await
    }

    pub async fn agent_task_patches(&self, task_id: &str) -> Vec<AgentPatchRecord> {
        self.agent_task_supervisor
            .patch_store()
            .list_by_task(&AgentTaskId::from(task_id))
            .await
    }

    pub async fn set_agent_task_patch_status(
        &self,
        task_id: &str,
        patch_id: &str,
        status: PatchStatus,
    ...
    pub async fn agent_task_patch_summary(&self, task_id: &str) -> AgentPatchSummary {
        self.agent_task_supervisor
            .patch_store()
            .summary_by_task(&AgentTaskId::from(task_id))
            .await
    }

    pub async fn agent_team_status(&self, team_id: &str) -> OpenHarnessResult<AgentTeamStatus> {
        self.agent_task_supervisor.team_status(team_id).await
    }

    pub async fn agent_team_status_from_members(
        &self,
        team_name: Option<String>,
        objective: Option<String>,
        member_task_ids: Vec<String>,
    ) -> OpenHarnessResult<AgentTeamStatus> {
        let members = member_task_ids
            .into_iter()
        ...
        if members.is_empty() {
            return Err(OpenHarnessError::Validation(
                "member_task_ids is required".to_string(),
            ));
        }
        ...
        let team_name = team_name
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "ad-hoc-team".to_string());
        let objective = objective
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "generate ad-hoc team status".to_string());

        self.agent_task_supervisor
            .team_status_from_members(team_name, objective, members)
            .await
    }
    ...
    pub async fn send_agent_mailbox_message(
        ...
        let to_task_id = AgentTaskId::from(to_task_id.trim());
        let from_task_id = from_task_id
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty())
            .map(AgentTaskId::from);

        self.agent_task_supervisor
            .send_to_agent_mailbox(&to_task_id, content, from_task_id, team_id)
            .await
    }
    ...
    pub async fn broadcast_agent_team_message(
        ...
        self.agent_task_supervisor
            .broadcast_to_team_mailbox(team_id.trim(), content, from_task_id)
            .await
    }
    ...
    pub async fn wait_agent_mailbox_messages(
        ...
        self.agent_task_supervisor
            .wait_agent_mailbox_messages(&AgentTaskId::from(task_id.trim()), timeout_ms)
            .await
    }
    ...
    pub async fn execute_subagent(
        ...
        overrides: Option<SubagentExecutionOverrides>,
    ) -> OpenHarnessResult<SubagentResult> {
        let overrides = overrides.unwrap_or_default();
        let task_id = Self::extract_agent_task_id_from_context(context.as_ref());
        ...
        let mut subagent_config = SessionConfig {
            workspace_path: Some(workspace_path),
        ...
        if let Some(model_id) = overrides.model_id.as_ref() {
            if !model_id.trim().is_empty() {
                subagent_config.model_id = Some(model_id.trim().to_string());
            }
        }

        if let Some(max_turns) = overrides.max_turns {
            if max_turns > 0 {
                subagent_config.max_turns = max_turns as usize;
            }
        }
        ...
        let response_text = match result {
            Ok(exec_result) => {
                if let Some(task_id) = task_id.as_ref() {
                    self.append_agent_task_observability_from_turn(
                        task_id,
                        &session.session_id,
                        &dialog_turn_id,
                    )
                ...
                match exec_result.final_message.content {
            ...
            Err(e) => {
                ...
                if overrides.cleanup_session_on_finish {
                    if let Err(cleanup_err) =
                        self.cleanup_subagent_resources(&session.session_id).await
                    {
                        warn!(
    ...
    async fn cleanup_subagent_resources(&self, session_id: &str) -> OpenHarnessResult<()> {
        debug!(
