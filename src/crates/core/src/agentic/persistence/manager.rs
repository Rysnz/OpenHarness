...
impl PersistenceManager {
    pub fn new(path_manager: Arc<PathManager>) -> OpenHarnessResult<Self> {
    ...
    async fn ensure_project_sessions_dir(
        &self,
        workspace_path: &Path,
    ) -> OpenHarnessResult<PathBuf> {
    ...
    async fn ensure_session_dir(
        ...
        fs::create_dir_all(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to create session directory: {}", e))
        })?;
        Ok(dir)
    }
    ...
    async fn ensure_turns_dir(
        ...
        fs::create_dir_all(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to create turns directory: {}", e))
        })?;
        Ok(dir)
    }
    ...
    async fn ensure_snapshots_dir(
        ...
        fs::create_dir_all(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to create snapshots directory: {}", e))
        })?;
        Ok(dir)
    }
    ...
    async fn ensure_artifacts_dir(
        ...
        fs::create_dir_all(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to create artifacts directory: {}", e))
        })?;
        Ok(dir)
    }
    ...
    async fn write_json_atomic<T: Serialize>(
        ...
        fs::create_dir_all(parent).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to create parent directory: {}", e))
        })?;

        let json = serde_json::to_string_pretty(value).map_err(|e| {
            OpenHarnessError::serialization(format!("Failed to serialize JSON: {}", e))
        })?;
        ...
        for attempt in 0..=JSON_WRITE_MAX_RETRIES {
            ...
            if let Err(e) = fs::write(&tmp_path, &json_bytes).await {
                return Err(OpenHarnessError::io(format!(
        ...
        Err(OpenHarnessError::io(format!(
    ...
    fn build_temp_json_path(path: &Path, attempt: usize) -> OpenHarnessResult<PathBuf> {
        let parent = path.parent().ok_or_else(|| {
            OpenHarnessError::io(format!(
                "Target path has no parent directory: {}",
                path.display()
    ...
    fn transcript_fingerprint(
        ...
        let bytes = serde_json::to_vec(&payload).map_err(|e| {
            OpenHarnessError::serialization(format!(
                "Failed to serialize transcript fingerprint: {}",
                e
            ))
        })?;
    ...
    fn build_transcript_section(
        ...
        if !round_blocks.is_empty() {
            ...
            for (round_index, round) in round_blocks.iter().enumerate() {
                ...
                lines.push(format!("[/partner_round {}]", round.round_index));
    ...
    fn parse_transcript_turn_selectors(
        selectors: &[String],
    ) -> OpenHarnessResult<Vec<ParsedTranscriptTurnSelector>> {
    ...
    fn parse_transcript_turn_selector(
        selector: &str,
    ) -> OpenHarnessResult<ParsedTranscriptTurnSelector> {
    ...
    fn parse_transcript_turn_value(value: &str, selector: &str) -> OpenHarnessResult<isize> {
    ...
    async fn scan_session_metadata_dirs(
        &self,
        workspace_path: &Path,
    ) -> OpenHarnessResult<Vec<SessionMetadata>> {
        let sessions_root = self.ensure_project_sessions_dir(workspace_path).await?;
        let mut metadata_list = Vec::new();
        let mut entries = fs::read_dir(&sessions_root)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read sessions root: {}", e)))?;

        while let Some(entry) = entries.next_entry().await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to read session directory entry: {}", e))
        })? {
            let file_type = entry
                .file_type()
                .await
                .map_err(|e| OpenHarnessError::io(format!("Failed to get file type: {}", e)))?;
            if !file_type.is_dir() {
                continue;
            }
    ...
    pub async fn load_session_metadata(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> OpenHarnessResult<Option<SessionMetadata>> {
    ...
    async fn load_stored_session_state(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> OpenHarnessResult<Option<StoredSessionStateFile>> {
    ...
    pub async fn load_turn_context_snapshot(
        ...
        session_id: &str,
        turn_index: usize,
    ) -> OpenHarnessResult<Option<Vec<Message>>> {
        let snapshot = self
    ...
    pub async fn load_latest_turn_context_snapshot(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> OpenHarnessResult<Option<(usize, Vec<Message>)>> {
        let dir = self.snapshots_dir(workspace_path, session_id);
        if !dir.exists() {
            return Ok(None);
        }
        ...
        let mut rd = fs::read_dir(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to read snapshots directory: {}", e))
        })?;

        while let Some(entry) = rd.next_entry().await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to iterate snapshots directory: {}", e))
        })? {
            let path = entry.path();
    ...
    pub async fn load_session(
        ...
        let metadata = self
            ...
            .await?
            .ok_or_else(|| {
                OpenHarnessError::NotFound(format!("Session metadata not found: {}", session_id))
            })?;
    ...
    pub async fn delete_session(
        ...
        if dir.exists() {
            fs::remove_dir_all(&dir).await.map_err(|e| {
                OpenHarnessError::io(format!("Failed to delete session directory: {}", e))
            })?;
        }
    ...
    pub async fn load_dialog_turn(
        ...
        session_id: &str,
        turn_index: usize,
    ) -> OpenHarnessResult<Option<DialogTurnData>> {
        Ok(self
    ...
    pub async fn load_session_turns(
        ...
        workspace_path: &Path,
        session_id: &str,
    ) -> OpenHarnessResult<Vec<DialogTurnData>> {
        ...
        let mut entries = fs::read_dir(&turns_dir)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read turns directory: {}", e)))?;

        while let Some(entry) = entries.next_entry().await.map_err(|e| {
            OpenHarnessError::io(format!("Failed to iterate turns directory: {}", e))
        })? {
            let path = entry.path();
            if path.extension().and_then(|value| value.to_str()) != Some("json") {
                continue;
    ...
    pub async fn export_session_transcript(
        ...
        session_id: &str,
        options: &SessionTranscriptExportOptions,
    ) -> OpenHarnessResult<SessionTranscriptExport> {
        if self
            .load_session_metadata(workspace_path, session_id)
            .await?
            .is_none()
            ...
            .filter(|value| value.turn_index > turn_index)
        {
            ...
            if path.exists() {
                fs::remove_file(&path).await.map_err(|e| {
                    OpenHarnessError::io(format!("Failed to delete turn file: {}", e))
                })?;
                deleted += 1;
            }
        }
    ...
    impl TestWorkspace {
        fn new() -> Self {
            let path = std::env::temp_dir().join(format!(
                "openharness-session-transcript-test-{}",
                Uuid::new_v4()
            ));
