...
use crate::agentic::runtime::AgentTaskRegistry;
...
impl ExecutionEngine {
    ...
    pub fn set_agent_task_registry(&self, registry: Arc<AgentTaskRegistry>) {
        self.round_executor.set_agent_task_registry(registry);
    }
    ...
    fn emergency_truncate_messages(
        ...
        let tool_tokens = tools
            ...
            .unwrap_or(0);
        let preserved_tokens: usize =
            preserved.iter().map(|m| m.estimate_tokens()).sum::<usize>() + tool_tokens + 3;
    ...
    fn is_recoverable_historical_image_error(err: &OpenHarnessError) -> bool {
        match err {
            OpenHarnessError::Io(_) | OpenHarnessError::Deserialization(_) => true,
            OpenHarnessError::Validation(msg) => {
    ...
    fn can_fallback_to_text_only(
        ...
        let is_redacted_payload_error = matches!(
            err,
            OpenHarnessError::Validation(msg) if msg.starts_with("Image context missing image_path/data_url")
        ) && !images.is_empty()
    ...
    pub(crate) async fn resolve_model_id_for_turn(
        ...
        let fallback_model_id = agent_registry
            ...
            .await
            .map_err(|e| OpenHarnessError::AIClient(format!("Failed to get model ID: {}", e)))?;
        let config_service = get_global_config_service().await.map_err(|e| {
            OpenHarnessError::AIClient(format!(
    ...
    async fn build_ai_messages_for_send(
        ...
        current_turn_id: &str,
        attach_images: bool,
    ) -> OpenHarnessResult<Vec<AIMessage>> {
            ...
            match &msg.content {
                MessageContent::Multimodal { text, images } => {
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            if matches!(&err, OpenHarnessError::Validation(msg) if msg.starts_with("Too many images in one request"))
                            {
                                return Err(err);
                            }
    ...
    pub async fn compress_messages(
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(OpenHarnessError::Session(e.to_string()))
            }
        }
    }
    ...
    pub async fn compact_session_context(
        ...
        trigger: &str,
        tail_policy: CompressionTailPolicy,
    ) -> OpenHarnessResult<ContextCompactionOutcome> {
        let mut session = self
            .session_manager
            .get_session(session_id)
            .ok_or_else(|| {
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                Err(OpenHarnessError::Session(err.to_string()))
            }
        }
    }
    ...
    ) -> OpenHarnessResult<ExecutionResult> {
        let start_time = std::time::Instant::now();
    ...
    async fn execute_dialog_turn_impl(
        ...
        let ai_client_factory = get_global_ai_client_factory().await.map_err(|e| {
            OpenHarnessError::AIClient(format!("Failed to get AI client factory: {}", e))
        })?;
        ...
        let mut allowed_tools = agent_registry
            ...
            .await;
        if let Some(override_raw) = context.context.get("allowed_tools_override") {
            let requested_tools = override_raw
                .split(',')
                .map(|item| item.trim())
            ...
            if !requested_tools.is_empty() {
                allowed_tools.retain(|tool| requested_tools.contains(tool.as_str()));
                debug!(
                    "Applied allowed_tools_override for agent '{}': {} tool(s)",
                    agent_type,
                    allowed_tools.len()
                );
            }
        ...
        loop {
            ...
            if dialog_turn_cancelled {
                ...
                return Err(OpenHarnessError::cancelled("Dialog cancelled"));
            }
    ...
    pub async fn cancel_dialog_turn(&self, dialog_turn_id: &str) -> OpenHarnessResult<()> {
