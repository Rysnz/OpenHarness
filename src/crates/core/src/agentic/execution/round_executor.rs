...
impl RoundExecutor {
    ...
    pub fn set_agent_task_registry(&self, registry: Arc<AgentTaskRegistry>) {
        self.stream_processor
            .set_agent_task_registry(Arc::clone(&registry));
    ...
    async fn emit_agent_task_event(
        &self,
    ...
    pub async fn execute_round(
        ...
        let stream_result = loop {
            ...
            self.emit_agent_task_event(
                ...
                AgentTaskEventKind::ModelRequestStarted,
                Some("Model request started".to_string()),
                Some(serde_json::json!({
                    "session_id": context.session_id.clone(),
                    "dialog_turn_id": context.dialog_turn_id.clone(),
                    "round_id": round_id.clone(),
                    "round_index": context.round_number,
                    "model": context.model_name.clone(),
                    "message_count": ai_messages.len(),
                    "tool_count": tool_definitions.as_ref().map(|tools| tools.len()).unwrap_or(0),
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    return Err(OpenHarnessError::AIClient(err_msg));
                }
            };
