...
impl ImageAnalyzer {
    ...
    ) -> OpenHarnessResult<Vec<ImageAnalysisResult>> {
    ...
    async fn analyze_single_image(
        ...
        workspace_path: Option<PathBuf>,
        ai_client: Arc<AIClient>,
    ) -> OpenHarnessResult<ImageAnalysisResult> {
        ...
        let ai_response = ai_client.send_message(messages, None).await.map_err(|e| {
            ...
            OpenHarnessError::service(format!("Image analysis AI call failed: {}", e))
        })?;
    ...
    ) -> OpenHarnessResult<(Vec<u8>, Option<String>)> {
