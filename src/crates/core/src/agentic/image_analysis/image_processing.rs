...
pub fn resolve_vision_model_from_ai_config(
...
) -> OpenHarnessResult<AIModelConfig> {
    ...
    let model = ai_config
        ...
        .find(|m| m.id == id)
        .cloned()
        .ok_or_else(|| OpenHarnessError::service(format!("Model not found: {}", id)))?;
...
pub async fn resolve_vision_model_from_global_config() -> OpenHarnessResult<AIModelConfig> {
    let config_service = get_global_config_service().await?;
    let ai_config: ServiceAIConfig = config_service
        .get_config(Some("ai"))
        .await
        .map_err(|e| OpenHarnessError::service(format!("Failed to get AI config: {}", e)))?;

    resolve_vision_model_from_ai_config(&ai_config)
}

pub fn resolve_image_path(path: &str, workspace_path: Option<&Path>) -> OpenHarnessResult<PathBuf> {
...
pub async fn load_image_from_path(
    ...
    fs::read(path)
        .await
        .map_err(|e| OpenHarnessError::io(format!("Failed to read image: {}", e)))
}

pub fn decode_data_url(data_url: &str) -> OpenHarnessResult<(Vec<u8>, Option<String>)> {
    if !data_url.starts_with("data:") {
        return Err(OpenHarnessError::validation("Invalid data URL format"));
    }

    let parts: Vec<&str> = data_url.splitn(2, ',').collect();
    if parts.len() != 2 {
        return Err(OpenHarnessError::validation("Data URL format error"));
    }
    ...
    let image_data = BASE64
        .decode(base64_data)
        .map_err(|e| OpenHarnessError::parse(format!("Base64 decode failed: {}", e)))?;
...
pub fn optimize_image_for_provider(
    ...
    provider: &str,
    fallback_mime: Option<&str>,
) -> OpenHarnessResult<ProcessedImage> {
...
pub fn optimize_image_with_size_limit(
    ...
    let dynamic = image::load_from_memory(&image_data)
        .map_err(|e| OpenHarnessError::validation(format!("Failed to decode image data: {}", e)))?;
...
) -> OpenHarnessResult<Vec<ProcessedImage>> {
    let limits = ImageLimits::for_provider(provider);
...
fn encode_dynamic_image(
    ...
    format: ImageFormat,
    jpeg_quality: u8,
) -> OpenHarnessResult<(Vec<u8>, String)> {
    ...
    match target_format {
        ImageFormat::Png => {
            ...
            encoder
                ...
                .map_err(|e| OpenHarnessError::tool(format!("PNG encode failed: {}", e)))?;
        }
        ImageFormat::Jpeg => {
            let mut encoder = JpegEncoder::new_with_quality(&mut buffer, jpeg_quality);
            encoder
                .encode_image(image)
                .map_err(|e| OpenHarnessError::tool(format!("JPEG encode failed: {}", e)))?;
        }
