...
pub use definition::{
    AgentDefinition, AgentHookConfig, AgentMemoryConfig, AgentMode, PermissionMode,
};
...
pub use partner_mode::PartnerMode;
...
pub trait Agent: Send + Sync + 'static {
    ...
    async fn build_prompt(&self, context: &PromptBuilderContext) -> OpenHarnessResult<String> {
        ...
        let system_prompt_template = get_embedded_prompt(template_name).ok_or_else(|| {
            OpenHarnessError::Agent(format!("{} not found in embedded files", template_name))
        })?;
    ...
    async fn get_system_reminder(&self, _index: usize) -> OpenHarnessResult<String> {
