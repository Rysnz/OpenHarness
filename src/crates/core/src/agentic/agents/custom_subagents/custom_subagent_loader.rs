...
/// Project subagent directory names (relative to workspace root, each item is in [".openharness", "agents"] format)
...
impl CustomSubagentLoader {
    ...
    /// - Project subagents: .openharness/agents, .claude/agents, .cursor/agents, .codex/agents under workspace
    /// - User subagents: agents under openharness user config, ~/.claude/agents, ~/.cursor/agents, ~/.codex/agents
    pub fn get_possible_paths(workspace_root: &Path) -> Vec<SubagentDirEntry> {
        ...
        // User subagents: agents under openharness user config
        let pm = get_path_manager_arc();
        let openharness_agents = pm.user_agents_dir();
        if openharness_agents.exists() && openharness_agents.is_dir() {
            entries.push(SubagentDirEntry {
                path: openharness_agents,
        ...
        if let Some(home) = dirs::home_dir() {
            for (parent, sub) in PROJECT_AGENT_SUBDIRS {
                if *parent == ".openharness" {
                    continue; // openharness user path already handled by path_manager
                }
