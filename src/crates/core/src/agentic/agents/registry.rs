use super::{
    Agent, AgentDefinition, AgentHookConfig, AgentMemoryConfig, AgentMode, AgenticMode,
    CodeReviewAgent, CoworkMode, DebugMode, ExploreAgent, FileFinderAgent, GenerateDocAgent,
    InitAgent, PartnerMode, PermissionMode, PlanMode,
};
...
fn default_model_id_for_builtin_agent(agent_type: &str) -> &'static str {
    match agent_type {
        "agentic" | "Cowork" | "Plan" | "debug" | "Partner" => "auto",
        _ => "primary",
    }
}
...
impl AgentRegistry {
    ...
    fn validate_custom_subagent(
        ...
        let filter_tool_list = |items: Vec<String>| {
        ...
        let (valid_tools_only, invalid_tools) = filter_tool_list(subagent.tools.clone());
        if !invalid_tools.is_empty() {
            warn!(
                "[Subagent {}] Invalid tools filtered out: {:?}",
                agent_id, invalid_tools
            );
        }
        subagent.tools = valid_tools_only;

        let (valid_allowed, invalid_allowed) = filter_tool_list(subagent.allowed_tools.clone());
        if !invalid_allowed.is_empty() {
            warn!(
                "[Subagent {}] Invalid allowed_tools filtered out: {:?}",
                agent_id, invalid_allowed
            );
        }
        subagent.allowed_tools = valid_allowed;

        let (valid_disallowed, invalid_disallowed) =
            filter_tool_list(subagent.disallowed_tools.clone());
        if !invalid_disallowed.is_empty() {
            warn!(
                "[Subagent {}] Invalid disallowed_tools filtered out: {:?}",
                agent_id, invalid_disallowed
            );
        }
        subagent.disallowed_tools = valid_disallowed;

        if subagent.allowed_tools.is_empty() {
            subagent.allowed_tools = subagent.tools.clone();
        }

        let deny_set: std::collections::HashSet<&str> = subagent
            .disallowed_tools
            .iter()
            .map(|s| s.as_str())
            .collect();
        subagent
            .allowed_tools
            .retain(|tool| !deny_set.contains(tool.as_str()));
        subagent
            .tools
            .retain(|tool| !deny_set.contains(tool.as_str()));

        if subagent.tools.is_empty() && !subagent.allowed_tools.is_empty() {
            subagent.tools = subagent.allowed_tools.clone();
        }
        if subagent.allowed_tools.is_empty() {
            subagent.allowed_tools = subagent.tools.clone();
        }
        ...
        if let Some(max_turns) = subagent.max_turns {
            if max_turns == 0 {
                warn!(
                    "[Subagent {}] max_turns=0 is invalid, reset to None",
                    agent_id
                );
                subagent.max_turns = None;
            }
        }

        if let Some(temperature) = subagent.temperature {
            if !(0.0..=2.0).contains(&temperature) {
                warn!(
                    "[Subagent {}] temperature={} out of range [0, 2], clearing value",
        ...
        let mcp_servers = std::mem::take(&mut subagent.mcp_servers);
        subagent.mcp_servers = mcp_servers
            .into_iter()
            .filter(|item| !item.trim().is_empty())
            .collect();
        let skills = std::mem::take(&mut subagent.skills);
    ...
    pub fn update_and_save_custom_subagent_config(
        ...
        let entry = if let Some(entry) = map.get_mut(agent_id) {
            ...
            project_maps
                ...
                .ok_or_else(|| {
                    OpenHarnessError::agent(format!("Subagent not found: {}", agent_id))
                })?
        };

        if entry.category != AgentCategory::SubAgent {
            return Err(OpenHarnessError::agent(format!(
        ...
        let config = entry.custom_config.as_mut().ok_or_else(|| {
            OpenHarnessError::agent(format!("Subagent '{}' is not a custom subagent", agent_id))
        })?;
    ...
    pub async fn get_custom_subagent_detail(
        ...
        agent_id: &str,
        workspace_root: Option<&Path>,
    ) -> OpenHarnessResult<CustomSubagentDetail> {
    ...
    fn get_custom_subagent_detail_inner(
        ...
        let entry = self
            ...
            .ok_or_else(|| OpenHarnessError::agent(format!("Subagent not found: {}", agent_id)))?;
        if entry.category != AgentCategory::SubAgent {
            return Err(OpenHarnessError::agent(format!(
                "Agent '{}' is not a subagent",
                agent_id
        ...
        if entry.subagent_source == Some(SubAgentSource::Builtin) {
            return Err(OpenHarnessError::agent(
    ...
    pub async fn update_custom_subagent_definition(
        ...
        new_subagent.mode = old.mode.clone();
        new_subagent.temperature = old.temperature;
        new_subagent.max_turns = old.max_turns;
        new_subagent.permission_mode = old.permission_mode.clone();
        new_subagent.allowed_tools = new_subagent.tools.clone();
        new_subagent.disallowed_tools = old.disallowed_tools.clone();
        new_subagent.mcp_servers = old.mcp_servers.clone();
        new_subagent.skills = old.skills.clone();
        new_subagent.memory = old.memory.clone();
        new_subagent.hooks = old.hooks.clone();
        new_subagent.isolation = old.isolation;
        new_subagent.cwd = old.cwd.clone();
    ...
    pub fn remove_subagent(&self, agent_id: &str) -> OpenHarnessResult<Option<String>> {
        let mut map = self.write_agents();
        ...
        Err(OpenHarnessError::agent(format!(
            "Subagent not found: {}",
            agent_id
        )))
    }
    ...
    pub fn get_agent_definition(
        ...
        if let Some(custom) = entry.agent.as_any().downcast_ref::<CustomSubagent>() {
            return Some(custom.to_definition());
        }

        Some(AgentDefinition {
            name: entry.agent.id().to_string(),
            description: entry.agent.description().to_string(),
            prompt: String::new(),
            mode: match entry.category {
                AgentCategory::Mode => AgentMode::from_str(agent_type),
                AgentCategory::SubAgent | AgentCategory::Hidden => AgentMode::Standard,
            },
            model: None,
            temperature: None,
            max_turns: None,
...
mod tests {
    ...
    fn top_level_modes_default_to_auto() {
        for agent_type in ["agentic", "Cowork", "Plan", "debug", "Partner"] {
