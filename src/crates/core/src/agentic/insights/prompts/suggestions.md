Analyze this OpenHarness usage data and suggest improvements.

## OPENHARNESS FEATURES REFERENCE (pick from these for features_to_try):

1. **Skills**: Create reusable prompt templates as markdown files that run with a single button or command.
   - How to use: Create `.openharness/skills/commit/SKILL.md` with instructions. Then trigger it from the Skills panel.
...
2. **SubAgents (Task Agents)**: Custom agents you define for specific domains or tasks. SubAgents run in parallel and return results to the parent agent.
   - How to use: Create agents in `.openharness/agents/` with custom prompts and tool configurations.
   - Good for: domain-specific tasks, parallel exploration, focused code review
   - Example agent config (`.openharness/agents/security-reviewer/agent.json`):
     ```json
     {
...
3. **MCP Servers**: Connect OpenHarness to external tools, databases, and APIs via Model Context Protocol.
   ...
   - Common integrations:
     ...
     - **GitHub**: Create issues, review PRs, manage releases without leaving OpenHarness
...
5. **CLI Exec (Headless)**: Run OpenHarness non-interactively from scripts and CI/CD pipelines.
   - How to use: `openharness exec "fix lint errors" --tools "Edit,Read,Bash"`
   - Good for: CI/CD integration, batch code fixes, automated reviews
   - CI/CD examples:
     ```bash
     # Pre-commit hook: auto-fix lint errors
     openharness exec "fix all lint errors in staged files" --tools "Edit,Read,Bash"

     # PR review bot
     openharness exec "review changes in this PR for security issues" --tools "Read,Grep,Glob"

     # Automated documentation
     openharness exec "update API docs for all changed endpoints" --tools "Read,Edit,Glob"
     ```

RESPOND WITH ONLY A VALID JSON OBJECT:
{
  "openharness_md_additions": [
    {"section": "Section name in OPENHARNESS.md", "content": "A specific line or block to add based on workflow patterns", "rationale": "1 sentence explaining why this would help based on actual sessions"}
  ],
  "features_to_try": [
    {"feature": "Feature name from OPENHARNESS FEATURES REFERENCE above", "description": "What it does", "example_usage": "Actual command or config to copy", "benefit": "Why this would help YOU based on your sessions"}
  ],
  "usage_patterns": [
...
IMPORTANT for openharness_md_additions: PRIORITIZE instructions that appear MULTIPLE TIMES in the user data. If user told AI the same thing in 2+ sessions (e.g., 'always run tests', 'use TypeScript'), that's a PRIME candidate - they shouldn't have to repeat themselves.

IMPORTANT for features_to_try: Pick 2-3 from the OPENHARNESS FEATURES REFERENCE above. Include concrete, copy-pasteable example_usage for each. Tailor the benefit to the user's actual workflow patterns.
