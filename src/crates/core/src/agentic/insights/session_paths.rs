...
/// Map a workspace record to the directory where `.openharness/sessions` lives
/// (local project root or `~/.openharness/remote_ssh/...` mirror).
...
/// Unique effective roots that have a `.openharness/sessions` directory.
pub async fn collect_effective_session_storage_roots() -> Vec<PathBuf> {
    ...
    for ws in ws_service.list_workspace_infos().await {
        ...
        let sessions_dir = root.join(".openharness").join("sessions");
