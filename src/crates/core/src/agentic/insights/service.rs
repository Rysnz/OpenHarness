...
impl InsightsService {
    ...
    pub async fn generate(days: u32) -> OpenHarnessResult<InsightsReport> {
    ...
    async fn generate_inner(
    ...
    ) -> OpenHarnessResult<InsightsReport> {
        ...
        let ai_factory = get_global_ai_client_factory().await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to get AI client factory: {}", e))
        })?;
        let ai_client_fast = ai_factory.get_client_resolved("fast").await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to resolve fast model: {}", e))
        })?;
    ...
    fn check_cancelled(token: &CancellationToken) -> OpenHarnessResult<()> {
        if token.is_cancelled() {
            Err(OpenHarnessError::service("Insights generation cancelled"))
        } else {
            Ok(())
        }
    }
    ...
    async fn extract_facets_adaptive(
        ...
        token: &CancellationToken,
    ) -> OpenHarnessResult<Vec<SessionFacet>> {
            ...
            .map(|(idx, t)| {
                ...
                tokio::spawn(async move {
                    let _permit = sem.acquire().await.map_err(|e| {
                        OpenHarnessError::service(format!("Semaphore error: {}", e))
                    })?;

                    if cl.load(Ordering::Relaxed) || child_token.is_cancelled() {
                        return Err(OpenHarnessError::service("Insights generation cancelled"));
                    }

                    if rl.load(Ordering::Relaxed) {
                        return Err(OpenHarnessError::service("skipped_rate_limited"));
                    }
    ...
    async fn extract_single_facet(
        ...
        lang_instruction: &str,
    ) -> OpenHarnessResult<SessionFacet> {
        ...
        let response = ai_client
            ...
            .await
            .map_err(|e| OpenHarnessError::service(format!("AI call failed: {}", e)))?;

        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse facet JSON: {}", e))
        })?;
    ...
    async fn resolve_with_retry<T, RetryFut, RetryFn, DefaultFn>(
        handle: tokio::task::JoinHandle<OpenHarnessResult<T>>,
        label: &str,
        retry_fn: RetryFn,
        default_fn: DefaultFn,
    ) -> T
    where
        RetryFut: std::future::Future<Output = OpenHarnessResult<T>>,
        RetryFn: FnOnce() -> RetryFut,
        DefaultFn: FnOnce() -> T,
    {
        let result = handle
            .await
            .map_err(|e| OpenHarnessError::service(format!("{} task panicked: {}", label, e)));
    ...
    async fn generate_suggestions(
        ...
        aggregate: &InsightsAggregate,
        lang_instruction: &str,
    ) -> OpenHarnessResult<InsightsSuggestions> {
        let aggregate_json = aggregate_stats_json_for_prompt(aggregate);
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Suggestions AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!(
        ...
        debug!(
            ...
            value["openharness_md_additions"]
                .as_array()
                .map(|a| a.len())
                .unwrap_or(0),
        ...
        Ok(InsightsSuggestions {
            openharness_md_additions: value["openharness_md_additions"]
                .as_array()
                .map(|arr| {
                    arr.iter()
                        .filter_map(|v| {
    ...
    async fn identify_areas(
        ...
        aggregate: &InsightsAggregate,
        lang_instruction: &str,
    ) -> OpenHarnessResult<Vec<ProjectArea>> {
        let aggregate_json = aggregate_stats_json_for_prompt(aggregate);
        let summaries = summaries_block(aggregate);
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Areas AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse areas JSON: {}", e))
        })?;
    ...
    async fn analyze_wins(
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Wins AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse wins JSON: {}", e))
        })?;
    ...
    async fn analyze_friction(
        ...
        lang_instruction: &str,
    ) -> OpenHarnessResult<FrictionResult> {
        let prompt = format!(
            "{}{}",
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Friction AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse friction JSON: {}", e))
        })?;
    ...
    async fn analyze_interaction_style(
        ...
        summaries: &str,
        lang_instruction: &str,
    ) -> OpenHarnessResult<InteractionStyleResult> {
        let prompt = format!(
            "{}{}",
        ...
        let response = ai_client.send_message(messages, None).await.map_err(|e| {
            OpenHarnessError::service(format!("Interaction Style AI call failed: {}", e))
        })?;
    ...
    async fn generate_at_a_glance(
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("At a Glance AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse at-a-glance JSON: {}", e))
        })?;
    ...
    async fn generate_horizon(
        ...
        friction_details: &str,
        lang_instruction: &str,
    ) -> OpenHarnessResult<HorizonResult> {
        let prompt = format!(
            "{}{}",
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Horizon AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse horizon JSON: {}", e))
        })?;
    ...
    async fn generate_fun_ending(
        ...
        summaries: &str,
        lang_instruction: &str,
    ) -> OpenHarnessResult<Option<FunEnding>> {
        let prompt = format!(
            "{}{}",
        ...
        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Fun Ending AI call failed: {}", e)))?;
        ...
        let json_str = extract_json_from_response(&response.text)?;
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse fun ending JSON: {}", e))
        })?;
    ...
    async fn save_report(
        ...
        tokio::fs::create_dir_all(&usage_dir)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to create usage-data dir: {}", e)))?;
        ...
        tokio::fs::write(&html_path, &html_content)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write HTML report: {}", e)))?;
        ...
        let json_str = serde_json::to_string_pretty(&report).map_err(|e| {
            OpenHarnessError::serialization(format!("Failed to serialize report: {}", e))
        })?;
        tokio::fs::write(&json_path, &json_str)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write report JSON: {}", e)))?;
    ...
    pub async fn has_data(days: u32) -> OpenHarnessResult<bool> {
        let path_manager = get_path_manager_arc();
    ...
    pub async fn load_report(path: &str) -> OpenHarnessResult<InsightsReport> {
        let json_str = tokio::fs::read_to_string(path)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read report file: {}", e)))?;
        let report: InsightsReport = serde_json::from_str(&json_str).map_err(|e| {
            OpenHarnessError::Deserialization(format!("Failed to parse report: {}", e))
        })?;
        Ok(report)
    }

    pub async fn load_latest_reports() -> OpenHarnessResult<Vec<InsightsReportMeta>> {
        let path_manager = get_path_manager_arc();
        let usage_dir = path_manager.user_data_dir().join("usage-data");
        ...
        let mut entries = tokio::fs::read_dir(&usage_dir)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read usage-data dir: {}", e)))?;
...
fn is_rate_limit_error(e: &OpenHarnessError) -> bool {
...
fn is_retryable_error(e: &OpenHarnessError) -> bool {
    if is_rate_limit_error(e) {
        return true;
    }
    let msg = e.to_string().to_lowercase();
...
fn default_suggestions() -> InsightsSuggestions {
    InsightsSuggestions {
        openharness_md_additions: Vec::new(),
...
fn extract_json_from_response(response: &str) -> OpenHarnessResult<String> {
    crate::util::extract_json_from_ai_response(response)
        .ok_or_else(|| OpenHarnessError::service("Cannot extract JSON from AI response"))
}
