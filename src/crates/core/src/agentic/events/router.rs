...
pub trait EventSubscriber: Send + Sync + 'static {
    async fn on_event(&self, event: &AgenticEvent) -> OpenHarnessResult<()>;
}
...
impl EventRouter {
    ...
    pub async fn route(&self, envelope: EventEnvelope) -> OpenHarnessResult<()> {
    ...
    pub async fn route_batch(&self, envelopes: Vec<EventEnvelope>) -> OpenHarnessResult<()> {
        // First collect subscribers list (avoid holding DashMap iterator across await points)
        let subscribers: Vec<(String, Arc<dyn EventSubscriber>)> = self
            .internal_subscribers
            .iter()
