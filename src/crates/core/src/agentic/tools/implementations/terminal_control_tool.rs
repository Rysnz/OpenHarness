...
The terminal_session_id is returned inside <terminal_session_id>...</terminal_session_id> tags in BashTool results."#
    ...
    async fn call_impl(
        ...
        let terminal_session_id = input
            .get("terminal_session_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("terminal_session_id is required".to_string()))?;
                ...
                terminal_api
                    ...
                    .map_err(|e| {
                        OpenHarnessError::tool(format!(
                            "Failed to interrupt terminal session: {}",
                            e
                        ))
                    })?;
            ...
            "kill" => {
                ...
                if is_primary {
                    binding.remove(terminal_session_id).await.map_err(|e| {
                        OpenHarnessError::tool(format!("Failed to close terminal session: {}", e))
                    })?;
                } else {
                    terminal_api
