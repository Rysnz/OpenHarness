...
const SKELETON_CSS: &str = r#"/* MiniApp skeleton — uses host theme via --openharness-* variables */
* { box-sizing: border-box; margin: 0; padding: 0; }
body {
  font-family: var(--openharness-font-sans, -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif);
  font-size: 13px;
  color: var(--openharness-text, #e8e8e8);
  background: var(--openharness-bg, #121214);
  min-height: 100vh;
}
...
Returns app_id and the app root directory. Use the root directory and file names above with Read/Write/Edit to implement the app. The MiniApp uses window.app (app.fs, app.call, app.dialog, etc.) — see miniapp-dev skill for API reference."#
    ...
    async fn call_impl(
        ...
        let manager = try_get_global_miniapp_manager()
            .ok_or_else(|| OpenHarnessError::tool("MiniAppManager not initialized".to_string()))?;

        let name = input
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::validation("Missing required field: name"))?
            .to_string();
        ...
        let app = manager
            ...
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Failed to create MiniApp: {}", e)))?;
