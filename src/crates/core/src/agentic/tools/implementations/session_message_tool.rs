...
impl SessionMessageTool {
    ...
    fn sender_session_id<'a>(&self, context: &'a ToolUseContext) -> OpenHarnessResult<&'a str> {
        context.session_id.as_deref().ok_or_else(|| {
            OpenHarnessError::tool("SessionMessage requires a source session".to_string())
        })
    }

    fn sender_workspace(&self, context: &ToolUseContext) -> OpenHarnessResult<String> {
        context
            .workspace_root()
            .map(|path| path.to_string_lossy().to_string())
            .ok_or_else(|| {
                OpenHarnessError::tool("SessionMessage requires a source workspace".to_string())
            })
    }
...
When overriding an existing session's agent_type, only switching between "agentic" and "Plan" is allowed. It will not switch coding sessions to or from "Cowork"."#
    ...
    async fn call_impl(
        ...
        let params: SessionMessageInput = serde_json::from_value(input.clone())
            .map_err(|e| OpenHarnessError::tool(format!("Invalid input: {}", e)))?;
        ...
        let scheduler = get_global_scheduler()
            .ok_or_else(|| OpenHarnessError::tool("scheduler not initialized".to_string()))?;
