...
pub fn ensure_pointer_move_uses_screen_coordinates_only(input: &Value) -> OpenHarnessResult<()> {
...
pub fn parse_screenshot_crop_center(
    input: &Value,
) -> OpenHarnessResult<Option<ScreenshotCropCenter>> {
    ...
    match (x_none, y_none) {
        ...
        (false, false) => {
            let x = xv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| OpenHarnessError::tool("screenshot_crop_center_x must be a non-negative integer (full-display native pixels).".to_string()))?;
            let y = yv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| OpenHarnessError::tool("screenshot_crop_center_y must be a non-negative integer (full-display native pixels).".to_string()))?;
            Ok(Some(ScreenshotCropCenter {
                x: u32::try_from(x)
                    .map_err(|_| OpenHarnessError::tool("screenshot_crop_center_x is too large.".to_string()))?,
                y: u32::try_from(y)
                    .map_err(|_| OpenHarnessError::tool("screenshot_crop_center_y is too large.".to_string()))?,
            }))
        }
...
pub fn parse_screenshot_crop_half_extent_native(input: &Value) -> OpenHarnessResult<Option<u32>> {
...
pub fn parse_screenshot_implicit_center(
    input: &Value,
) -> OpenHarnessResult<Option<ComputerUseImplicitScreenshotCenter>> {
    match input
        ...
        .map(str::trim)
    {
        ...
        Some(other) => Err(OpenHarnessError::tool(format!(
...
pub fn parse_screenshot_navigate_quadrant(
    input: &Value,
) -> OpenHarnessResult<Option<ComputerUseNavigateQuadrant>> {
    let value = input
...
pub fn parse_screenshot_params(
    input: &Value,
) -> OpenHarnessResult<(ComputerUseScreenshotParams, bool)> {
