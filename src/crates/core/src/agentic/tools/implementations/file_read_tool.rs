...
impl FileReadTool {
    ...
    async fn read_remote_window(
    ...
    ) -> OpenHarnessResult<tool_runtime::fs::read_file::ReadFileResult> {
        const TOTAL_LINES_MARKER: &str = "__OPENHARNESS_TOTAL_LINES__=";
        const HIT_TOTAL_CHAR_LIMIT_MARKER: &str = "__OPENHARNESS_HIT_TOTAL_CHAR_LIMIT__=";

        let end_line = start_line
            .checked_add(limit.saturating_sub(1))
            .ok_or_else(|| {
                OpenHarnessError::tool("Requested line range is too large".to_string())
            })?;

        let ws_shell = context.ws_shell().ok_or_else(|| {
            OpenHarnessError::tool("Remote workspace shell is unavailable".to_string())
        })?;
        ...
        let (stdout, stderr, status) = ws_shell
            .exec(&command, None)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Failed to read file: {}", e)))?;
        ...
        if status != 0 {
            ...
            return Err(OpenHarnessError::tool(message));
        }

        let total_lines = total_lines.ok_or_else(|| {
            OpenHarnessError::tool(
                "Failed to read file: remote line count was unavailable".to_string(),
            )
        })?;
...
"#,
    ...
    async fn call_impl(
        ...
        let file_path = input
            .get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("file_path is required".to_string()))?;
