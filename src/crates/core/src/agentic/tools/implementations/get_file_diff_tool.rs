...
impl GetFileDiffTool {
    ...
    ) -> Option<OpenHarnessResult<Value>> {
    ...
    async fn try_git_diff(&self, file_path: &Path) -> Option<OpenHarnessResult<Value>> {
    ...
    fn return_full_content(&self, file_path: &Path) -> OpenHarnessResult<Value> {
