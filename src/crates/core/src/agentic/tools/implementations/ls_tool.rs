    ...
    async fn call_impl(
        ...
        if context.is_remote() {
            let ws_shell = context.ws_shell().ok_or_else(|| {
                OpenHarnessError::tool("Workspace shell not available for remote LS".to_string())
            })?;
            ...
            let (stdout, _stderr, _exit_code) =
                ws_shell.exec(&ls_cmd, Some(15_000)).await.map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to list remote directory: {}", e))
                })?;
        ...
        let entries = list_directory_entries(path, limit).map_err(OpenHarnessError::tool)?;
