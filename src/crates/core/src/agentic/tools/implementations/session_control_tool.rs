...
impl SessionControlTool {
    ...
    fn resolve_workspace(&self, workspace: &str) -> OpenHarnessResult<String> {
    ...
    fn creator_session_marker(&self, context: &ToolUseContext) -> OpenHarnessResult<String> {
        let creator_session_id = context.session_id.as_ref().ok_or_else(|| {
            OpenHarnessError::tool("create requires a creator session in tool context".to_string())
        })?;
        ...
        match params.action {
            ...
            SessionControlAction::Delete => {
                let session_id = params.session_id.as_deref().ok_or_else(|| {
                    OpenHarnessError::tool("session_id is required for delete".to_string())
                })?;
                Self::validate_session_id(session_id).map_err(OpenHarnessError::tool)?;
