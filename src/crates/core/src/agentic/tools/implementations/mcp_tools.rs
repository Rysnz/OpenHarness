...
fn tool_error(message: impl Into<String>) -> OpenHarnessError {
    OpenHarnessError::tool(message.into())
}
...
async fn get_mcp_server_manager() -> OpenHarnessResult<Arc<MCPServerManager>> {
...
async fn list_resources_for_server(
    ...
    server_id: &str,
    refresh: bool,
) -> OpenHarnessResult<Vec<MCPResource>> {
...
async fn list_prompts_for_server(
    ...
    server_id: &str,
    refresh: bool,
) -> OpenHarnessResult<Vec<MCPPrompt>> {
