...
- A good plan is glanceable, not a wall of text."###
    ...
    async fn call_impl(
        ...
        let name = input
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or(OpenHarnessError::validation("Missing required field: name"))?;

        let overview =
            input
                .get("overview")
                .and_then(|v| v.as_str())
                .ok_or(OpenHarnessError::validation(
        ...
        let plan = input
            .get("plan")
            .and_then(|v| v.as_str())
            .ok_or(OpenHarnessError::validation("Missing required field: plan"))?;
        ...
        let plan_file_path_str = if context.is_remote() {
            ...
            let ws_shell = context.ws_shell().ok_or_else(|| {
                OpenHarnessError::tool(
                    "Workspace shell not available for remote CreatePlan".to_string(),
                )
            })?;
            let root = context
                .workspace
                ...
                .ok_or_else(|| OpenHarnessError::tool("Workspace path not set".to_string()))?;
            ws_shell
                .exec("mkdir -p .openharness/plans", Some(30_000))
                .await
                .map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to create plans directory: {}", e))
                })?;
            ...
            ws_fs
                ...
                .await
                .map_err(|e| OpenHarnessError::tool(format!("Failed to write plan file: {}", e)))?;
            plan_path
        } else {
            let workspace_path = context
                .workspace_root()
                .ok_or(OpenHarnessError::tool("Workspace path not set".to_string()))?;
