...
impl GrepTool {
    ...
    async fn call_remote(
        ...
        let ws_shell = context
            .ws_shell()
            .ok_or_else(|| OpenHarnessError::tool("Workspace shell not available".to_string()))?;
        ...
        let (stdout, _stderr, _exit_code) = ws_shell
            ...
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Remote grep failed: {}", e)))?;
...
- Multiline matching: By default patterns match within single lines only. For cross-line patterns like `struct \{[\s\S]*?field`, use `multiline: true`"#.to_string())
    ...
    async fn call_impl(
        ...
        let GrepSearchResult {
            ...
            Ok(Ok(result)) => result,
            Ok(Err(e)) => return Err(OpenHarnessError::tool(e)),
            Err(e) => return Err(OpenHarnessError::tool(format!("grep search failed: {}", e))),
        };
