...
impl GitTool {
        ...
        if let Some(dir) = working_directory {
            ...
            if trimmed.is_empty() {
                return context
                    ...
                    .ok_or_else(|| {
                        OpenHarnessError::tool("No workspace path available".to_string())
                    });
            }
            ...
            context
                ...
                .as_ref()
                .map(|w| w.root_path_string())
                .ok_or_else(|| OpenHarnessError::tool("No workspace path available".to_string()))
        }
    }
    ...
    async fn execute_remote_git_cli(
        ...
        let shell = context.ws_shell().ok_or_else(|| {
            OpenHarnessError::tool("Remote Git requires workspace shell (SSH)".to_string())
        })?;
        ...
        let (stdout, stderr, exit_code) = shell
            ...
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Remote git failed: {}", e)))?;
    ...
    async fn execute_status(repo_path: &str) -> OpenHarnessResult<Value> {
        let status = GitService::get_status(repo_path)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git status failed: {}", e)))?;
    ...
    async fn execute_diff(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        ...
        let diff_output = GitService::get_diff(repo_path, &params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git diff failed: {}", e)))?;
    ...
    async fn execute_log(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let commits = GitService::get_commits(repo_path, params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git log failed: {}", e)))?;
    ...
    async fn execute_add(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or(".");
        ...
        let result = GitService::add_files(repo_path, params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git add failed: {}", e)))?;
    ...
    async fn execute_commit(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let result = GitService::commit(repo_path, params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git commit failed: {}", e)))?;
    ...
    async fn execute_push(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let result = GitService::push(repo_path, params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git push failed: {}", e)))?;
    ...
    async fn execute_pull(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        let parts: Vec<&str> = args_str
            .split_whitespace()
            .filter(|s| !s.starts_with('-'))
        ...
        let result = GitService::pull(repo_path, params)
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Git pull failed: {}", e)))?;
    ...
    async fn execute_checkout(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        let branch_name = args_str
            ...
            .ok_or_else(|| OpenHarnessError::tool("Branch name is required".to_string()))?;
        ...
        .map_err(|e| OpenHarnessError::tool(format!("Git checkout failed: {}", e)))?;
    ...
    async fn execute_branch(repo_path: &str, args: Option<&str>) -> OpenHarnessResult<Value> {
        let args_str = args.unwrap_or("");
        ...
        if is_list {
            ...
            let branches = GitService::get_branches(repo_path, include_remote)
                .await
                .map_err(|e| OpenHarnessError::tool(format!("Git branch failed: {}", e)))?;
            ...
            let branch_name = args_str
                ...
                .ok_or_else(|| {
                    OpenHarnessError::tool("Branch name is required for deletion".to_string())
                })?;

            let result = GitService::delete_branch(repo_path, branch_name, force)
                .await
                .map_err(|e| OpenHarnessError::tool(format!("Git branch delete failed: {}", e)))?;
...
- End with:
  ...
  Generated with OpenHarness

  Co-Authored-By: OpenHarness"#.to_string())
    }
    ...
    async fn call_impl(
        ...
        let operation = input
            .get("operation")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("operation is required".to_string()))?;
