...
impl WebSearchTool {
    ...
    async fn search(
        ...
        let cli = reqwest::Client::builder()
            ...
            .build()
            .map_err(|err| {
                OpenHarnessError::tool(format!("Failed to create HTTP client: {}", err))
            })?;
        ...
        let res = cli
            ...
            .send()
            .await
            .map_err(|err| OpenHarnessError::tool(format!("Failed to send request: {}", err)))?;
        ...
        let text = res
            .text()
            .await
            .map_err(|err| OpenHarnessError::tool(format!("Failed to read response: {}", err)))?;

        self.parse_sse(&text)
    }

    fn parse_sse(&self, text: &str) -> OpenHarnessResult<String> {
        let out = text
            .lines()
        ...
        out.ok_or_else(|| OpenHarnessError::tool("Web search returned no content".to_string()))
    }
...
impl Tool for WebSearchTool {
    ...
    async fn description(&self) -> OpenHarnessResult<String> {
        Ok(
            r#"- Allows OpenHarness to search the web and use the results to inform responses
- Provides up-to-date information for current events and recent data
- Uses Exa's hosted MCP web search service with no local API key setup
- Returns search result information formatted as search result blocks
- Use this tool for accessing information beyond OpenHarness's knowledge cutoff
...
- Return up to 10 results per query"#
    ...
    async fn call_impl(
        ...
        let query = input
            .get("query")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("query is required".to_string()))?;
...
- Read webpage: {"url": "https://example.com/article"}"#
    ...
    async fn call_impl(
        ...
        let url = input
            .get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("url is required".to_string()))?;
        ...
        let client = reqwest::Client::builder()
            .user_agent("OpenHarness/1.0")
            .timeout(std::time::Duration::from_secs(30))
            .build()
            .map_err(|e| OpenHarnessError::tool(format!("Failed to create HTTP client: {}", e)))?;

        let response = client
            .get(url)
            .send()
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Failed to fetch URL: {}", e)))?;
        ...
        let content = response
            .text()
            .await
            .map_err(|e| OpenHarnessError::tool(format!("Failed to read response: {}", e)))?;
        ...
        let processed_content = match format {
            ...
            "json" => {
                ...
                serde_json::from_str::<Value>(&content)
                    .map_err(|e| OpenHarnessError::tool(format!("Invalid JSON response: {}", e)))?;
                content
            }
            _ => content,
        };
