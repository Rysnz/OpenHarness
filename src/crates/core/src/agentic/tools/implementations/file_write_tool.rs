...
- Only use emojis if the user explicitly requests it. Avoid writing emojis to files unless asked."#.to_string())
    ...
    async fn call_impl(
        ...
        let content = input
            .get("content")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("content is required".to_string()))?;
        ...
        if let Some(ws_fs) = context.ws_fs() {
            ...
            if let Some(parent) = Path::new(&resolved_path).parent() {
                fs::create_dir_all(parent).await.map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to create directory: {}", e))
                })?;
            }
            fs::write(&resolved_path, content).await.map_err(|e| {
                OpenHarnessError::tool(format!("Failed to write file {}: {}", resolved_path, e))
            })?;
        }
