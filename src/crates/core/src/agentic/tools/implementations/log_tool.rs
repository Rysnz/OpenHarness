...
impl LogTool {
    ...
    /// Read log file
    async fn read_log(&self, log_path: &str, lines: Option<usize>) -> OpenHarnessResult<String> {
        ...
        let mut file = fs::File::open(&path)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to open log file: {}", e)))?;

        let mut content = String::new();
        file.read_to_string(&mut content)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read log file: {}", e)))?;
    ...
    async fn analyze_log(&self, log_path: &str) -> OpenHarnessResult<Value> {
        let content = self.read_log(log_path, None).await?;
...
The tool will return the log content or analysis results that you can use to diagnose issues."#.to_string())
    ...
    async fn call_impl(
        ...
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::validation("Missing required field: action"))?;
        ...
        let log_input: LogToolInput = serde_json::from_value(input.clone())
            .map_err(|e| OpenHarnessError::validation(format!("Invalid input: {}", e)))?;

        let log_path = log_input
            .log_path
            .as_ref()
            .ok_or_else(|| OpenHarnessError::validation("log_path is required"))?;
        ...
        let result = match log_input.action.as_str() {
            ...
            "search" => {
                let pattern = log_input.pattern.as_ref().ok_or_else(|| {
                    OpenHarnessError::validation("pattern is required for search action")
                })?;
