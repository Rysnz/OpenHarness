...
impl CronTool {
    ...
    fn resolve_workspace(
        ...
        Self::validate_workspace_format(workspace, context).map_err(OpenHarnessError::tool)?;
    ...
    fn resolve_workspace_from_context(
        &self,
    ...
    fn resolve_effective_session_id(
        ...
        Self::validate_session_id(&resolved).map_err(OpenHarnessError::tool)?;
        Ok(resolved)
    }
    ...
    fn normalize_optional_name(name: Option<String>) -> OpenHarnessResult<Option<String>> {
        match name {
            Some(name) if name.trim().is_empty() => Err(OpenHarnessError::tool(
    ...
    fn validate_payload(payload: &str, field_name: &str) -> OpenHarnessResult<()> {
    ...
    fn parse_iso_timestamp_ms(value: &str, field_name: &str) -> OpenHarnessResult<i64> {
    ...
    fn seconds_to_every_ms(seconds: u64, field_name: &str) -> OpenHarnessResult<u64> {
        if seconds == 0 {
            return Err(OpenHarnessError::tool(format!(
        ...
        seconds
            .checked_mul(1_000)
            .ok_or_else(|| OpenHarnessError::tool(format!("{}.every is too large", field_name)))
    }

    fn serialize_job(job: &CronJob) -> OpenHarnessResult<Value> {
        serde_json::to_value(CronToolJobOutput::try_from(job)?)
            .map_err(|err| OpenHarnessError::serialization(err.to_string()))
    }

    fn serialize_jobs(jobs: &[CronJob]) -> OpenHarnessResult<Vec<Value>> {
...
impl CronToolScheduleInput {
    fn to_service_schedule(&self, field_name: &str) -> OpenHarnessResult<CronSchedule> {
        match self {
            Self::At { at } => {
                let at = at.trim();
                if at.is_empty() {
...
impl TryFrom<&CronSchedule> for CronToolScheduleOutput {
    type Error = OpenHarnessError;

    fn try_from(schedule: &CronSchedule) -> OpenHarnessResult<Self> {
        match schedule {
...
impl TryFrom<&CronJob> for CronToolJobOutput {
    ...
    fn try_from(job: &CronJob) -> OpenHarnessResult<Self> {
        Ok(Self {
            id: job.id.clone(),
            name: job.name.clone(),
...
- Same fields as "job", but every field is optional."#
    ...
    async fn call_impl(
        ...
        let params: CronToolInput = serde_json::from_value(input.clone())
            .map_err(|err| OpenHarnessError::tool(format!("Invalid input: {}", err)))?;
        ...
        match params.action {
            ...
            CronAction::List => {
                let cron_service = get_global_cron_service().ok_or_else(|| {
                    OpenHarnessError::tool("cron service not initialized".to_string())
                })?;
                let workspace =
            ...
            CronAction::Add => {
                ...
                let job = params
                    .job
                    .ok_or_else(|| OpenHarnessError::tool("job is required for add".to_string()))?;
            ...
            CronAction::Update => {
                ...
                let job_id = params.job_id.ok_or_else(|| {
                    OpenHarnessError::tool("job_id is required for update".to_string())
                })?;
                Self::validate_job_id(&job_id).map_err(OpenHarnessError::tool)?;
                let patch = params.patch.ok_or_else(|| {
                    OpenHarnessError::tool("patch is required for update".to_string())
                })?;
                if patch.is_empty() {
            ...
            CronAction::Remove => {
                ...
                let job_id = params.job_id.ok_or_else(|| {
                    OpenHarnessError::tool("job_id is required for remove".to_string())
                })?;
                Self::validate_job_id(&job_id).map_err(OpenHarnessError::tool)?;
            ...
            CronAction::Run => {
                ...
                let job_id = params.job_id.ok_or_else(|| {
                    OpenHarnessError::tool("job_id is required for run".to_string())
                })?;
                Self::validate_job_id(&job_id).map_err(OpenHarnessError::tool)?;
