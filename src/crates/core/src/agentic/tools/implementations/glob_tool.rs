...
fn resolve_glob_config(pattern: &str) -> (bool, bool) {
    let is_whitelisted = pattern.starts_with(".openharness")
        || pattern.contains("/.openharness")
        || pattern.contains("\\.openharness");
...
"#.to_string())
    ...
    async fn call_impl(
        ...
        let pattern = input
            .get("pattern")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("pattern is required".to_string()))?;
        ...
        if context.is_remote() {
            let ws_shell = context.ws_shell().ok_or_else(|| {
                OpenHarnessError::tool("Workspace shell not available".to_string())
            })?;
            ...
            let (_stdout, _stderr, exit_code) = ws_shell
                ...
                .await
                .map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to detect rg on remote: {}", e))
                })?;
            ...
            let (stdout, _stderr, _exit_code) = ws_shell
                ...
                .await
                .map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to glob on remote with rg: {}", e))
                })?;
        ...
        .await
        .map_err(|err| OpenHarnessError::tool(format!("Glob tool task failed: {}", err)))?
...
mod tests {
    ...
    fn make_temp_dir(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("openharness-glob-tool-{name}-{unique}"));
    ...
    fn does_not_expand_walk_root_outside_search_path() {
        let root = std::env::temp_dir().join("openharness-glob-root");
