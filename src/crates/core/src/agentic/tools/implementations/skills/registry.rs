...
const PROJECT_SKILL_SLOTS: &[(&str, &str, &str)] = &[
    (".openharness", "skills", "openharness"),
...
impl SkillRegistry {
    ...
    fn get_possible_paths_for_workspace(workspace_root: Option<&Path>) -> Vec<SkillRootEntry> {
        ...
        let openharness_skills = path_manager.user_skills_dir();
        if openharness_skills.exists() && openharness_skills.is_dir() {
            entries.push(SkillRootEntry {
                path: openharness_skills,
    ...
    pub async fn find_and_load_skill_for_workspace(
        ...
        workspace_root: Option<&Path>,
        agent_type: Option<&str>,
    ) -> OpenHarnessResult<SkillData> {
        let info = self
            .get_resolved_skills_for_workspace(workspace_root, agent_type)
            .await
            .into_iter()
            .find(|skill| skill.name == skill_name)
            .ok_or_else(|| OpenHarnessError::tool(format!("Skill '{}' not found", skill_name)))?;

        let skill_md_path = PathBuf::from(&info.path).join("SKILL.md");
        let content = fs::read_to_string(&skill_md_path).await.map_err(|error| {
            OpenHarnessError::tool(format!("Failed to read skill file: {}", error))
        })?;
