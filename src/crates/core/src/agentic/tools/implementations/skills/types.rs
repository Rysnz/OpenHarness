...
pub struct SkillInfo {
    ...
    pub dir_name: String,
    /// Whether this skill is bundled with OpenHarness as a built-in skill.
    #[serde(default)]
    pub is_builtin: bool,
...
impl SkillData {
    ...
    pub fn from_markdown(
        ...
        let (metadata, body) = FrontMatterMarkdown::load_str(content)
            .map_err(|e| OpenHarnessError::tool(format!("Invalid SKILL.md format: {}", e)))?;
        ...
        let name = metadata
            ...
            .map(|s| s.to_string())
            .ok_or_else(|| {
                OpenHarnessError::tool("Missing required field 'name' in SKILL.md".to_string())
            })?;
        ...
        let description = metadata
            ...
            .ok_or_else(|| {
                OpenHarnessError::tool(
                    "Missing required field 'description' in SKILL.md".to_string(),
                )
            })?;
        ...
        let dir_name = std::path::Path::new(&path)
            ...
            .ok_or_else(|| OpenHarnessError::tool(format!("Invalid skill path: {}", path)))?
            .to_string();
