//! Built-in skills shipped with OpenHarness.
//!
//! These skills are embedded into the `openharness-core` binary and installed into the user skills
...
pub async fn ensure_builtin_skills_installed() -> OpenHarnessResult<()> {
...
async fn sync_dir(dir: &Dir<'_>, dest_root: &Path) -> OpenHarnessResult<SyncStats> {
...
fn safe_join(root: &Path, relative: &Path) -> OpenHarnessResult<PathBuf> {
    if relative.is_absolute() {
        return Err(crate::util::errors::OpenHarnessError::validation(format!(
