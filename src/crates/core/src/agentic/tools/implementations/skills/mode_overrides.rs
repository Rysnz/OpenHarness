...
pub async fn load_user_mode_skill_overrides(
    mode_id: &str,
) -> OpenHarnessResult<UserModeSkillOverrides> {
...
fn mode_skills_object_mut(document: &mut Value) -> OpenHarnessResult<&mut Map<String, Value>> {
    if !document.is_object() {
        *document = Value::Object(Map::new());
    }

    document.as_object_mut().ok_or_else(|| {
        OpenHarnessError::config("Project mode skills must be a JSON object".to_string())
    })
}
...
pub fn set_mode_skill_disabled_in_document(
    ...
    let mode_object = mode_entry.as_object_mut().ok_or_else(|| {
        OpenHarnessError::config("Mode skills entry must be a JSON object".to_string())
    })?;
...
pub async fn load_project_mode_skills_document_local(
    ...
    match tokio::fs::read_to_string(&path).await {
        ...
        Err(error) => Err(OpenHarnessError::config(format!(
