...
- Use `replace_all` for replacing and renaming strings across the file. This parameter is useful if you want to rename a variable for instance."#
    ...
    async fn call_impl(
        ...
        let new_string = input
            .get("new_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("new_string is required".to_string()))?;

        let old_string = input
            .get("old_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("old_string is required".to_string()))?;
        ...
        if context.is_remote() {
            let ws_fs = context.ws_fs().ok_or_else(|| {
                OpenHarnessError::tool("Remote workspace file system is unavailable".to_string())
            })?;
            let content = ws_fs
            ...
            ws_fs
                ...
                .await
                .map_err(|e| OpenHarnessError::tool(format!("Failed to write file: {}", e)))?;
