 ...
 - The tool will fail gracefully if permissions are insufficient"#.to_string())
    ...
    async fn call_impl(
        ...
        let path_str = input
            .get("path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("path is required".to_string()))?;
        ...
        if context.is_remote() {
            let ws_shell = context.ws_shell().ok_or_else(|| {
                OpenHarnessError::tool(
                    "Workspace shell not available for remote Delete".to_string(),
                )
            })?;
            ...
            let (_stdout, stderr, exit_code) =
                ws_shell.exec(&rm_cmd, Some(15_000)).await.map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to delete on remote: {}", e))
                })?;
        ...
        if is_directory {
            if recursive {
                fs::remove_dir_all(path).await.map_err(|e| {
                    OpenHarnessError::tool(format!("Failed to delete directory: {}", e))
                })?;
            } else {
            ...
            fs::remove_file(path)
                .await
                .map_err(|e| OpenHarnessError::tool(format!("Failed to delete file: {}", e)))?;
        }
