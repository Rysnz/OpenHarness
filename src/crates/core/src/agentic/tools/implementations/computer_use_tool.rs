...
const COMPUTER_USE_DEBUG_SUBDIR: &str = ".openharness/computer_use_debug";
...
**`key_chord` / `type_text` / `scroll` / `wait`:** standard desktop automation without any screenshot step.\n",
    ...
    fn require_multimodal_tool_output_for_screenshot(
        ctx: &ToolUseContext,
    ) -> OpenHarnessResult<()> {
    ...
    fn ensure_click_has_no_coordinate_fields(input: &Value) -> OpenHarnessResult<()> {
    ...
    ) -> OpenHarnessResult<Vec<ScreenOcrTextMatch>> {
    ...
    ) -> OpenHarnessResult<(Value, ToolImageAttachment, String)> {
...
pub(crate) async fn computer_use_execute_mouse_click_tool(
    ...
    let act = input
        .get("action")
        .and_then(|v| v.as_str())
        .ok_or_else(|| OpenHarnessError::tool("action is required (click or wheel)".to_string()))?;
    match act {
        "click" => {
            let button = input
                .get("button")
...
fn parse_ocr_region_native(
    input: &Value,
) -> OpenHarnessResult<Option<crate::agentic::tools::computer_use_host::OcrRegionNative>> {
    let v = input
    ...
    let x0 = o.get("x0").and_then(|x| x.as_i64()).ok_or_else(|| {
        OpenHarnessError::tool("ocr_region_native.x0 (integer) is required.".to_string())
    })? as i32;
    let y0 = o.get("y0").and_then(|x| x.as_i64()).ok_or_else(|| {
        OpenHarnessError::tool("ocr_region_native.y0 (integer) is required.".to_string())
    })? as i32;
    let width = o.get("width").and_then(|x| x.as_u64()).ok_or_else(|| {
        OpenHarnessError::tool(
            "ocr_region_native.width (positive integer) is required.".to_string(),
        )
    })? as u32;
...
**`type_text`:** Type text; prefer clipboard for long content. Does **not** move the pointer — **Enter** **`key_chord`** may follow without a mandatory `screenshot` unless you moved the pointer since the last capture. If **`screenshot`** shows the correct chat is already open and the input may be focused, **try `type_text` first** before spending steps on `click_element` / `move_to_text`.",
    ...
    async fn call_impl(
        ...
        let host = context.computer_use_host.as_ref().ok_or_else(|| {
            OpenHarnessError::tool(
                "Computer use is only available in the OpenHarness desktop app.".to_string(),
            )
        })?;
        ...
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| OpenHarnessError::tool("action is required".to_string()))?;
        ...
        match action {
            ...
            "click_label" => {
                ...
                let label = input.get("label").and_then(|v| v.as_u64()).ok_or_else(|| {
                    OpenHarnessError::tool(
                        "click_label requires integer field `label`.".to_string(),
                    )
                })? as u32;
                if label == 0 {
                    return Err(OpenHarnessError::tool(
                ...
                let matched = latest_shot
                    ...
                    .cloned()
                    .ok_or_else(|| OpenHarnessError::tool(format!(
            ...
            "key_chord" => {
                let keys: Vec<String> = input
                    ...
                    .ok_or_else(|| OpenHarnessError::tool("keys array is required".to_string()))?
                    .iter()
                    .filter_map(|v| v.as_str().map(|s| s.to_string()))
                    .collect();
                if keys.is_empty() {
                    return Err(OpenHarnessError::tool("keys must not be empty".to_string()));
                }
            ...
            "type_text" => {
                let text = input
                    .get("text")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| OpenHarnessError::tool("text is required".to_string()))?;
            ...
            "wait" => {
                let ms = input
                    .get("ms")
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| OpenHarnessError::tool("ms is required".to_string()))?;
...
fn req_i32(input: &Value, key: &str) -> OpenHarnessResult<i32> {
    input
        .get(key)
        .and_then(|v| v.as_i64())
        .map(|v| v as i32)
        .ok_or_else(|| OpenHarnessError::tool(format!("{} is required (integer)", key)))
}
