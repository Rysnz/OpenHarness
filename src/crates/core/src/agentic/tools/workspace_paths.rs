...
//!
//! When OpenHarness runs on Windows but the open workspace is a **remote SSH** (POSIX) tree,
...
pub fn resolve_path(path: &str) -> OpenHarnessResult<String> {
...
pub fn posix_resolve_path_with_workspace(
    ...
    let path = path.trim();
    if path.is_empty() {
        return Err(OpenHarnessError::tool("path cannot be empty".to_string()));
    }
