//! Host abstraction for desktop automation (implemented in `openharness-desktop`).
...
pub trait ComputerUseHost: Send + Sync + std::fmt::Debug {
    ...
    ) -> OpenHarnessResult<Vec<OcrTextMatch>> {
    ...
    async fn accessibility_hit_at_global_point(
        ...
        _gx: f64,
        _gy: f64,
    ) -> OpenHarnessResult<Option<OcrAccessibilityHit>> {
        Ok(None)
    }
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_down(&self, _button: &str) -> OpenHarnessResult<()> {
        Err(OpenHarnessError::tool(
    ...
    /// `button`: "left" | "right" | "middle"
    async fn mouse_up(&self, _button: &str) -> OpenHarnessResult<()> {
        Err(OpenHarnessError::tool(
