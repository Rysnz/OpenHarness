...
impl ToolUseContext {
    ...
    pub fn resolve_workspace_tool_path(&self, path: &str) -> OpenHarnessResult<String> {
        ...
        if let Some(cancellation_token) = context.cancellation_token.as_ref() {
            tokio::select! {
                ...
                _ = cancellation_token.cancelled() => {
                    Err(crate::util::errors::OpenHarnessError::Cancelled("Tool execution cancelled".to_string()))
                }
            }
        } else {
