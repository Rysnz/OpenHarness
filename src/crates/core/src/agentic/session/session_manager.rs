...
impl SessionManager {
    fn normalize_session_title_input(title: &str) -> OpenHarnessResult<String> {
    ...
    pub async fn create_session_with_id_and_details(
        ...
        let _workspace_path = Self::session_workspace_from_config(&config).ok_or_else(|| {
            OpenHarnessError::Validation("Session workspace_path is required".to_string())
        })?;
    ...
    pub async fn update_session_title(
        ...
        if self.config.enable_persistence {
            let Some(workspace_path) = workspace_path.as_ref() else {
                return Err(OpenHarnessError::Session(format!(
    ...
    pub async fn complete_dialog_turn(
        ...
        let turn_index = self
            ...
            .ok_or_else(|| {
                OpenHarnessError::NotFound(format!("Dialog turn not found: {}", turn_id))
            })?;
        let mut turn = self
            .persistence_manager
    ...
    pub async fn persist_btw_turn(
        ...
        let session = self.sessions.get(child_session_id).ok_or_else(|| {
            OpenHarnessError::NotFound(format!("Session not found: {}", child_session_id))
        })?;
    ...
    pub async fn get_context_messages(&self, session_id: &str) -> OpenHarnessResult<Vec<Message>> {
        let context_messages = self.context_store.get_context_messages(session_id);
    ...
    pub async fn add_message(&self, session_id: &str, message: Message) -> OpenHarnessResult<()> {
    ...
    async fn try_generate_session_title_with_ai(
        ...
        let ai_client = ai_client_factory
            ...
            .await
            .map_err(|e| OpenHarnessError::AIClient(format!("Failed to get AI client: {}", e)))?;

        let response = ai_client
            .send_message(messages, None)
            .await
            .map_err(|e| OpenHarnessError::ai(format!("AI call failed: {}", e)))?;
