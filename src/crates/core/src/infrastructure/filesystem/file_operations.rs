...
impl FileOperationService {
    ...
    pub async fn read_file(&self, file_path: &str) -> OpenHarnessResult<FileReadResult> {
        ...
        let metadata = fs::metadata(path).await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to read file metadata: {}", e))
        })?;
        ...
        match fs::read_to_string(path).await {
            ...
            Err(_) => {
                let bytes = fs::read(path).await.map_err(|e| {
                    OpenHarnessError::service(format!("Failed to read file: {}", e))
                })?;
    ...
    pub async fn write_file(
    ...
    ) -> OpenHarnessResult<FileWriteResult> {
        let path = Path::new(file_path);
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create parent directory: {}", e))
            })?;
        }
    ...
    pub async fn write_binary_file(
        ...
        fs::write(path, data).await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to write binary file: {}", e))
        })?;
    ...
    pub async fn copy_file(&self, from: &str, to: &str) -> OpenHarnessResult<u64> {
        ...
        if let Some(parent) = to_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create target directory: {}", e))
            })?;
        }

        let bytes_copied = fs::copy(from_path, to_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to copy file: {}", e)))?;

        Ok(bytes_copied)
    }

    pub async fn move_file(&self, from: &str, to: &str) -> OpenHarnessResult<()> {
        let from_trim = from.trim();
        let to_trim = to.trim();
        let from_path = Path::new(from_trim);
        let to_path = Path::new(to_trim);
        ...
        fs::rename(from_path, to_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to move file: {}", e)))?;

        Ok(())
    }

    pub async fn delete_file(&self, file_path: &str) -> OpenHarnessResult<()> {
        let path = Path::new(file_path);
    ...
    pub async fn get_file_info(&self, file_path: &str) -> OpenHarnessResult<FileInfo> {
        let path = Path::new(file_path);
    ...
    pub async fn create_directory(&self, dir_path: &str) -> OpenHarnessResult<()> {
    ...
    pub async fn delete_directory(&self, dir_path: &str, recursive: bool) -> OpenHarnessResult<()> {
        let path = Path::new(dir_path);
        ...
        if recursive {
            fs::remove_dir_all(path).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to delete directory recursively: {}", e))
            })?;
        } else {
            fs::remove_dir(path).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to delete directory: {}", e))
            })?;
        }
    ...
    async fn validate_file_access(&self, path: &Path, is_write: bool) -> OpenHarnessResult<()> {
    ...
    async fn create_backup(&self, path: &Path) -> OpenHarnessResult<String> {
        ...
        fs::copy(path, &backup_path)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to create backup: {}", e)))?;
