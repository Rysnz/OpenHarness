...
impl FileTreeService {
    ...
    pub async fn build_tree_with_stats(
        &self,
        root_path: &str,
    ) -> OpenHarnessResult<(Vec<FileTreeNode>, FileTreeStatistics)> {
    ...
    fn should_skip_file(&self, file_name: &str) -> bool {
        ...
        // But .gitignore and .openharness are always shown
        if !self.options.include_hidden
            && file_name.starts_with('.')
            && file_name != ".gitignore"
            && file_name != ".openharness"
        {
            return true;
        }
    ...
    fn compile_search_regex(
        ...
        RegexBuilder::new(&search_pattern)
            ...
            .build()
            .map_err(|error| OpenHarnessError::service(format!("Invalid regex pattern: {}", error)))
    }
    ...
    fn search_file_content_lines(
        ...
        let file = File::open(path).map_err(|error| {
            OpenHarnessError::service(format!("Failed to open file: {}", error))
        })?;
        ...
        for (index, line_result) in reader.split(b'\n').enumerate() {
            ...
            let line_bytes = line_result.map_err(|error| {
                OpenHarnessError::service(format!("Failed to read file: {}", error))
            })?;
