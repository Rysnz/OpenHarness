...
impl CleanupService {
    ...
    pub async fn cleanup_all(&self) -> OpenHarnessResult<CleanupResult> {
    ...
    async fn cleanup_temp_files(&self) -> OpenHarnessResult<CleanupResult> {
    ...
    async fn cleanup_old_logs(&self) -> OpenHarnessResult<CleanupResult> {
    ...
    async fn cleanup_old_sessions(&self) -> OpenHarnessResult<CleanupResult> {
        let mut result = CleanupResult::default();
        ...
        let mut read_dir = fs::read_dir(&workspaces_dir)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to read workspaces: {}", e)))?;

        while let Some(entry) = read_dir.next_entry().await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to read workspace entry: {}", e))
        })? {
    ...
    async fn cleanup_oversized_cache(&self) -> OpenHarnessResult<CleanupResult> {
    ...
    async fn cleanup_old_files(
        ...
        dir: &Path,
        retention: Duration,
    ) -> OpenHarnessResult<CleanupResult> {
        let mut result = CleanupResult::default();
    ...
    async fn cleanup_by_size(&self, dir: &Path, max_size: u64) -> OpenHarnessResult<CleanupResult> {
        let mut result = CleanupResult::default();
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = OpenHarnessResult<()>> + Send + 'a>>
    where
        F: Fn(&std::fs::Metadata) -> bool + Copy + Send + 'a,
    {
        Box::pin(async move {
            ...
            while let Some(entry) = read_dir
                .next_entry()
                .await
                .map_err(|e| OpenHarnessError::service(format!("Failed to read entry: {}", e)))?
            {
                let path = entry.path();
    ...
    fn calculate_dir_size(
        dir: &Path,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = OpenHarnessResult<u64>> + Send + '_>>
    {
        Box::pin(async move {
            let mut total = 0u64;
