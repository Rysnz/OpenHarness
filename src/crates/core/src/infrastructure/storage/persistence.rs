...
impl PersistenceService {
    pub async fn new(base_dir: PathBuf) -> OpenHarnessResult<Self> {
        if !base_dir.exists() {
            fs::create_dir_all(&base_dir).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create storage directory: {}", e))
            })?;
        }
    ...
    pub async fn new_user_level(path_manager: Arc<PathManager>) -> OpenHarnessResult<Self> {
    ...
    pub async fn save_json<T: Serialize>(
        ...
        let json_data = serde_json::to_string_pretty(data)
            .map_err(|e| OpenHarnessError::service(format!("Serialization failed: {}", e)))?;
        ...
        fs::write(&temp_path, &json_data)
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to write temp file: {}", e)))?;

        fs::rename(&temp_path, &file_path).await.map_err(|e| {
            let _ = std::fs::remove_file(&temp_path);
            OpenHarnessError::service(format!("Failed to rename temp file: {}", e))
        })?;
    ...
    pub async fn load_json<T: for<'de> Deserialize<'de>>(
        ...
        let data: T = serde_json::from_str(&content)
            .map_err(|e| OpenHarnessError::service(format!("Deserialization failed: {}", e)))?;

        Ok(Some(data))
    }

    pub async fn delete(&self, key: &str) -> OpenHarnessResult<bool> {
        let json_path = self.base_dir.join(format!("{}.json", key));

        if json_path.exists() {
            fs::remove_file(&json_path).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to delete JSON file: {}", e))
            })?;
            return Ok(true);
        }
    ...
    async fn create_backup(&self, file_path: &Path, max_backups: usize) -> OpenHarnessResult<()> {
        let backup_dir = self.base_dir.join("backups");
        if !backup_dir.exists() {
            fs::create_dir_all(&backup_dir).await.map_err(|e| {
                OpenHarnessError::service(format!("Failed to create backup directory: {}", e))
            })?;
        }
    ...
    async fn cleanup_old_backups(
        ...
        let mut read_dir = fs::read_dir(backup_dir).await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to read backup directory: {}", e))
        })?;

        while let Some(entry) = read_dir
            .next_entry()
            .await
            .map_err(|e| OpenHarnessError::service(format!("Failed to read backup entry: {}", e)))?
        {
