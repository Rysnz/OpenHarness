...
impl AIClientFactory {
    ...
    pub async fn initialize_global() -> OpenHarnessResult<()> {
        ...
        let config_service = get_global_config_service().await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to get global config service: {}", e))
        })?;
        ...
        GLOBAL_AI_CLIENT_FACTORY.set(wrapper).map_err(|_| {
            OpenHarnessError::service("Failed to initialize global AIClientFactory".to_string())
        })?;
    ...
    pub async fn get_global() -> OpenHarnessResult<Arc<AIClientFactory>> {
        let wrapper = GLOBAL_AI_CLIENT_FACTORY.get().ok_or_else(|| {
            OpenHarnessError::service(
        ...
        guard
            .as_ref()
            .ok_or_else(|| OpenHarnessError::service("Global AIClientFactory is None".to_string()))
            .map(Arc::clone)
    }
    ...
    pub async fn update_global(new_factory: Arc<AIClientFactory>) -> OpenHarnessResult<()> {
        let wrapper = GLOBAL_AI_CLIENT_FACTORY.get().ok_or_else(|| {
            OpenHarnessError::service("Global AIClientFactory not initialized".to_string())
        })?;
...
pub async fn get_global_ai_client_factory() -> OpenHarnessResult<Arc<AIClientFactory>> {
    AIClientFactory::get_global().await
}

pub async fn initialize_global_ai_client_factory() -> OpenHarnessResult<()> {
