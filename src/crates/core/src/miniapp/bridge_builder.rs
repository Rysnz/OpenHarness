//! Bridge script builder — generate window.app Runtime Adapter (OpenHarness Hosted) for iframe.
...
(function() {{
  ...
  window.addEventListener('message', (e) => {{
    if (e.data?.type === 'openharness:event') {{
  ...
  window.parent.postMessage({{ method: 'openharness/request-theme' }}, '*');
}})();
"#,
...
pub fn scroll_boundary_script() -> &'static str {
    r#"<script>(()=>{const s=(e)=>{for(let n=e.target;n;n=n.parentNode){if(!(n instanceof Element))continue;if(n===document.documentElement||n===document.body)continue;const o=window.getComputedStyle(n).overflowY;if(o==='hidden'||o==='visible')continue;if(e.deltaY<0&&n.scrollTop>0)return false;if(e.deltaY>0&&n.scrollTop+n.clientHeight<n.scrollHeight)return false;}return true};window.addEventListener('wheel',e=>{if(!e.defaultPrevented&&s(e))window.parent.postMessage({jsonrpc:'2.0',method:'openharness/sandbox-wheel',params:{deltaX:e.deltaX,deltaY:e.deltaY,deltaZ:e.deltaZ,deltaMode:e.deltaMode}},'*')},{passive:true});})();</script>"#
}
...
pub fn build_miniapp_default_theme_css() -> &'static str {
    r#"<style id="openharness-theme-default">:root{--openharness-bg:#121214;--openharness-bg-secondary:#18181a;--openharness-bg-tertiary:#121214;--openharness-bg-elevated:#18181a;--openharness-text:#e8e8e8;--openharness-text-secondary:#b0b0b0;--openharness-text-muted:#858585;--openharness-accent:#60a5fa;--openharness-accent-hover:#3b82f6;--openharness-success:#34d399;--openharness-warning:#f59e0b;--openharness-error:#ef4444;--openharness-info:#E1AB80;--openharness-border:#2e2e32;--openharness-border-subtle:#27272a;--openharness-element-bg:#27272a;--openharness-element-hover:#3f3f46;--openharness-radius:6px;--openharness-radius-lg:10px;--openharness-font-sans:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;--openharness-font-mono:ui-monospace,SFMono-Regular,'SF Mono',Menlo,Consolas,monospace;--openharness-scrollbar-thumb:rgba(255,255,255,0.12);}</style>"#
}
