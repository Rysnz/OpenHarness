...
impl MiniAppStorage {
    ...
    pub async fn ensure_app_dir(&self, app_id: &str) -> OpenHarnessResult<()> {
        let dir = self.app_dir(app_id);
        let source = self.source_dir(app_id);
        tokio::fs::create_dir_all(&dir).await.map_err(|e| {
            OpenHarnessError::io(format!(
    ...
    pub async fn list_app_ids(&self) -> OpenHarnessResult<Vec<String>> {
        ...
        let mut read_dir = tokio::fs::read_dir(&root)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read miniapps dir: {}", e)))?;
        while let Some(entry) = read_dir
            .next_entry()
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read miniapps entry: {}", e)))?
        {
            let path = entry.path();
            if path.is_dir() {
    ...
    pub async fn load(&self, app_id: &str) -> OpenHarnessResult<MiniApp> {
        let meta_path = self.meta_path(app_id);
        let meta_content = tokio::fs::read_to_string(&meta_path).await.map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                OpenHarnessError::NotFound(format!("MiniApp not found: {}", app_id))
            } else {
                OpenHarnessError::io(format!("Failed to read meta: {}", e))
            }
        })?;
        let meta: MiniAppMeta = serde_json::from_str(&meta_content)
            .map_err(|e| OpenHarnessError::parse(format!("Invalid meta.json: {}", e)))?;
    ...
    pub async fn load_meta(&self, app_id: &str) -> OpenHarnessResult<MiniAppMeta> {
        let meta_path = self.meta_path(app_id);
    ...
    async fn load_source(&self, app_id: &str) -> OpenHarnessResult<MiniAppSource> {
    ...
    pub async fn load_source_only(&self, app_id: &str) -> OpenHarnessResult<MiniAppSource> {
        self.load_source(app_id).await
    }

    async fn load_npm_dependencies(&self, app_id: &str) -> OpenHarnessResult<Vec<NpmDep>> {
        ...
        let c = tokio::fs::read_to_string(&p)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read package.json: {}", e)))?;
        let pkg: serde_json::Value = serde_json::from_str(&c)
            .map_err(|e| OpenHarnessError::parse(format!("Invalid package.json: {}", e)))?;
    ...
    async fn load_compiled_html(&self, app_id: &str) -> OpenHarnessResult<String> {
        let p = self.compiled_path(app_id);
        tokio::fs::read_to_string(&p).await.map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                OpenHarnessError::NotFound(format!("Compiled HTML not found: {}", app_id))
            } else {
                OpenHarnessError::io(format!("Failed to read compiled.html: {}", e))
            }
        })
    }

    /// Save full MiniApp (meta, source files, compiled.html).
    pub async fn save(&self, app: &MiniApp) -> OpenHarnessResult<()> {
        self.ensure_app_dir(&app.id).await?;

        let meta = MiniAppMeta::from(app);
        let meta_path = self.meta_path(&app.id);
        let meta_json = serde_json::to_string_pretty(&meta).map_err(OpenHarnessError::from)?;
        tokio::fs::write(&meta_path, meta_json)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write meta: {}", e)))?;

        let sd = self.source_dir(&app.id);
        tokio::fs::write(sd.join(INDEX_HTML), &app.source.html)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write index.html: {}", e)))?;
        tokio::fs::write(sd.join(STYLE_CSS), &app.source.css)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write style.css: {}", e)))?;
        tokio::fs::write(sd.join(UI_JS), &app.source.ui_js)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write ui.js: {}", e)))?;
        tokio::fs::write(sd.join(WORKER_JS), &app.source.worker_js)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write worker.js: {}", e)))?;

        let esm_json = serde_json::to_string_pretty(&app.source.esm_dependencies)
            .map_err(OpenHarnessError::from)?;
        tokio::fs::write(sd.join(ESM_DEPS_JSON), esm_json)
            .await
            .map_err(|e| {
                OpenHarnessError::io(format!("Failed to write esm_dependencies.json: {}", e))
            })?;
        ...
        tokio::fs::write(self.compiled_path(&app.id), &app.compiled_html)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write compiled.html: {}", e)))?;

        Ok(())
    }

    async fn write_package_json(&self, app_id: &str, deps: &[NpmDep]) -> OpenHarnessResult<()> {
        ...
        let p = self.app_dir(app_id).join(PACKAGE_JSON);
        let json = serde_json::to_string_pretty(&pkg).map_err(OpenHarnessError::from)?;
        tokio::fs::write(&p, json)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write package.json: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn save_version(
        ...
        tokio::fs::create_dir_all(&versions_dir)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to create versions dir: {}", e)))?;
        let path = self.version_path(app_id, version);
        let json = serde_json::to_string_pretty(app).map_err(OpenHarnessError::from)?;
        tokio::fs::write(&path, json)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write version file: {}", e)))?;
        Ok(())
    }

    /// Load app storage (KV JSON). Returns empty object if missing.
    pub async fn load_app_storage(&self, app_id: &str) -> OpenHarnessResult<serde_json::Value> {
        ...
        let c = tokio::fs::read_to_string(&p)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read storage: {}", e)))?;
    ...
    pub async fn save_app_storage(
        ...
        let obj = current.as_object_mut().ok_or_else(|| {
            OpenHarnessError::validation("App storage is not an object".to_string())
        })?;
        obj.insert(key.to_string(), value);
        let p = self.storage_path(app_id);
        let json = serde_json::to_string_pretty(&current).map_err(OpenHarnessError::from)?;
        tokio::fs::write(&p, json)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write storage: {}", e)))?;
        Ok(())
    }

    /// Delete MiniApp directory entirely.
    pub async fn delete(&self, app_id: &str) -> OpenHarnessResult<()> {
        let dir = self.app_dir(app_id);
        if dir.exists() {
            tokio::fs::remove_dir_all(&dir).await.map_err(|e| {
                OpenHarnessError::io(format!("Failed to delete miniapp dir: {}", e))
            })?;
        }
        Ok(())
    }
    ...
    pub async fn list_versions(&self, app_id: &str) -> OpenHarnessResult<Vec<u32>> {
        ...
        let mut read_dir = tokio::fs::read_dir(&vdir)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read versions dir: {}", e)))?;
        while let Some(entry) = read_dir
            .next_entry()
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read versions entry: {}", e)))?
        {
    ...
    pub async fn load_version(&self, app_id: &str, version: u32) -> OpenHarnessResult<MiniApp> {
        let p = self.version_path(app_id, version);
        let c = tokio::fs::read_to_string(&p).await.map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                OpenHarnessError::NotFound(format!("Version v{} not found", version))
            } else {
                OpenHarnessError::io(format!("Failed to read version: {}", e))
            }
        })?;
        serde_json::from_str(&c)
            .map_err(|e| OpenHarnessError::parse(format!("Invalid version file: {}", e)))
    }
}
