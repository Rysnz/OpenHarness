...
impl JsWorkerPool {
    pub fn new(
        ...
        let runtime = detect_runtime().ok_or_else(|| {
            OpenHarnessError::validation("No JS runtime found (install Bun or Node.js)".to_string())
        })?;
    ...
    pub async fn get_or_spawn(
    ...
    ) -> OpenHarnessResult<Arc<Mutex<JsWorker>>> {
        ...
        .await
        .map_err(OpenHarnessError::validation)?;
    ...
    pub async fn install_deps(
        ...
        app_id: &str,
        _deps: &[NpmDep],
    ) -> OpenHarnessResult<InstallResult> {
        let app_dir = self.path_manager.miniapp_dir(app_id);
