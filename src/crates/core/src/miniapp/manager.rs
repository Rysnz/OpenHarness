...
impl MiniAppManager {
    ...
    pub async fn list(&self) -> OpenHarnessResult<Vec<MiniAppMeta>> {
    ...
    pub async fn get(&self, app_id: &str) -> OpenHarnessResult<MiniApp> {
    ...
    pub async fn mark_deps_installed(&self, app_id: &str) -> OpenHarnessResult<MiniApp> {
        let mut app = self.storage.load(app_id).await?;
        Self::ensure_runtime_state(&mut app);
    ...
    pub async fn clear_worker_restart_required(&self, app_id: &str) -> OpenHarnessResult<MiniApp> {
        let mut app = self.storage.load(app_id).await?;
        Self::ensure_runtime_state(&mut app);
    ...
    pub async fn rollback(&self, app_id: &str, version: u32) -> OpenHarnessResult<MiniApp> {
    ...
    pub async fn import_from_path(
        ...
        let meta_content = tokio::fs::read_to_string(&meta_path)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to read meta.json: {}", e)))?;
        ...
        tokio::fs::create_dir_all(&dest_source)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to create app dir: {}", e)))?;
        ...
        tokio::fs::write(dest_dir.join("meta.json"), meta_json)
            .await
            .map_err(|e| OpenHarnessError::io(format!("Failed to write meta.json: {}", e)))?;
        ...
        for name in &["index.html", "style.css", "ui.js", "worker.js"] {
            ...
            if from.exists() {
                tokio::fs::copy(&from, &to)
                    .await
                    .map_err(|e| OpenHarnessError::io(format!("Failed to copy {}: {}", name, e)))?;
            }
        }
        ...
        if esm_path.exists() {
            tokio::fs::copy(&esm_path, dest_source.join("esm_dependencies.json"))
                .await
                .map_err(|e| {
                    OpenHarnessError::io(format!("Failed to copy esm_dependencies.json: {}", e))
                })?;
        } else {
            tokio::fs::write(dest_source.join("esm_dependencies.json"), "[]")
                .await
                .map_err(|_e| OpenHarnessError::io("Failed to write esm_dependencies.json"))?;
        }
        ...
        if pkg_src.exists() {
            tokio::fs::copy(&pkg_src, dest_dir.join("package.json"))
                .await
                .map_err(|e| OpenHarnessError::io(format!("Failed to copy package.json: {}", e)))?;
        } else {
            ...
            tokio::fs::write(
                ...
                serde_json::to_string_pretty(&pkg).map_err(OpenHarnessError::from)?,
            )
            .await
            .map_err(|_e| OpenHarnessError::io("Failed to write package.json"))?;
        }
        ...
        if storage_src.exists() {
            tokio::fs::copy(&storage_src, dest_dir.join("storage.json"))
                .await
                .map_err(|e| OpenHarnessError::io(format!("Failed to copy storage.json: {}", e)))?;
        } else {
            tokio::fs::write(dest_dir.join("storage.json"), "{}")
                .await
                .map_err(|_e| OpenHarnessError::io("Failed to write storage.json"))?;
        }
        ...
        tokio::fs::write(dest_dir.join("compiled.html"), placeholder_html)
            .await
            .map_err(|_e| OpenHarnessError::io("Failed to write placeholder compiled.html"))?;
