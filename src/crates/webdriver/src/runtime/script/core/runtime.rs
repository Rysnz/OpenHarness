pub(super) fn script() -> &'static str {
    ...
    const EVENT_NAME = "openharness_webdriver_result";
    const STORE_KEY = "__openharnessWdElements";
    const LOG_KEY = "__openharnessWdLogs";
    const consolePatchedKey = "__openharnessWdConsolePatched";
    ...
    const ensureRuntimeState = () => {
      if (!window.__openharnessWdRuntimeState) {
        window.__openharnessWdRuntimeState = {
          pointer: {
            x: 0,
            y: 0,
            target: null,
      ...
      return window.__openharnessWdRuntimeState;
    };
    ...
    const ensureAlertState = (targetWindow = window) => {
      if (!targetWindow.__openharnessWdAlertState) {
        targetWindow.__openharnessWdAlertState = {
          open: false,
          type: null,
          text: "",
          defaultValue: null,
      ...
      return targetWindow.__openharnessWdAlertState;
    };

    const patchDialogs = (targetWindow = window) => {
      const patchedKey = "__openharnessWdDialogsPatched";
