...
(() => {{
  {helper}
  window.__openharnessWd.run({request_id}, {script}, {args}, {async_mode}, {frame_context});
}})();
"#,
...
return (async () => {{
  {helper}
  const response = await window.__openharnessWd.execute({script}, {args}, {async_mode}, {frame_context});
...
fn bridge_helper_script() -> String {
    format!(
        r#"
if (!window.__openharnessWd) {{
  window.__openharnessWd = (() => {{
{core_head}
{input}
{keyboard}
{pointer}
