...
pub(crate) fn perform_actions() -> &'static str {
    "async (actions) => { await window.__openharnessWd.performActions(actions); return null; }"
}

pub(crate) fn release_actions() -> &'static str {
    "async (pressedKeys, pressedButtons) => { await window.__openharnessWd.releaseActions(pressedKeys, pressedButtons); return null; }"
}

pub(crate) fn dismiss_alert() -> &'static str {
    "() => window.__openharnessWd.closeAlert(false)"
}

pub(crate) fn accept_alert() -> &'static str {
    "() => window.__openharnessWd.closeAlert(true)"
}

pub(crate) fn alert_text() -> &'static str {
    "() => window.__openharnessWd.getAlertText()"
}

pub(crate) fn send_alert_text() -> &'static str {
    "(text) => window.__openharnessWd.sendAlertText(text)"
}

pub(crate) fn take_logs() -> &'static str {
    "() => window.__openharnessWd.takeLogs()"
}
