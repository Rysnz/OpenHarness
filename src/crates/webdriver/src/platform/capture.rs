...
mod imp {
    ...
    pub async fn print_page<R: Runtime>(
        ...
        let css = format!(
            r#"(function() {{
                let style = document.getElementById('__openharness_webdriver_print_style');
                if (!style) {{
                    style = document.createElement('style');
                    style.id = '__openharness_webdriver_print_style';
        ...
        let _ = webview.eval(
            "(() => { document.getElementById('__openharness_webdriver_print_style')?.remove(); })();",
        );
        response
    }
