...
fn is_embedded_webdriver_mode() -> bool {
    cfg!(debug_assertions) && std::env::var_os("OPENHARNESS_WEBDRIVER_PORT").is_some()
}

fn resolve_logs_root() -> PathBuf {
    if let Some(path) = std::env::var_os("OPENHARNESS_LOG_DIR").map(PathBuf::from) {
        return path;
    }

    if let Some(path) = std::env::var_os("OPENHARNESS_E2E_LOG_DIR").map(PathBuf::from) {
        return path;
    }

    if is_embedded_webdriver_mode() {
        return std::env::temp_dir().join("openharness-e2e-logs");
    }
