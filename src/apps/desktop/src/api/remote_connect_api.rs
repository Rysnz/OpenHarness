...
pub fn cleanup_on_exit() {
    openharness_core::service::remote_connect::ngrok::cleanup_all_ngrok();
...
async fn restore_saved_bots() {
    use openharness_core::service::remote_connect::bot;
...
fn detect_mobile_web_dir() -> Option<String> {
    if let Ok(dir) = std::env::var("OPENHARNESS_MOBILE_WEB_DIR") {
        let p = std::path::Path::new(&dir);
        if p.join("index.html").exists() {
            log::info!("Using OPENHARNESS_MOBILE_WEB_DIR: {dir}");
            return Some(dir);
        }
        log::warn!("OPENHARNESS_MOBILE_WEB_DIR set but index.html not found: {dir}");
    }
...
fn detect_from_exe() -> Option<String> {
    ...
    if cfg!(target_os = "linux") {
        candidates.push(exe_dir.join("../lib/openharness/mobile-web/dist"));
        candidates.push(exe_dir.join("../lib/openharness/mobile-web"));
        candidates.push(exe_dir.join("../share/openharness/mobile-web/dist"));
        candidates.push(exe_dir.join("../share/openharness/mobile-web"));
        candidates.push(exe_dir.join("../share/com.openharness.desktop/mobile-web/dist"));
        candidates.push(exe_dir.join("../share/com.openharness.desktop/mobile-web"));
    }
...
fn detect_default_gateway_ip() -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        let output = process_manager::create_command("route")
            .args(["-n", "get", "default"])
            .output()
            .ok()?;
    ...
    #[cfg(target_os = "linux")]
    {
        let output = process_manager::create_command("ip")
...
pub async fn remote_connect_get_methods() -> Result<Vec<ConnectionMethodInfo>, String> {
    ...
    let infos = methods
        ...
        .map(|m| match m {
            ...
            ConnectionMethod::BotQq => ConnectionMethodInfo {
                id: "bot_qq".into(),
                name: "QQ Bot".into(),
                available: true,
                description: "Via QQ messenger".into(),
            },
...
fn parse_connection_method(
    ...
    match method {
        ...
        "bot_qq" => Ok(ConnectionMethod::BotQq),
...
pub async fn remote_connect_configure_bot(request: ConfigureBotRequest) -> Result<(), String> {
    ...
    let bot_config = match request.bot_type.as_str() {
        "qq" => BotConfig::Qq {
            ...
            sandbox: request.sandbox.unwrap_or(false),
        },
    ...
    if guard.is_none() {
        ...
        match &bot_config {
            BotConfig::Qq { .. } => config.bot_qq = Some(bot_config),
...
#[derive(Debug, Deserialize)]
pub struct WeixinQrStartRequest { pub base_url: Option<String> }
#[derive(Debug, Deserialize)]
pub struct WeixinQrPollRequest { pub session_key: String, pub base_url: Option<String> }

#[tauri::command]
pub async fn remote_connect_weixin_qr_start(request: WeixinQrStartRequest) -> Result<weixin::WeixinQrStartResponse, String> {
    weixin::weixin_qr_start(request.base_url).await.map_err(|e| format!("weixin qr: {e}"))
}
#[tauri::command]
pub async fn remote_connect_weixin_qr_poll(request: WeixinQrPollRequest) -> Result<weixin::WeixinQrPollResponse, String> {
    weixin::weixin_qr_poll(&request.session_key, request.base_url).await.map_err(|e| format!("weixin poll: {e}"))
}
