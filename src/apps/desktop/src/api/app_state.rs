...
use openharness_core::agentic::side_question::SideQuestionRuntime;
use openharness_core::agentic::{agents, tools};
use openharness_core::infrastructure::ai::{AIClient, AIClientFactory};
use openharness_core::miniapp::{initialize_global_miniapp_manager, JsWorkerPool, MiniAppManager};
use openharness_core::service::remote_ssh::{
    init_remote_workspace_manager, RemoteFileService, RemoteTerminalManager, SSHConnectionManager,
};
use openharness_core::service::{
    ai_rules, announcement, config, filesystem, mcp, token_usage, workspace,
};
use openharness_core::util::errors::*;
...
impl AppState {
    pub async fn new_async(
        ...
        let config_service = config::get_global_config_service().await.map_err(|e| {
            OpenHarnessError::config(format!("Failed to get global config service: {}", e))
        })?;

        let ai_client = Arc::new(RwLock::new(None));
        let ai_client_factory = AIClientFactory::get_global().await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to get global AIClientFactory: {}", e))
        })?;
        ...
        ai_rules::initialize_global_ai_rules_service()
            .await
            .map_err(|e| {
                OpenHarnessError::service(format!("Failed to initialize AI rules service: {}", e))
            })?;
        let ai_rules_service = ai_rules::get_global_ai_rules_service().await.map_err(|e| {
            OpenHarnessError::service(format!("Failed to get AI rules service: {}", e))
        })?;
    ...
    pub async fn set_remote_workspace(
        ...
        *self.remote_workspace.write().await = Some(workspace.clone());
        ...
        if let Ok(manager) = self.get_ssh_manager_async().await {
            let core_workspace = openharness_core::service::remote_ssh::RemoteWorkspace {
    ...
    pub async fn unregister_remote_workspace_entry(&self, connection_id: &str, remote_path: &str) {
        let rp =
            openharness_core::service::remote_ssh::normalize_remote_workspace_path(remote_path);
        if let Ok(manager) = self.get_ssh_manager_async().await {
        ...
        if let Some(state_manager) =
            openharness_core::service::remote_ssh::get_remote_workspace_manager()
        {
            state_manager
        ...
        let clear_slot = slot
            ...
            .map(|w| {
                w.connection_id == connection_id
                    && openharness_core::service::remote_ssh::normalize_remote_workspace_path(
                        &w.remote_path,
                    ) == rp
            })
            .unwrap_or(false);
        if clear_slot {
            *slot = None;
            if let Some(m) = openharness_core::service::remote_ssh::get_remote_workspace_manager() {
