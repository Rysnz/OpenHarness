...
use openharness_core::agentic::tools::implementations::skills::mode_overrides::{
...
use openharness_core::agentic::tools::implementations::skills::{
...
use openharness_core::agentic::workspace::RemoteWorkspaceFs;
...
use openharness_core::service::runtime::RuntimeManager;
...
async fn persist_user_mode_skill_selection(
    ...
    openharness_core::service::config::mode_config_canonicalizer::persist_mode_config_from_value(
        mode_id,
        serde_json::json!({
...
pub async fn add_skill(
    ...
    let target_dir = if level == "project" {
        if let Some(workspace_root) = workspace_root_from_input(workspace_path.as_deref()) {
            ...
            workspace_root.join(".openharness").join("skills")
        } else {
...
pub async fn download_skill_market(
    ...
    let resolved_npx = runtime_manager.resolve_command("npx").ok_or_else(|| {
        "Command 'npx' is not available. Install Node.js or configure OpenHarness runtimes."
            .to_string()
    })?;
