...
pub async fn set_config(
    ...
    {
        Ok(_) => {
            if let Err(e) = openharness_core::service::config::reload_global_config().await {
                warn!(
...
pub async fn import_config(state: State<'_, AppState>, config: Value) -> Result<Value, String> {
    ...
    let export_data: openharness_core::service::config::ConfigExport =
...
pub async fn get_mode_configs(_state: State<'_, AppState>) -> Result<Value, String> {
    let mode_configs =
        openharness_core::service::config::mode_config_canonicalizer::get_mode_config_views()
            .await
...
pub async fn get_mode_config(
    ...
    let config =
        openharness_core::service::config::mode_config_canonicalizer::get_mode_config_view(
            &mode_id,
        )
        .await
...
pub async fn set_mode_config(
    ...
    match openharness_core::service::config::mode_config_canonicalizer::persist_mode_config_from_value(
        &mode_id, config,
    )
    .await
    {
...
pub async fn reset_mode_config(
    ...
    match openharness_core::service::config::mode_config_canonicalizer::reset_mode_config_to_default(
        &mode_id,
    )
    .await
    {
...
pub async fn get_subagent_configs(state: State<'_, AppState>) -> Result<Value, String> {
    use openharness_core::service::config::types::SubAgentConfig;
...
pub async fn canonicalize_mode_configs(_state: State<'_, AppState>) -> Result<Value, String> {
    match openharness_core::service::config::mode_config_canonicalizer::canonicalize_mode_configs()
        .await
    {
        Ok(report) => {
            info!(
