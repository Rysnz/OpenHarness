...
use openharness_core::infrastructure::{
    BatchedFileSearchProgressSink, FileOperationOptions, FileSearchResult, FileSearchResultGroup,
    FileTreeNode, SearchMatchType,
};
use openharness_core::service::file_watch;
use openharness_core::service::remote_ssh::workspace_state::is_remote_path;
use openharness_core::service::remote_ssh::{get_remote_workspace_manager, RemoteWorkspaceEntry};
use openharness_core::service::workspace::{
...
fn remote_workspace_from_info(info: &WorkspaceInfo) -> Option<crate::api::RemoteWorkspace> {
    ...
    let name = info
        ...
        .unwrap_or(&cid)
        .to_string();
    let rp = openharness_core::service::remote_ssh::normalize_remote_workspace_path(
...
fn serialize_search_response(
    outcome: openharness_core::infrastructure::FileSearchOutcome,
    limit: usize,
) -> serde_json::Value {
...
pub struct CreatePartnerWorkspaceRequest {}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DeletePartnerWorkspaceRequest {
    pub workspace_id: String,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ResetPartnerWorkspaceRequest {
    pub workspace_id: String,
}
...
pub struct TestAIConfigConnectionRequest {
    pub config: openharness_core::service::config::types::AIModelConfig,
}
...
async fn apply_active_workspace_context(
    state: &State<'_, AppState>,
    app: &AppHandle,
    workspace_info: &openharness_core::service::workspace::manager::WorkspaceInfo,
) {
    #[cfg(not(target_os = "macos"))]
    let _ = app;
    ...
    *state.workspace_path.write().await = Some(workspace_info.root_path.clone());
    ...
    // creating `/.openharness` (or drive root) here which fails with access denied.
    ...
    if !skip_local_snapshot {
        if let Err(e) =
            openharness_core::service::snapshot::initialize_snapshot_manager_for_workspace(
...
pub async fn initialize_ai(state: State<'_, AppState>) -> Result<String, String> {
    ...
    let global_config: openharness_core::service::config::GlobalConfig = config_service
        .get_config(None)
        .await
    ...
    let ai_config = openharness_core::util::types::AIConfig::try_from(model_config.clone())
        .map_err(|e| format!("Failed to convert AI configuration: {}", e))?;
    let ai_client = openharness_core::infrastructure::ai::AIClient::new(ai_config);
...
pub async fn test_ai_config_connection(
...
) -> Result<openharness_core::util::types::ConnectionTestResult, String> {
    let model_name = request.config.name.clone();
    let supports_image_input = request.config.capabilities.iter().any(|cap| {
        matches!(
            cap,
            openharness_core::service::config::types::ModelCapability::ImageUnderstanding
        )
    }) || matches!(
        request.config.category,
        openharness_core::service::config::types::ModelCategory::Multimodal
    );
    ...
    let ai_client = openharness_core::infrastructure::ai::client::AIClient::new(ai_config);
                ...
                match ai_client.test_image_input_connection().await {
                    Ok(image_result) => {
                        ...
                        if !image_result.success {
                            let merged = openharness_core::util::types::ConnectionTestResult {
                                success: false,
                                response_time_ms,
...
) -> Result<Vec<openharness_core::util::types::RemoteModelInfo>, String> {
...
pub async fn fix_mermaid_code(
    ...
    use openharness_core::util::types::message::Message;
...
pub async fn open_remote_workspace(
    ...
    use openharness_core::service::remote_ssh::normalize_remote_workspace_path;
    use openharness_core::service::remote_ssh::workspace_state::remote_workspace_stable_id;
    use openharness_core::service::workspace::WorkspaceCreateOptions;
...
pub async fn create_partner_workspace(
    state: State<'_, AppState>,
    app: tauri::AppHandle,
    _request: CreatePartnerWorkspaceRequest,
) -> Result<WorkspaceInfoDto, String> {
    match state.workspace_service.create_partner_workspace(None).await {
        Ok(workspace_info) => {
            apply_active_workspace_context(&state, &app, &workspace_info).await;
            ...
            {
                warn!(
                    "Failed to sync workspace identity watchers after partner workspace creation: {}",
                    e
                );
            }

            info!(
                "Partner workspace created: workspace_id={}, path={}",
                workspace_info.id,
                workspace_info.root_path.display()
            );
            Ok(WorkspaceInfoDto::from_workspace_info(&workspace_info))
        ...
        Err(e) => {
            error!("Failed to create partner workspace: {}", e);
            Err(format!("Failed to create partner workspace: {}", e))
        }
    }
}

#[tauri::command]
pub async fn delete_partner_workspace(
    state: State<'_, AppState>,
    app: tauri::AppHandle,
    request: DeletePartnerWorkspaceRequest,
) -> Result<(), String> {
    let workspace_info = state
        .workspace_service
        .get_workspace(&request.workspace_id)
        .await
        .ok_or_else(|| format!("Partner workspace not found: {}", request.workspace_id))?;

    if workspace_info.workspace_kind != WorkspaceKind::Partner {
        return Err(format!(
            "Workspace is not an partner workspace: {}",
            request.workspace_id
        ));
    }

    let partner_id = workspace_info
        .partner_id
        .clone()
        .ok_or_else(|| "Default partner workspace cannot be deleted".to_string())?;

    if !state
        .workspace_service
        .is_partner_workspace_path(&workspace_info.root_path)
    {
        return Err(format!(
            "Workspace path is not a managed partner workspace: {}",
            workspace_info.root_path.display()
        ));
    }
    ...
    if is_active_workspace {
        state
            ...
            .await
            .map_err(|e| format!("Failed to close partner workspace before deletion: {}", e))?;
    }
    ...
    state
        ...
        .await
        .map_err(|e| format!("Failed to delete partner workspace files: {}", e))?;

    state
        .workspace_service
        .remove_workspace(&request.workspace_id)
        .await
        .map_err(|e| format!("Failed to remove partner workspace state: {}", e))?;
    ...
    {
        warn!(
            "Failed to sync workspace identity watchers after partner workspace deletion: {}",
            e
        );
    }

    info!(
        "Partner workspace deleted: workspace_id={}, partner_id={}, path={}",
        request.workspace_id,
        partner_id,
        workspace_info.root_path.display()
    );
...
#[tauri::command]
pub async fn reset_partner_workspace(
    state: State<'_, AppState>,
    app: tauri::AppHandle,
    request: ResetPartnerWorkspaceRequest,
) -> Result<WorkspaceInfoDto, String> {
    let workspace_info = state
        .workspace_service
        .get_workspace(&request.workspace_id)
    ...
    openharness_core::service::reset_workspace_persona_files_to_default(&workspace_info.root_path)
        .await
        .map_err(|e| format!("Failed to restore partner workspace persona files: {}", e))?;

    let updated_workspace = state
        .workspace_service
        .rescan_workspace(&request.workspace_id)
        .await
        .map_err(|e| format!("Failed to rescan partner workspace after reset: {}", e))?;
    ...
    info!(
        "Partner workspace reset: workspace_id={}, partner_id={:?}, path={}",
        request.workspace_id,
        workspace_info.partner_id,
        workspace_info.root_path.display()
    );
...
pub async fn reset_workspace_persona_files(
    ...
    if !state
        .workspace_service
        .is_partner_workspace_path(&workspace_path)
    {
        return Err(format!(
            "Workspace is not a managed partner workspace: {}",
            request.workspace_path
        ));
    }

    openharness_core::service::reset_workspace_persona_files_to_default(&workspace_path)
        .await
        .map_err(|e| {
            error!(
...
pub async fn reveal_in_explorer(request: RevealInExplorerRequest) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        if is_directory {
            ...
            openharness_core::util::process_manager::create_command("explorer")
                .arg(&normalized_path)
                .spawn()
    ...
    {
        if is_directory {
            openharness_core::util::process_manager::create_command("open")
                .arg(&request.path)
                .spawn()
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        openharness_core::util::process_manager::create_command("xdg-open")
            .arg(target)
            .spawn()
...
pub async fn search_files(
    ...
    use openharness_core::service::filesystem::FileSearchOptions;
...
pub async fn reload_global_config() -> Result<String, String> {
    match openharness_core::service::config::reload_global_config().await {
        Ok(_) => {
...
pub async fn get_global_config_status() -> Result<bool, String> {
    Ok(openharness_core::service::config::GlobalConfigManager::is_initialized())
}

#[tauri::command]
pub async fn subscribe_config_updates() -> Result<(), String> {
    if let Some(mut receiver) = openharness_core::service::config::subscribe_config_updates() {
        tokio::spawn(async move {
