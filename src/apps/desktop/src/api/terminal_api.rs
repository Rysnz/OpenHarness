...
use openharness_core::service::remote_ssh::workspace_state::get_remote_workspace_manager;
use openharness_core::service::runtime::RuntimeManager;
use openharness_core::service::terminal::TerminalEvent;
use openharness_core::service::terminal::{
...
impl TerminalState {
    ...
    pub async fn get_or_init_api(&self) -> Result<TerminalApi, String> {
        ...
        if !*initialized {
            ...
            // Set scripts directory to app data dir: {config_dir}/openharness/temp/scripts
            let scripts_dir = Self::get_scripts_dir();
            config.shell_integration.scripts_dir = Some(scripts_dir);

            // Prepend OpenHarness-managed runtime dirs to PATH so Bash/Skill commands can
