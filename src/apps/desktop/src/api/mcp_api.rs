...
use openharness_core::service::mcp::auth::{
    has_stored_oauth_credentials, MCPRemoteOAuthSessionSnapshot,
};
use openharness_core::service::mcp::config::MCPConfigService;
use openharness_core::service::mcp::protocol::{
    MCPPrompt, MCPResource, PromptsGetResult, ResourcesReadResult,
};
use openharness_core::service::mcp::MCPServerType;
use openharness_core::service::runtime::{RuntimeManager, RuntimeSource};
...
async fn load_mcp_resources(
    mcp_service: &openharness_core::service::mcp::MCPService,
    server_id: &str,
    refresh: bool,
...
pub async fn get_mcp_servers(state: State<'_, AppState>) -> Result<Vec<MCPServerInfo>, String> {
    ...
    for config in configs {
        ...
        let (command, command_available, command_source, command_resolved_path) =
            if transport == openharness_core::service::mcp::MCPServerTransport::Stdio {
...
pub async fn get_mcp_tool_ui_uri(
    ...
    let registry = openharness_core::agentic::tools::registry::get_global_tool_registry();
