            ...
            match tool.call(&request.input, &context).await {
                Ok(results) => {
                    let combined_result = if results.len() == 1 {
                        match &results[0] {
                            openharness_core::agentic::tools::framework::ToolResult::Result {
                                data,
                                ..
                            } => Some(data.clone()),
                            openharness_core::agentic::tools::framework::ToolResult::Progress {
                                content,
                                ..
                            } => Some(content.clone()),
                            openharness_core::agentic::tools::framework::ToolResult::StreamChunk {
                                data,
                                ..
                            } => Some(data.clone()),
                        }
                        ...
                        openharness_core::agentic::tools::framework::ToolResult::Result { data, .. } => {
                            data.clone()
                        }
                        openharness_core::agentic::tools::framework::ToolResult::Progress { content, .. } => content.clone(),
                        openharness_core::agentic::tools::framework::ToolResult::StreamChunk { data, .. } => data.clone(),
                                        }).collect::<Vec<_>>()
                                    }))
                    };
...
pub async fn submit_user_answers(
    ...
    use openharness_core::agentic::tools::user_input_manager::get_user_input_manager;
