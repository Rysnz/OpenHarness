...
use openharness_core::service::lsp::types::CompletionItem;
use openharness_core::service::lsp::{
    close_workspace, get_workspace_manager, open_workspace_with_emitter, ServerState,
};
use openharness_transport::TauriTransportAdapter;
...
pub async fn lsp_open_workspace(
    ...
    let emitter: Arc<dyn openharness_core::infrastructure::events::EventEmitter> =
...
) -> Result<Vec<openharness_core::service::lsp::types::InlayHint>, String> {
    let workspace_path = PathBuf::from(&request.workspace_path);
    let manager = get_workspace_manager(workspace_path)
        .await
        .map_err(|e| format!("Workspace not found: {}", e))?;
...
pub async fn lsp_list_workspaces() -> Result<Vec<String>, String> {
    use openharness_core::service::lsp::get_all_workspace_paths;
...
pub async fn lsp_detect_project(
    ...
    use openharness_core::service::lsp::project_detector::ProjectDetector;
