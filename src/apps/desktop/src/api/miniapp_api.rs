...
use openharness_core::infrastructure::events::{emit_global_event, BackendEvent};
use openharness_core::miniapp::{
...
use openharness_core::util::types::Message;
    ...
    fn from(d: MiniAppSourceDto) -> Self {
        MiniAppSource {
            ...
            esm_dependencies: d
                .esm_dependencies
                .into_iter()
                .map(|x| openharness_core::miniapp::EsmDep {
                    name: x.name,
                    version: x.version,
                    url: x.url,
                })
            ...
            npm_dependencies: d
                .npm_dependencies
                .into_iter()
                .map(|x| openharness_core::miniapp::NpmDep {
                    name: x.name,
                    version: x.version,
                })
                .collect(),
...
pub async fn miniapp_runtime_status(state: State<'_, AppState>) -> Result<RuntimeStatus, String> {
    ...
    Ok(RuntimeStatus {
        ...
        kind: Some(match info.kind {
            openharness_core::miniapp::RuntimeKind::Bun => "bun".to_string(),
            openharness_core::miniapp::RuntimeKind::Node => "node".to_string(),
        }),
...
fn validate_model(
    model: Option<&str>,
    ai_perms: &openharness_core::miniapp::AiPermissions,
...
fn build_messages_for_ai(
    ...
    for m in chat_messages {
        ...
        if role == "assistant" || role == "partner" {
