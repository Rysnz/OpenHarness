...
use log::{debug, warn};
use openharness_core::agentic::tools::computer_use_host::{
...
use openharness_core::agentic::tools::computer_use_optimizer::ComputerUseOptimizer;
use openharness_core::util::errors::{OpenHarnessError, OpenHarnessResult};
...
impl MacPointerGeo {
    ...
    fn full_pixel_center_to_global_f64(&self, cx: f64, cy: f64) -> OpenHarnessResult<(f64, f64)> {
...
impl PointerMap {
    ...
    fn map_image_to_global_f64(&self, x: i32, y: i32) -> OpenHarnessResult<(f64, f64)> {
        if self.image_w == 0
            || self.image_h == 0
            || self.content_w == 0
            || self.content_h == 0
    ...
    fn map_normalized_to_global_f64(&self, x: i32, y: i32) -> OpenHarnessResult<(f64, f64)> {
...
impl DesktopComputerUseHost {
  ...
  return (unix id of p as text) & "|" & (name of p) & "|" & (try (bundle identifier of p as text) on error "" end try)
    ...
    fn ensure_input_automation_allowed() -> OpenHarnessResult<()> {
        #[cfg(target_os = "macos")]
        {
            if macos::ax_trusted() {
                return Ok(());
            ...
            return Err(OpenHarnessError::tool(format!(
                "macOS Accessibility is not enabled for this executable. System Settings > Privacy & Security > Accessibility: add and enable OpenHarness. Development builds use the debug binary at: {}",
                exe
            )));
        }
        #[cfg(not(target_os = "macos"))]
    ...
    fn with_enigo<F, T>(f: F) -> OpenHarnessResult<T>
    where
        F: FnOnce(&mut Enigo) -> OpenHarnessResult<T>,
    {
        Self::ensure_input_automation_allowed()?;
        let settings = Settings::default();
        let mut enigo = Enigo::new(&settings)
            .map_err(|e| OpenHarnessError::tool(format!("enigo init: {}", e)))?;
        f(&mut enigo)
    }
    ...
    fn run_enigo_job<F, T>(job: F) -> OpenHarnessResult<T>
    where
        F: FnOnce(&mut Enigo) -> OpenHarnessResult<T> + Send,
        T: Send,
    {
        #[cfg(target_os = "macos")]
        {
    ...
    #[cfg(target_os = "macos")]
    fn post_mouse_moved_cg_global(x: f64, y: f64) -> OpenHarnessResult<()> {
        ...
        let source =
            CGEventSource::new(CGEventSourceStateID::CombinedSessionState).map_err(|_| {
                OpenHarnessError::tool("CGEventSource create failed (mouse_move)".to_string())
            })?;
        let pt = CGPoint { x, y };
        let ev = CGEvent::new_mouse_event(source, CGEventType::MouseMoved, pt, CGMouseButton::Left)
            .map_err(|_| OpenHarnessError::tool("CGEvent MouseMoved failed".to_string()))?;
    ...
    #[cfg(target_os = "macos")]
    fn smooth_mouse_move_cg_global(x1: f64, y1: f64) -> OpenHarnessResult<()> {
    ...
    #[cfg(not(target_os = "macos"))]
    fn smooth_mouse_move_enigo_abs(x1: f64, y1: f64) -> OpenHarnessResult<()> {
        const MIN_DIST: f64 = 2.5;
        const MIN_STEPS: usize = 8;
        const MAX_STEPS: usize = 85;
        const MAX_DURATION_MS: u64 = 400;
        ...
        Self::run_enigo_job(|e| {
            let (cx, cy) = e.location().map_err(|err| {
                OpenHarnessError::tool(format!("smooth_mouse_move: pointer location: {}", err))
            })?;
            let x0 = cx as f64;
            let y0 = cy as f64;
            let dx = x1 - x0;
            ...
            if dist < MIN_DIST {
                return e
                    ...
                    .map_err(|err| OpenHarnessError::tool(format!("mouse_move: {}", err)));
            }
            let duration_ms = (70.0 + dist * 0.28).min(MAX_DURATION_MS as f64) as u64;
            let steps = ((dist / 5.5).ceil() as usize).clamp(MIN_STEPS, MAX_STEPS);
            let step_delay = Duration::from_millis((duration_ms / steps as u64).max(1));
    ...
    fn map_button(s: &str) -> OpenHarnessResult<Button> {
    ...
    fn map_key(name: &str) -> OpenHarnessResult<Key> {
    ...
    fn encode_jpeg(rgb: &RgbImage, quality: u8) -> OpenHarnessResult<Vec<u8>> {
        let mut buf = Vec::new();
        ...
        .map_err(|e| OpenHarnessError::tool(format!("JPEG encode: {}", e)))?;
        Ok(buf)
    }
    ...
    fn raw_shot_from_rgb_crop(
        ...
        native_w: u32,
        native_h: u32,
    ) -> OpenHarnessResult<ComputerScreenshot> {
    ...
    fn ocr_full_primary_display_region() -> OpenHarnessResult<OcrRegionNative> {
        let screen = Screen::from_point(0, 0)
            .map_err(|e| OpenHarnessError::tool(format!("Screen capture init (OCR raw): {}", e)))?;
    ...
    ) -> OpenHarnessResult<OcrRegionNative> {
    ...
    fn screenshot_raw_native_region(
        ...
        let rgba = screen
            .capture()
            .map_err(|e| OpenHarnessError::tool(format!("Screenshot failed (OCR raw): {}", e)))?;
    ...
    fn crop_rgb(src: &RgbImage, x0: u32, y0: u32, w: u32, h: u32) -> OpenHarnessResult<RgbImage> {
    ...
    fn screenshot_sync_tool_with_capture(
    ...
    ) -> OpenHarnessResult<(ComputerScreenshot, PointerMap, Option<ComputerUseNavFocus>)> {
        ...
        let (pointer_image_x, pointer_image_y) = {
            let pointer_loc = Self::run_enigo_job(|e| {
                e.location()
                    .map_err(|err| OpenHarnessError::tool(format!("pointer location: {}", err)))
            });
            match pointer_loc {
    ...
    fn permission_sync() -> ComputerUsePermissionSnapshot {
        #[cfg(target_os = "macos")]
        {
            let platform_note = if cfg!(debug_assertions) && !macos::ax_trusted() {
                Some(
                    "Development build: grant Accessibility to target/debug/openharness-desktop (path appears in errors if mouse fails)."
                        .to_string(),
                )
            } else {
                None
    ...
    fn computer_use_guard_verified_ui(&self) -> OpenHarnessResult<()> {
        let s = self
            .state
            .lock()
            .map_err(|e| OpenHarnessError::tool(format!("lock: {}", e)))?;
    ...
    fn resolve_screenshot_capture(
        ...
        mouse_x: f64,
        mouse_y: f64,
    ) -> OpenHarnessResult<(image::RgbaImage, Screen)> {
        ...
        let screen = Screen::from_point(mx, my)
            .or_else(|_| Screen::from_point(0, 0))
            .map_err(|e| OpenHarnessError::tool(format!("Screen capture init: {}", e)))?;
        let rgba = screen.capture().map_err(|e| {
            OpenHarnessError::tool(format!(
                "Screenshot failed (on macOS grant Screen Recording for OpenHarness): {}",
                e
            ))
        })?;
        Ok((rgba, screen))
...
#[cfg(target_os = "macos")]
mod macos {
    use super::{OpenHarnessError, OpenHarnessResult};
    ...
    pub fn quartz_mouse_location() -> OpenHarnessResult<(f64, f64)> {
        unsafe {
...
impl DesktopComputerUseHost {
    ...
    async fn mouse_click_at_current_pointer(&self, button: &str) -> OpenHarnessResult<()> {
        let button = button.to_string();
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                let b = Self::map_button(&button)?;
                e.button(b, Direction::Click)
                    .map_err(|err| OpenHarnessError::tool(format!("click: {}", err)))
            })
        })
        .await
        .map_err(|e| OpenHarnessError::tool(e.to_string()))??;
...
impl ComputerUseHost for DesktopComputerUseHost {
    async fn permission_snapshot(&self) -> OpenHarnessResult<ComputerUsePermissionSnapshot> {
    ...
    async fn request_accessibility_permission(&self) -> OpenHarnessResult<()> {
        #[cfg(target_os = "macos")]
        {
    ...
    async fn request_screen_capture_permission(&self) -> OpenHarnessResult<()> {
        #[cfg(target_os = "macos")]
        {
    ...
    async fn screenshot_peek_full_display(&self) -> OpenHarnessResult<ComputerScreenshot> {
        let (shot, _map, _) = tokio::task::spawn_blocking(|| {
            let screen = Screen::from_point(0, 0).map_err(|e| {
                OpenHarnessError::tool(format!("Screen capture init (peek): {}", e))
            })?;
            let rgba = screen
                .capture()
                .map_err(|e| OpenHarnessError::tool(format!("Screenshot failed (peek): {}", e)))?;
            Self::screenshot_sync_tool_with_capture(
    ...
    async fn ocr_find_text_matches(
        &self,
        text_query: &str,
        region_native: Option<openharness_core::agentic::tools::computer_use_host::OcrRegionNative>,
    ) -> OpenHarnessResult<Vec<openharness_core::agentic::tools::computer_use_host::OcrTextMatch>>
    {
        ...
        Ok(desktop_matches
            .into_iter()
            .map(
                |m| openharness_core::agentic::tools::computer_use_host::OcrTextMatch {
                    text: m.text,
    ...
    async fn accessibility_hit_at_global_point(
        ...
        Option<openharness_core::agentic::tools::computer_use_host::OcrAccessibilityHit>,
    > {
        #[cfg(target_os = "macos")]
        {
    ...
    ) -> OpenHarnessResult<UiElementLocateResult> {
        Self::ensure_input_automation_allowed()?;
        #[cfg(target_os = "macos")]
        {
            return tokio::task::spawn_blocking(move || {
    ...
    fn map_image_coords_to_pointer_f64(&self, x: i32, y: i32) -> OpenHarnessResult<(f64, f64)> {
        let s = self
            .state
            .lock()
            .map_err(|e| OpenHarnessError::tool(format!("lock: {}", e)))?;
    ...
    fn map_image_coords_to_pointer(&self, x: i32, y: i32) -> OpenHarnessResult<(i32, i32)> {
        let (gx, gy) = self.map_image_coords_to_pointer_f64(x, y)?;
        Ok((gx.round() as i32, gy.round() as i32))
    }

    fn map_normalized_coords_to_pointer_f64(
        &self,
        x: i32,
        y: i32,
    ) -> OpenHarnessResult<(f64, f64)> {
        let s = self
            .state
            .lock()
            .map_err(|e| OpenHarnessError::tool(format!("lock: {}", e)))?;
    ...
    fn map_normalized_coords_to_pointer(&self, x: i32, y: i32) -> OpenHarnessResult<(i32, i32)> {
        let (gx, gy) = self.map_normalized_coords_to_pointer_f64(x, y)?;
        Ok((gx.round() as i32, gy.round() as i32))
    }

    async fn mouse_move_global_f64(&self, gx: f64, gy: f64) -> OpenHarnessResult<()> {
        debug!(
    ...
    async fn mouse_move(&self, x: i32, y: i32) -> OpenHarnessResult<()> {
        self.mouse_move_global_f64(x as f64, y as f64).await
    }

    async fn pointer_move_relative(&self, dx: i32, dy: i32) -> OpenHarnessResult<()> {
        if dx == 0 && dy == 0 {
            return Ok(());
        }
            ...
            tokio::task::spawn_blocking(move || {
                Self::run_enigo_job(|e| {
                    let (cx, cy) = macos::quartz_mouse_location().map_err(|err| {
                        OpenHarnessError::tool(format!("quartz pointer (relative move): {}", err))
                    })?;
                    ...
                    e.move_mouse(nx, ny, Coordinate::Abs).map_err(|err| {
                        OpenHarnessError::tool(format!("pointer_move_relative: {}", err))
                    })
                })
            })
            .await
    ...
    async fn mouse_click(&self, button: &str) -> OpenHarnessResult<()> {
    ...
    async fn mouse_click_authoritative(&self, button: &str) -> OpenHarnessResult<()> {
        debug!("computer_use: mouse_click_authoritative button={}", button);
        self.mouse_click_at_current_pointer(button).await
    }

    async fn mouse_down(&self, button: &str) -> OpenHarnessResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                ...
                e.button(b, Direction::Press)
                    .map_err(|err| OpenHarnessError::tool(format!("mouse_down: {}", err)))
            })
        })
        .await
        .map_err(|e| OpenHarnessError::tool(e.to_string()))??;
    ...
    async fn mouse_up(&self, button: &str) -> OpenHarnessResult<()> {
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                ...
                e.button(b, Direction::Release)
                    .map_err(|err| OpenHarnessError::tool(format!("mouse_up: {}", err)))
            })
        })
        .await
        .map_err(|e| OpenHarnessError::tool(e.to_string()))??;
    ...
    async fn scroll(&self, delta_x: i32, delta_y: i32) -> OpenHarnessResult<()> {
            ...
            Self::run_enigo_job(|e| {
                if delta_x != 0 {
                    e.scroll(delta_x, Axis::Horizontal).map_err(|err| {
                        OpenHarnessError::tool(format!("scroll horizontal: {}", err))
                    })?;
                }
                if delta_y != 0 {
                    e.scroll(delta_y, Axis::Vertical).map_err(|err| {
                        OpenHarnessError::tool(format!("scroll vertical: {}", err))
                    })?;
                }
                Ok(())
            })
    ...
    async fn key_chord(&self, keys: Vec<String>) -> OpenHarnessResult<()> {
        if keys.is_empty() {
            return Ok(());
        }
                ...
                if mapped.len() == 1 {
                    ...
                    for k in mods {
                        e.key(*k, Direction::Press)
                            .map_err(|err| OpenHarnessError::tool(format!("key press: {}", err)))?;
                    }
                    ...
                    e.key(last, Direction::Click)
                        .map_err(|err| OpenHarnessError::tool(format!("key click: {}", err)))?;
                    for k in mods.iter().rev() {
                        e.key(*k, Direction::Release).map_err(|err| {
                            OpenHarnessError::tool(format!("key release: {}", err))
                        })?;
                    }
                    if chord_has_modifier {
    ...
    async fn type_text(&self, text: &str) -> OpenHarnessResult<()> {
        if text.is_empty() {
            return Ok(());
        }
        ...
        tokio::task::spawn_blocking(move || {
            Self::run_enigo_job(|e| {
                e.text(&owned)
                    .map_err(|err| OpenHarnessError::tool(format!("type_text: {}", err)))
            })
        })
        .await
        .map_err(|e| OpenHarnessError::tool(e.to_string()))??;
    ...
    async fn wait_ms(&self, ms: u64) -> OpenHarnessResult<()> {
    ...
    fn computer_use_guard_click_allowed(&self) -> OpenHarnessResult<()> {
        let s = self
            .state
            .lock()
            .map_err(|e| OpenHarnessError::tool(format!("lock: {}", e)))?;
    ...
    fn computer_use_guard_click_allowed_relaxed(&self) -> OpenHarnessResult<()> {
