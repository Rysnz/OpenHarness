...
/// If unset or non-zero: write the exact JPEG passed to OCR into `computer_use_debug` under the app data dir (see implementation). Set `OPENHARNESS_COMPUTER_USE_OCR_DEBUG=0` to disable.
fn ocr_debug_save_enabled() -> bool {
    !matches!(
        std::env::var("OPENHARNESS_COMPUTER_USE_OCR_DEBUG"),
        Ok(v) if v == "0" || v.eq_ignore_ascii_case("false")
    )
}

/// Same directory as agent `screenshot` debug (`workspace/.openharness/computer_use_debug`), when PathManager is available.
fn computer_use_ocr_debug_dir() -> PathBuf {
    if let Ok(pm) = try_get_path_manager_arc() {
        return pm
            .default_partner_workspace_dir(None)
            .join(".openharness")
...
fn normalize_query(text_query: &str) -> OpenHarnessResult<String> {
...
pub fn crop_shot_to_ocr_region(
    ...
    let dyn_img = image::load_from_memory(&shot.bytes)
        .map_err(|e| OpenHarnessError::tool(format!("OCR crop: decode JPEG: {}", e)))?;
    ...
    .map_err(|e| OpenHarnessError::tool(format!("OCR crop: encode JPEG: {}", e)))?;
...
mod macos {
    ...
    use openharness_core::agentic::tools::computer_use_host::ComputerScreenshot;
    use openharness_core::util::errors::{OpenHarnessError, OpenHarnessResult};
    ...
    fn recognize_text_observations(
        jpeg_bytes: &[u8],
    ) -> OpenHarnessResult<Vec<Retained<VNRecognizedTextObservation>>> {
        ...
        handler
            ...
            .map_err(ns_error_to_openharness)?;
    ...
    fn ns_error_to_openharness(err: Retained<NSError>) -> OpenHarnessError {
        let desc = err.localizedDescription().to_string();
        OpenHarnessError::tool(format!("macOS Vision OCR failed: {}", desc))
    }
...
mod windows_backend {
    ...
    fn w<T>(r: windows::core::Result<T>) -> OpenHarnessResult<T> {
        r.map_err(|e| OpenHarnessError::tool(format!("Windows OCR: {}", e)))
    }
    ...
    pub fn find_text_matches(
        ...
        let result = (|| -> OpenHarnessResult<Vec<OcrTextMatch>> {
    ...
    pub fn find_text_matches(
        ...
        let mut api = match TessApi::new(None, "eng") {
            ...
            Err(_) => {
                ...
                api.ok_or_else(|| OpenHarnessError::tool(
