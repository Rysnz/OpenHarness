...
pub async fn locate_ui_element_center(
    ...
    let conn = AccessibilityConnection::new()
        .await
        .map_err(|e| OpenHarnessError::tool(format!("AT-SPI connection: {}.", e)))?;

    let registry_root = conn
        .root_accessible_on_registry()
        .await
        .map_err(|e| OpenHarnessError::tool(format!("AT-SPI registry root: {}.", e)))?;

    let children = registry_root
        .get_children()
        .await
        .map_err(|e| OpenHarnessError::tool(format!("AT-SPI get_children (registry): {}.", e)))?;
