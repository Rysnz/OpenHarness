...
) -> OpenHarnessResult<Vec<IUIAutomationElement>> {
    let mut out = Vec::new();
...
pub fn locate_ui_element_center(
    ...
    let root = unsafe {
        automation.ElementFromHandle(hwnd).map_err(|e| {
            OpenHarnessError::tool(format!("UI Automation ElementFromHandle failed: {}.", e))
        })?
    };
    ...
    let walker = unsafe {
        automation
            .RawViewWalker()
            .map_err(|e| OpenHarnessError::tool(format!("UI Automation RawViewWalker: {}.", e)))?
    };
...
pub fn accessibility_hit_at_global_point(
    ...
    let automation: IUIAutomation = unsafe {
        CoCreateInstance(&CUIAutomation, None, CLSCTX_INPROC_SERVER).map_err(|e| {
            OpenHarnessError::tool(format!("UI Automation (CoCreateInstance): {}.", e))
        })?
    };
    let pt = POINT {
        x: gx.round() as i32,
