...
fn frontmost_pid() -> OpenHarnessResult<i32> {
    let out = std::process::Command::new("/usr/bin/osascript")
        ...
        .output()
        .map_err(|e| OpenHarnessError::tool(format!("osascript spawn: {}", e)))?;
    ...
    s.trim()
        .parse::<i32>()
        .map_err(|_| OpenHarnessError::tool("Could not parse frontmost process id.".to_string()))
}
...
pub fn frontmost_window_bounds_global() -> OpenHarnessResult<(i32, i32, u32, u32)> {
    let pid = frontmost_pid()?;
    ...
    unsafe {
        ...
        let frame = element_frame_global(win).ok_or_else(|| {
            ...
            OpenHarnessError::tool(
                "Could not read AXPosition/AXSize for foreground window.".to_string(),
            )
        })?;
        ax_release(win as CFTypeRef);
