...
pub fn validate_query(q: &UiElementLocateQuery) -> OpenHarnessResult<()> {
    let t = q
        .title_contains
        .as_ref()
...
pub fn global_to_native_center(gx: f64, gy: f64) -> OpenHarnessResult<(u32, u32)> {
    let d = DisplayInfo::from_point(gx.round() as i32, gy.round() as i32)
        .map_err(|e| OpenHarnessError::tool(format!("DisplayInfo::from_point: {}", e)))?;
...
fn global_bounds_to_native_minmax(
    ...
    width: f64,
    height: f64,
) -> OpenHarnessResult<(u32, u32, u32, u32)> {
