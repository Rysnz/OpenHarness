# OpenHarness Relay Server

WebSocket relay server for OpenHarness Remote Connect. Bridges desktop (WebSocket) and mobile (HTTP) clients with E2E encryption support.
...
git clone https://github.com/GCWing/OpenHarness
cd OpenHarness/src/apps/relay-server
...
### What URL should I fill in OpenHarness Desktop?
...
```bash
# From project root
cargo build --release -p openharness-relay-server

# Run
RELAY_PORT=9700 ./target/release/openharness-relay-server
```
...
| `RELAY_ROOM_WEB_DIR` | `/tmp/openharness-room-web` | Directory for per-room uploaded mobile-web files |
...
# First-time setup (creates /opt/openharness-relay, copies static/)
...
4. Fill the same URL into OpenHarness Desktop "Custom Server"
