...
use openharness_core::service::{ai_rules, config, filesystem, mcp, token_usage, workspace};
use std::sync::Arc;
...
pub async fn initialize(workspace: Option<String>) -> anyhow::Result<Arc<ServerAppState>> {
    log::info!("Initializing OpenHarness server core services");
    ...
    // LSP
    if let Err(e) = openharness_core::service::lsp::initialize_global_lsp_manager().await {
    ...
    log::info!("OpenHarness server core services initialized");
    Ok(state)
}
