...
//!
//! When running OpenHarness Server on a remote machine that cannot directly access
//! AI APIs (due to network restrictions), AI Relay acts as a local proxy:
//!
//! ```text
//! Remote Server                    Local Machine
//! ┌─────────────┐    SSH Tunnel    ┌─────────────┐
//! │ OpenHarness      │ ───────────────► │ AI Relay    │ ──► AI API
//! │ Server      │   ssh -R 9090:   │ :9090       │     (OpenAI, etc.)
//! └─────────────┘                  └─────────────┘
//! ```
//!
//! Usage:
//! 1. Start AI Relay on local machine: `openharness-server --ai-relay --port 9090`
//! 2. SSH to remote with reverse tunnel: `ssh -R 9090:localhost:9090 user@remote`
//! 3. Configure remote OpenHarness to use proxy: `proxy_url = "http://localhost:9090"`
