...
#[derive(Parser)]
#[command(name = "openharness")]
#[command(about = "OpenHarness CLI - AI agent-driven command-line programming assistant", long_about = None)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> Result<()> {
    ...
    if is_tui_mode {
        ...
        let log_dir = CliConfig::config_dir()
            .ok()
            .map(|d| d.join("logs"))
            .unwrap_or_else(|| std::env::temp_dir().join("openharness-cli"));

        std::fs::create_dir_all(&log_dir).ok();
        let log_file = log_dir.join("openharness-cli.log");
    ...
    match cli.command {
        Some(Commands::Chat { agent, workspace }) => {
            ...
            openharness_core::service::config::initialize_global_config()
                .await
                .context("Failed to initialize global config service")?;
            tracing::info!("Global config service initialized");

            let config_service = openharness_core::service::config::get_global_config_service()
                .await
                .ok();
            let original_skip_confirmation = if let Some(ref svc) = config_service {
                let ai_config: openharness_core::service::config::types::AIConfig =
        ...
        Some(Commands::Health) => {
            println!("OpenHarness CLI is running normally");
