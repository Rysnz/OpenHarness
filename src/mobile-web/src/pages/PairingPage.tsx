...
const OpenHarnessLogo: React.FC = () => (
  <div className="pairing-page__openharness-logo">
    <svg viewBox="0 0 120 120" role="img" aria-label="OpenHarness">
      <rect className="pairing-page__openharness-logo-backplate" x="12" y="12" width="96" height="96" rx="24" />
      <path className="pairing-page__openharness-logo-loop pairing-page__openharness-logo-loop--base" d="M76 24c18 6 30 23 30 42 0 25-20 44-45 44S16 90 16 65s20-45 45-45c5 0 10 1 15 4" />
      <path className="pairing-page__openharness-logo-loop pairing-page__openharness-logo-loop--flow" d="M76 24c18 6 30 23 30 42 0 25-20 44-45 44S16 90 16 65s20-45 45-45c5 0 10 1 15 4" />
      <path className="pairing-page__openharness-logo-loop pairing-page__openharness-logo-loop--sheen" d="M76 24c18 6 30 23 30 42 0 25-20 44-45 44S16 90 16 65s20-45 45-45c5 0 10 1 15 4" />
      <path className="pairing-page__openharness-logo-bridge" d="M42 45v31M78 45v31M42 60h36" />
      <circle className="pairing-page__openharness-logo-anchor pairing-page__openharness-logo-anchor--left" cx="42" cy="60" r="5" />
      <circle className="pairing-page__openharness-logo-anchor pairing-page__openharness-logo-anchor--center" cx="60" cy="60" r="6" />
      <circle className="pairing-page__openharness-logo-anchor pairing-page__openharness-logo-anchor--right" cx="78" cy="60" r="5" />
      <path className="pairing-page__openharness-logo-opening" d="M79 22l15-8 1 17" />
    </svg>
  </div>
);

const MOBILE_INSTALL_ID_KEY = 'openharness.mobile.install_id';
const MOBILE_USER_ID_KEY = 'openharness.mobile.user_id';
const MOBILE_LOCK_UNTIL_KEY = 'openharness.mobile.user_id_lock_until';
const MOBILE_FAILURE_COUNT_KEY = 'openharness.mobile.user_id_failure_count';
  ...
  const attemptPair = useCallback(async (
    ...
    try {
      ...
      if (initialSync.has_workspace) {
        if (initialSync.workspace_kind === 'partner' && initialSync.path) {
          store.setPairedDisplayMode('partner');
          store.setCurrentPartner({
            path: initialSync.path,
            name: initialSync.project_name ?? 'Partner',
            partner_id: initialSync.partner_id,
          });
