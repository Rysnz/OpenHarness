...
function agentLabel(agentType: string, t: (key: string) => string): string {
  switch (agentType) {
    ...
    case 'partner':
    case 'Partner':
      return t('sessions.agentPartner');
    default:
...
function isPartnerAgent(agentType: string): boolean {
  return agentType === 'partner' || agentType === 'Partner';
}

/** Pick first workspace suitable for Expert mode (exclude Partner partner roots when kind is known). */
function pickFirstProWorkspace(list: RecentWorkspaceEntry[]): RecentWorkspaceEntry | undefined {
  ...
  if (anyKind) {
    return list.find((w) => w.workspace_kind !== 'partner');
  }
  return list[0];
}
...
const SessionListPage: React.FC<SessionListPageProps> = ({ sessionMgr, onSelectSession, onOpenWorkspace }) => {
  ...
  const [displayMode, setDisplayMode] = useState<DisplayMode>(() => {
    ...
    if (hint === 'partner' || hint === 'pro') return hint;
    return 'pro';
  });

  const [partnerList, setPartnerList] = useState<Array<{ path: string; name: string; partner_id?: string }>>([]);
  ...
  // Load partner list when entering partner mode
  const loadPartnerList = useCallback(async () => {
    try {
      const partners = await sessionMgr.listPartners();
      setPartnerList(partners);
      // Set default partner if none selected
      if (!currentPartner && partners.length > 0) {
        const defaultPartner = partners.find(a => !a.partner_id) || partners[0];
        setCurrentPartner(defaultPartner);
        return defaultPartner.path;
      }
      return currentPartner?.path;
    } catch (e: any) {
      setError(e.message);
      return undefined;
    }
  }, [sessionMgr, currentPartner, setCurrentPartner, setError]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (cancelled) return;
        if (info.workspace_kind === 'partner' && info.path) {
          setCurrentPartner({
            path: info.path,
            name: info.project_name ?? 'Partner',
            partner_id: info.partner_id,
          });
          setCurrentWorkspace(null);
          setDisplayMode('partner');
  ...
  const refreshData = useCallback(async () => {
    try {
      if (displayMode === 'pro') {
        const info = await sessionMgr.getWorkspaceInfo();
        if (info.workspace_kind === 'partner') {
          setCurrentWorkspace(null);
          setSessions([]);
          setHasMore(false);
          offsetRef.current = 0;
        ...
        // Partner mode: use currentPartner path
        const resp = await sessionMgr.listSessions(currentPartner?.path, PAGE_SIZE, 0);
        setSessions(resp.sessions);
        setHasMore(resp.has_more);
        offsetRef.current = resp.sessions.length;
      }
    } catch { /* ignore */ }
  }, [sessionMgr, setSessions, setCurrentWorkspace, currentPartner?.path, displayMode]);
  ...
  const handleScroll = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (el.scrollHeight - el.scrollTop - el.clientHeight < 150) {
      const workspacePath = displayMode === 'partner' ? currentPartner?.path : currentWorkspace?.path;
      loadNextPage(workspacePath);
    }
  }, [displayMode, currentPartner?.path, currentWorkspace?.path, loadNextPage]);

  const handleCreate = useCallback(async (agentType: string) => {
    if (creating) return;
    setCreating(true);
    try {
      // For partner mode (Partner), use currentPartner.path
      ...
      const label = isPartnerAgent(agentType)
        ? t('sessions.remotePartnerSession')
  ...
  }, [creating, currentWorkspace?.path, currentPartner?.path, displayMode, loadFirstPage, onSelectSession, sessionMgr, setError, t]);

  const handleSelectMode = useCallback(async (mode: DisplayMode) => {
    setDisplayMode(mode);
    setShowPartnerPicker(false);
    if (mode === 'partner') {
      const partnerPath = await loadPartnerList();
      loadFirstPage(partnerPath);
    } else {
  ...
  }, [currentWorkspace?.path, loadFirstPage, loadPartnerList, trySelectFirstProWorkspace]);

  const handleSelectPartner = useCallback(async (partner: { path: string; name: string; partner_id?: string }) => {
    try {
      await sessionMgr.setPartner(partner.path);
      setCurrentPartner(partner);
      setShowPartnerPicker(false);
      loadFirstPage(partner.path);
    } catch (e: any) {
      setError(e.message);
    }
  }, [sessionMgr, setCurrentPartner, setError, loadFirstPage]);

  const workspaceDisplayName = currentWorkspace?.project_name || t('sessions.noWorkspaceSelected');
  const partnerDisplayName = currentPartner?.name || t('sessions.defaultPartner');
    ...
    <div className="session-list">
      <div className="session-list__header">
        <div className="session-list__header-brand">
          <img src={logoIcon} alt="OpenHarness" className="session-list__logo" />
      ...
      >
        ...
        <div className="session-list__mode-toggle">
          ...
          >
            <PartnerModeIcon />
            <span>{t('sessions.partnerMode')}</span>
          </button>
        </div>
        ...
        {/* Partner Mode: Partner Selection */}
        {!isProMode && (
          <>
            <div
              className="session-list__partner-bar"
              onClick={() => {
                loadPartnerList();
                setShowPartnerPicker(true);
              }}
            >
              <span className="session-list__partner-icon">
                <PartnerModeIcon />
              </span>
              <div className="session-list__partner-copy">
                <span className="session-list__partner-label">{t('sessions.partner')}</span>
                <span className="session-list__partner-name">{partnerDisplayName}</span>
              </div>
              <span className="session-list__partner-switch" aria-label={t('sessions.switchPartner')}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m7 15 5 5 5-5"/><path d="m7 9 5-5 5 5"/></svg>
              </span>
            </div>
            ...
            {showPartnerPicker && (
              <div className="session-list__picker-overlay" onClick={() => setShowPartnerPicker(false)}>
                <div className="session-list__picker-modal" onClick={e => e.stopPropagation()}>
                  <div className="session-list__picker-header">
                    <h3>{t('sessions.selectPartner')}</h3>
                    <button className="session-list__picker-close" onClick={() => setShowPartnerPicker(false)}>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                  </div>
                  <div className="session-list__picker-list">
                    {partnerList.map((partner, index) => (
                      <button
                        key={partner.path || index}
                        className={`session-list__picker-item ${currentPartner?.path === partner.path ? 'is-selected' : ''}`}
                        onClick={() => handleSelectPartner(partner)}
                      >
                        <span className="session-list__picker-item-icon">
                          <PartnerModeIcon />
                        </span>
                        <span className="session-list__picker-item-name">{partner.name}</span>
                        {currentPartner?.path === partner.path && (
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                        )}
                      </button>
                    ))}
            ...
            <section className={`session-list__panel ${!isProMode ? 'session-list__panel--partner' : ''}`}>
              ...
              {isProMode ? (
                ...
                <div className="session-list__create-row">
                  <button
                    className="session-list__create-btn session-list__create-btn--partner"
                    onClick={() => handleCreate('partner')}
                    disabled={creating}
                  >
                    <div className="session-list__create-icon">
                      <SessionTypeIcon agentType="partner" />
                    </div>
                    <div className="session-list__create-copy">
                      <span className="session-list__create-title">{t('sessions.partnerSession')}</span>
                      <span className="session-list__create-desc">{t('sessions.partnerSessionDesc')}</span>
                    </div>
                    <span className="session-list__create-arrow">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                    </span>
            ...
            {/* Session History */}
            <section className={`session-list__panel session-list__panel--sessions ${!isProMode ? 'session-list__panel--partner' : ''}`}>
              <div className="session-list__section-head">
                <div>
