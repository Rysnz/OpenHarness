...
export interface WorkspaceInfo {
  ...
  project_name?: string;
  git_branch?: string;
  /** Mirrors desktop `WorkspaceKind`: normal project, Partner partner workspace, or remote SSH. */
  workspace_kind?: 'normal' | 'partner' | 'remote';
  partner_id?: string;
}
...
export class RemoteSessionManager {
  ...
  async listPartners(): Promise<PartnerEntry[]> {
    const resp = await this.request<{
      resp: string;
  ...
  async setPartner(
    ...
    return this.request({ cmd: 'set_partner', path });
  }
