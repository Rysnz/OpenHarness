...
const RELAY_SERVER_README_URL = 'https://github.com/GCWing/OpenHarness/blob/main/src/apps/relay-server/README.md';
const FEISHU_SETUP_GUIDE_URLS = {
  'zh-CN': 'https://github.com/GCWing/OpenHarness/blob/main/docs/remote-connect/feishu-bot-setup.zh-CN.md',
  'en-US': 'https://github.com/GCWing/OpenHarness/blob/main/docs/remote-connect/feishu-bot-setup.md',
} as const;
...
export const RemoteConnectDialog: React.FC<RemoteConnectDialogProps> = ({
  ...
  const renderInfoCard = (children: React.ReactNode) => (
    <div className="openharness-remote-connect__info-card">
      {children}
    </div>
  );
  ...
  const renderErrorBlock = () => {
    ...
    return (
      <div className="openharness-remote-connect__error-group">
        <p className="openharness-remote-connect__error">{error}</p>
        {isNgrokErr && (
          <button type="button" className="openharness-remote-connect__error-action" onClick={handleOpenNgrokSetup}>
  ...
  const renderConnectedView = (
    ...
    <div className="openharness-remote-connect__connected">
      <div className="openharness-remote-connect__status">
        <Badge variant="success">{t('remoteConnect.stateConnected')}</Badge>
        {userId && (
          <span className="openharness-remote-connect__peer-user-id">
            {t('remoteConnect.connectedUserId')}: {userId}
          </span>
        )}
      </div>
      <p className="openharness-remote-connect__hint">{t('remoteConnect.connectedHint')}</p>
      <button type="button" className="openharness-remote-connect__btn openharness-remote-connect__btn--disconnect" onClick={onDisconnect}>
    ...
    return (
      <div className="openharness-remote-connect__body">
        {connectionResult.qr_url && (
          <div
            className="openharness-remote-connect__qr-box"
        ...
        {connectionResult.bot_pairing_code && (
          <div className="openharness-remote-connect__pairing-code-box">
            <div className="openharness-remote-connect__pairing-code">
              {connectionResult.bot_pairing_code}
            </div>
          </div>
        )}
        ...
        <p className="openharness-remote-connect__hint">
          {activeGroup === 'bot' ? t('remoteConnect.botHint') : t('remoteConnect.scanHint')}
        </p>
        <button type="button" className="openharness-remote-connect__btn openharness-remote-connect__btn--cancel" onClick={handleCancelConnect}>
  ...
  const renderNetworkContent = () => {
    if (isRelayConnected && connectedNetworkTab === networkTab) {
      return (
        <>
          {networkTab === 'ngrok' && (
            <p className="openharness-remote-connect__ngrok-usage-link">
              <span
                className="openharness-remote-connect__description-link"
                role="link"
                tabIndex={0}
      ...
      <div className="openharness-remote-connect__body">
        {renderInfoCard(
          <>
            {networkTab === 'lan' && (lanNetworkInfo?.localIp || lanNetworkInfo?.gatewayIp) && (
              <div className="openharness-remote-connect__info-meta-group">
                {lanNetworkInfo?.localIp && (
                  <p className="openharness-remote-connect__info-meta">
            ...
            <p className="openharness-remote-connect__info-text">
        ...
        {networkTab === 'custom_server' && (
          <Input
            ...
            prefix={<span className="openharness-remote-connect__field-prefix">{t('remoteConnect.serverUrl')}</span>}
            value={customUrl}
        ...
        <button
          type="button"
          className="openharness-remote-connect__btn openharness-remote-connect__btn--connect"
    ...
    if (isBotConnected && connectedBotTab === botTab) {
      return (
        <div className="openharness-remote-connect__connected">
          ...
          <div className="openharness-remote-connect__mode-selector">
            <button
              type="button"
              className={`openharness-remote-connect__mode-btn ${!botVerboseMode ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className="openharness-remote-connect__btn openharness-remote-connect__btn--disconnect"
      ...
      <div className="openharness-remote-connect__body">
        {botTab === 'telegram' ? (
          ...
          <div className="openharness-remote-connect__bot-guide">
            {renderInfoCard(
              <>
                ...
                <div className="openharness-remote-connect__steps">
                  <p className="openharness-remote-connect__step">
                    1. {t('remoteConnect.botFeishuStep1Prefix')}
                    <span
                      className="openharness-remote-connect__step-link"
                      role="link"
                      tabIndex={0}
                  ...
                  <p className="openharness-remote-connect__step">2. {t('remoteConnect.botFeishuStep2')}</p>
                  <p className="openharness-remote-connect__step">3. {t('remoteConnect.botFeishuStep3')}</p>
                </div>
              </>,
            )}
            <Input
          ...
          <div className="openharness-remote-connect__bot-guide">
            {renderInfoCard(
              <div className="openharness-remote-connect__steps">
                <p className="openharness-remote-connect__info-text">{t('remoteConnect.botWeixinIntro')}</p>
                <p className="openharness-remote-connect__step">1. {t('remoteConnect.botWeixinStep1')}</p>
                <p className="openharness-remote-connect__step">2. {t('remoteConnect.botWeixinStep2')}</p>
              </div>,
            )}
            {weixinQrImageUrl && (
              <div className="openharness-remote-connect__weixin-qr">
                {isWeixinRasterQrSrc(weixinQrImageUrl) ? (
                  <img
                    src={weixinQrImageUrl}
                    alt="WeChat QR"
                    className="openharness-remote-connect__weixin-qr-img"
                  />
                ) : (
                  <div
                    className="openharness-remote-connect__weixin-qr-svg-wrap"
                    role="img"
                ...
                <p className="openharness-remote-connect__hint">{t('remoteConnect.botWeixinPolling')}</p>
                <button
                  type="button"
                  className="openharness-remote-connect__btn openharness-remote-connect__btn--cancel"
            ...
            {weixinQrSessionKey && !weixinQrImageUrl && weixinAwaitingPhoneConfirm && (
              <div className="openharness-remote-connect__weixin-qr openharness-remote-connect__weixin-qr--await">
                <p className="openharness-remote-connect__hint">{t('remoteConnect.botWeixinAwaitingPhoneConfirm')}</p>
                <button
                  type="button"
                  className="openharness-remote-connect__btn openharness-remote-connect__btn--cancel"
                  onClick={handleCancelWeixinQr}
            ...
            {weixinIlinkToken && weixinBotAccountId && !weixinQrSessionKey && (
              <p className="openharness-remote-connect__hint">{t('remoteConnect.botWeixinLinked')}</p>
            )}
          </div>
        )}
        {renderErrorBlock()}
  ...
  return (
    <>
      <Modal
        ...
        titleExtra={(
          <span className="openharness-remote-connect__title-extra">
            <button
              type="button"
              className="openharness-remote-connect__disclaimer-trigger"
        ...
        size="large"
      >
        <div className="openharness-remote-connect">
          {/* ── Group tabs ── */}
          <div className="openharness-remote-connect__groups">
            <button
              type="button"
              className={`openharness-remote-connect__group-btn${activeGroup === 'network' ? ' is-active' : ''}`}
            ...
            >
              ...
              {isRelayConnected && <span className="openharness-remote-connect__dot" />}
            </button>
            <span className="openharness-remote-connect__group-divider" />
            <button
              type="button"
              className={`openharness-remote-connect__group-btn${activeGroup === 'bot' ? ' is-active' : ''}`}
              onClick={() => { setActiveGroup('bot'); setConnectionResult(null); setError(null); }}
              disabled={isNetworkConnecting}
            >
              {t('remoteConnect.groupBot')}
              {isBotConnected && <span className="openharness-remote-connect__dot" />}
            </button>
          </div>

          {/* ── Sub-tabs ── */}
          {activeGroup === 'network' ? (
            <div className="openharness-remote-connect__subtabs">
              {NETWORK_TABS.map((tab, i) => (
                <React.Fragment key={tab.id}>
                  {i > 0 && <span className="openharness-remote-connect__subtab-divider" />}
                  <button
                    type="button"
                    className={`openharness-remote-connect__subtab${networkTab === tab.id ? ' is-active' : ''}${isRelayConnected && connectedNetworkTab === tab.id ? ' is-connected' : ''}`}
                  ...
                  >
                    ...
                    {isRelayConnected && connectedNetworkTab === tab.id && networkTab !== tab.id && (
                      <span className="openharness-remote-connect__dot-sm" />
                    )}
                  </button>
                </React.Fragment>
              ))}
            ...
            <div className="openharness-remote-connect__subtabs">
              {BOT_TABS.map((tab, i) => (
                <React.Fragment key={tab.id}>
                  ...
                  <button
                    type="button"
                    className={`openharness-remote-connect__subtab${botTab === tab.id ? ' is-active' : ''}${isBotConnected && connectedBotTab === tab.id ? ' is-connected' : ''}`}
