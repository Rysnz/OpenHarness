...
const NavPanel: React.FC<NavPanelProps> = ({ className = '' }) => {
  ...
  const updateClipOrigin = useCallback(() => {
    ...
    if (!container) return;
    const anchor = container.querySelector<HTMLElement>('.openharness-nav-panel__item-slot.is-departing-anchor');
    if (anchor) {
  ...
  const contentCls = [
    'openharness-nav-panel__content',
    showSceneNav && 'is-scene',
    useSplitOpen && 'is-split-open',
  ].filter(Boolean).join(' ');

  const sceneCls = [
    'openharness-nav-panel__layer openharness-nav-panel__layer--scene',
    showSceneNav && 'is-active',
  ].filter(Boolean).join(' ');

  return (
    <nav className={`openharness-nav-panel ${className}`} aria-label={t('nav.aria.mainNav')}>
      <div ref={contentRef} className={contentCls}>

        <div className="openharness-nav-panel__layer openharness-nav-panel__layer--main">
          <MainNav
        ...
        {SceneNavComponent && (
          <div className={sceneCls}>
            <Suspense fallback={null}>
              <div key={mountedSceneId} className="openharness-nav-panel__scene-inner">
                <SceneNavComponent />
              </div>
            </Suspense>
          </div>
