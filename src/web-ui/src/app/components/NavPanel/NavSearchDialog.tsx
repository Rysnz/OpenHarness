...
type ResultKind = 'workspace' | 'partner' | 'session';
...
const NavSearchDialog: React.FC<NavSearchDialogProps> = ({ open, onClose }) => {
  ...
  const { openedWorkspacesList, partnerWorkspacesList, setActiveWorkspace } = useWorkspaceContext();
  const { openScene } = useSceneManager();
  const { switchLeftPanelTab } = useApp();
  const setSelectedPartnerWorkspaceId = useMyAgentStore(s => s.setSelectedPartnerWorkspaceId);
  const openNurseryPartner = useNurseryStore(s => s.openPartner);
  ...
  const projectWorkspaces = useMemo(
    () => openedWorkspacesList.filter(w => w.workspaceKind !== WorkspaceKind.Partner),
  ...
  /** Sessions that resolve to an opened workspace (project + partner rows in the nav). */
  ...
  const results = useMemo((): SearchResultItem[] => {
    ...
    if (!q) {
      ...
      for (const w of partnerWorkspacesList.slice(0, MAX_PER_GROUP)) {
        const displayName = w.identity?.name?.trim() || w.name;
        items.push({ kind: 'partner', id: w.id, label: displayName, sublabel: w.description });
      }
      return items;
    }
    ...
    const filteredPartners = partnerWorkspacesList
      .filter(w => matchesQuery(q, w.name, w.identity?.name, w.description))
      .slice(0, MAX_PER_GROUP);
    for (const w of filteredPartners) {
      const displayName = w.identity?.name?.trim() || w.name;
      items.push({ kind: 'partner', id: w.id, label: displayName, sublabel: w.description });
    }
  ...
  const handleSelect = useCallback(async (item: SearchResultItem) => {
    ...
    if (item.kind === 'workspace') {
      ...
      setSelectedPartnerWorkspaceId(item.id);
      openNurseryPartner(item.id);
  ...
  }, [onClose, setActiveWorkspace, setSelectedPartnerWorkspaceId, openNurseryPartner, switchLeftPanelTab, openScene]);
  ...
  useEffect(() => {
    ...
    if (!list) return;
    const active = list.querySelector<HTMLButtonElement>('.openharness-nav-search-dialog__item--active');
  ...
  const partnerItems = results.filter(r => r.kind === 'partner');
  ...
  const renderGroup = (
    ...
    return (
      <div className="openharness-nav-search-dialog__group" key={groupLabel}>
        <div className="openharness-nav-search-dialog__group-label">{groupLabel}</div>
        {items.map((item, i) => {
          ...
          return (
            <button
              key={item.id}
              type="button"
              className={`openharness-nav-search-dialog__item${idx === activeIndex ? ' openharness-nav-search-dialog__item--active' : ''}`}
              onMouseEnter={() => setActiveIndex(idx)}
              onClick={() => void handleSelect(item)}
            >
              <span className="openharness-nav-search-dialog__item-icon">{icon(item)}</span>
              <span className="openharness-nav-search-dialog__item-content">
                <span className="openharness-nav-search-dialog__item-label">{item.label}</span>
                {item.sublabel && (
                  <span className="openharness-nav-search-dialog__item-sublabel">{item.sublabel}</span>
                )}
              </span>
            </button>
          );
  ...
  const dialog = (
    <div className="openharness-nav-search-dialog__overlay" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="openharness-nav-search-dialog__card" ref={cardRef}>
        <div className="openharness-nav-search-dialog__input-row">
          <Search
            ref={inputRef}
            className="openharness-nav-search-dialog__search"
        ...
        <div className="openharness-nav-search-dialog__results" ref={listRef}>
          {results.length === 0 && !showDefaultSessionColumn ? (
            <div className="openharness-nav-search-dialog__empty">{t('nav.search.empty')}</div>
          ) : (
            <>
              {renderGroup(t('nav.search.groupWorkspaces'), workspaceItems, () => <FolderOpen size={14} />)}
              {renderGroup(t('nav.search.groupPartners'), partnerItems, () => <User size={14} />)}
              {showDefaultSessionColumn ? (
                <div className="openharness-nav-search-dialog__group" key="nav-search-sessions-default">
                  <div className="openharness-nav-search-dialog__group-label">{t('nav.search.groupSessions')}</div>
                  <div className="openharness-nav-search-dialog__session-hint" role="status">
