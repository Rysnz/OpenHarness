...
const WORKSPACE_DRAG_MIME_TYPE = 'application/x-openharness-workspace';
...
const WorkspaceListSection: React.FC<WorkspaceListSectionProps> = ({ variant }) => {
  ...
  const workspaces = variant === 'partners'
    ? partnerWorkspacesList
    : normalWorkspacesList;
  const emptyLabel = variant === 'partners'
    ? t('nav.workspaces.emptyPartners')
  ...
  return (
    <div className={`openharness-nav-panel__workspace-list${draggedWorkspaceId ? ' is-dragging' : ''}`}>
      {workspaces.length === 0 ? (
        <div className="openharness-nav-panel__workspace-list-empty">
          {emptyLabel}
        </div>
      ) : (
        workspaces.map(workspace => (
          <div
            key={workspace.id}
            className={[
              'openharness-nav-panel__workspace-drop-target',
          ...
          >
            {dropTarget?.workspaceId === workspace.id && dropTarget.position === 'before' ? (
              <div className="openharness-nav-panel__workspace-drop-line" aria-hidden="true" />
            ) : null}
            <WorkspaceItem
              workspace={workspace}
