...
type SessionMode = 'code' | 'cowork' | 'partner';
...
const resolveSessionModeType = (session: Session): SessionMode => {
  ...
  if (normalizedMode === 'partner') return 'partner';
  return 'code';
};
...
interface SessionsSectionProps {
  ...
  /** When set (e.g. partner workspace), session row tooltip includes this partner name. */
  partnerLabel?: string;
  /** When false, hide the leading mode / running icon on each row (e.g. partner detail page). */
  showSessionModeIcon?: boolean;
  /** Restrict rows to one or more display modes. */
  modeFilter?: SessionMode | SessionMode[];
...
const SessionsSection: React.FC<SessionsSectionProps> = ({
  ...
  const sessions = useMemo(
    () => {
      const allowedModes = modeFilter
        ? new Set(Array.isArray(modeFilter) ? modeFilter : [modeFilter])
        : null;
      return Array.from(flowChatState.sessions.values())
        .filter((s: Session) => {
          if (workspacePath) {
            if (!sessionBelongsToWorkspaceNavRow(s, workspacePath, remoteConnectionId, remoteSshHost)) {
              return false;
            }
          } else if (s.workspacePath) {
            return false;
          }
          return !allowedModes || allowedModes.has(resolveSessionModeType(s));
        })
        .sort(compareSessionsForDisplay);
    },
    [flowChatState.sessions, workspacePath, remoteConnectionId, remoteSshHost, modeFilter]
  );
    ...
    (session: Session): string => {
      ...
      const label =
        mode === 'cowork'
          ...
          : mode === 'partner'
            ? t('nav.sessions.newPartnerSession')
    ...
    if (showEmptyState) {
      return (
        <div className="openharness-nav-panel__inline-empty openharness-nav-panel__inline-empty--tasks">
          <div className="openharness-nav-panel__inline-empty-title">
            {emptyTitle ?? t('nav.sessions.noSessions')}
          </div>
          {emptyDescription ? (
            <div className="openharness-nav-panel__inline-empty-description">
              {emptyDescription}
            </div>
          ) : null}
        </div>
  ...
  return (
    <div className="openharness-nav-panel__inline-list">
      {visibleItems.map(({ session, level }) => {
          ...
          const trimmedPartner = partnerLabel?.trim() ?? '';
          const showPartnerInTooltip = trimmedPartner.length > 0;
          const showRichTooltip = showPartnerInTooltip || isBtwChild;
          const tooltipContent = showRichTooltip ? (
            <div className="openharness-nav-panel__inline-item-tooltip">
              <div className="openharness-nav-panel__inline-item-tooltip-title">{sessionTitle}</div>
              {showPartnerInTooltip ? (
                <div className="openharness-nav-panel__inline-item-tooltip-meta">
                  {t('nav.sessions.partnerOwner', { name: trimmedPartner })}
                </div>
              ) : null}
              {isBtwChild ? (
                <div className="openharness-nav-panel__inline-item-tooltip-meta">
          ...
          const row = (
            <div
              className={[
                'openharness-nav-panel__inline-item',
                level === 1 && 'is-child',
            ...
            >
              {showSessionModeIcon && !isBtwChild ? (
                isRunning ? (
                  <Loader2
                    size={14}
                    className={[
                      'openharness-nav-panel__inline-item-icon',
                      'is-running',
                    ].join(' ')}
                  />
                ) : (
              ...
              {isEditing ? (
                <div className="openharness-nav-panel__inline-item-edit" onClick={e => e.stopPropagation()}>
                  <Input
                    ref={editInputRef}
                    className="openharness-nav-panel__inline-item-edit-field"
                    variant="default"
                    inputSize="small"
                    value={editingTitle}
                  ...
                  <IconButton
                    variant="success"
                    size="xs"
                    className="openharness-nav-panel__inline-item-edit-btn confirm"
                  ...
                  <IconButton
                    variant="default"
                    size="xs"
                    className="openharness-nav-panel__inline-item-edit-btn cancel"
                ...
                <>
                  <span className="openharness-nav-panel__inline-item-main">
                    <span className="openharness-nav-panel__inline-item-label">{sessionTitle}</span>
                    {isBtwChild ? (
                      <span className="openharness-nav-panel__inline-item-btw-badge">btw</span>
                    ) : null}
                  </span>
                  <div className="openharness-nav-panel__inline-item-actions">
                    <button
                      type="button"
                      className={`openharness-nav-panel__inline-item-action-btn${openMenuSessionId === session.sessionId ? ' is-open' : ''}`}
                  ...
                  {openMenuSessionId === session.sessionId && sessionMenuPosition && createPortal(
                    <div
                      ...
                      className="openharness-nav-panel__inline-item-menu-popover"
                      role="menu"
                      style={{ top: `${sessionMenuPosition.top}px`, left: `${sessionMenuPosition.left}px` }}
                    >
                      <button
                        type="button"
                        className="openharness-nav-panel__inline-item-menu-item"
                      ...
                      <button
                        type="button"
                        className="openharness-nav-panel__inline-item-menu-item is-danger"
      ...
      {topLevelSessions.length > SESSIONS_LEVEL_0 && (
        <button
          type="button"
          className="openharness-nav-panel__inline-toggle"
          onClick={() => {
            const total = topLevelSessions.length;
            if (expandLevel === 0) {
              setExpandLevel(1);
        ...
        >
          {expandLevel === 0 ? (
            <>
              <span className="openharness-nav-panel__inline-toggle-dots">···</span>
              <span>
