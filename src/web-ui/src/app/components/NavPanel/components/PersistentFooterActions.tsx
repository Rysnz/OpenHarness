...
const PersistentFooterActions: React.FC = () => {
  ...
  return (
    <>
      <div className="openharness-nav-panel__footer">
        <div className="openharness-nav-panel__footer-left">
          <div className="openharness-nav-panel__footer-more-wrap">
            <Tooltip content={t('nav.moreOptions')} placement="right" followCursor disabled={menuOpen}>
              <button
                type="button"
                className={`openharness-nav-panel__footer-btn openharness-nav-panel__footer-btn--icon${menuOpen ? ' is-active' : ''}`}
              ...
              >
                {menuOpen ? (
                  ...
                  <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
                    <MoreVertical size={15} className="openharness-nav-panel__footer-btn-icon-swap-default" />
                    <ChevronUp size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
                  </span>
                )}
              </button>
            </Tooltip>
            ...
            {menuOpen && (
              <>
                <div
                  className="openharness-nav-panel__footer-backdrop"
                  onClick={closeMenu}
                />
                <div
                  className={`openharness-nav-panel__footer-menu${menuClosing ? ' is-closing' : ''}`}
                  role="menu"
                >
                  <Tooltip
                  ...
                  >
                    <button
                      type="button"
                      className={`openharness-nav-panel__footer-menu-item${!hasWorkspace ? ' is-disabled' : ''}`}
                      role="menuitem"
                  ...
                  <div className="openharness-nav-panel__footer-menu-divider" />
                  <button
                    type="button"
                    className="openharness-nav-panel__footer-menu-item"
                    role="menuitem"
          ...
          <Tooltip content={t('scenes.shell')} placement="right">
            <button
              type="button"
              className={`openharness-nav-panel__footer-btn openharness-nav-panel__footer-btn--icon${showSceneNav && navSceneId === 'shell' ? ' is-active' : ''}`}
              aria-label={t('scenes.shell')}
              aria-pressed={showSceneNav && navSceneId === 'shell'}
              onClick={handleOpenShell}
            >
              <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
                <SquareTerminal size={15} className="openharness-nav-panel__footer-btn-icon-swap-default" />
                <Terminal size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
              </span>
            </button>
          </Tooltip>

        <div
          className="openharness-nav-panel__footer-multimodal-wrap"
        ...
        >
          {(() => {
            ...
            return (
              <>
                <button
                  type="button"
                  className={`openharness-nav-panel__footer-btn openharness-nav-panel__footer-btn--icon${isAnyActive ? ' is-active' : ''}${multimodalOpen ? ' is-hover-open' : ''}`}
                  aria-label={t('nav.multimodalTools')}
                  aria-expanded={multimodalOpen}
                  aria-haspopup="menu"
                >
                  <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
                    <Layers size={15} className="openharness-nav-panel__footer-btn-icon-swap-default" />
                    <PanelsTopLeft size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
                  </span>
                </button>

                {multimodalOpen && (
                  <div
                    className="openharness-nav-panel__footer-multimodal-menu"
                    role="menu"
                    aria-label={t('nav.multimodalTools')}
                  >
                    <button
                      type="button"
                      className={`openharness-nav-panel__footer-multimodal-item${isBrowserActive ? ' is-active' : ''}`}
                      role="menuitem"
                      aria-pressed={isBrowserActive}
                      onClick={handleOpenBrowser}
                    >
                      <Globe size={13} className="openharness-nav-panel__footer-multimodal-item-icon" />
                      <span className="openharness-nav-panel__footer-multimodal-item-label">{t('scenes.browser')}</span>
                    </button>

                    <button
                      type="button"
                      className={`openharness-nav-panel__footer-multimodal-item${isMermaidActive ? ' is-active' : ''}`}
                      role="menuitem"
                      aria-pressed={isMermaidActive}
                      onClick={handleOpenMermaidEditor}
                    >
                      <Network size={13} className="openharness-nav-panel__footer-multimodal-item-icon" />
                      <span className="openharness-nav-panel__footer-multimodal-item-label">{t('scenes.mermaidEditor')}</span>
                    </button>
                  </div>
                )}
              </>
          ...
          <Tooltip content={insightsTooltip} placement="right" followCursor>
            <button
              type="button"
              className={`openharness-nav-panel__footer-btn openharness-nav-panel__footer-btn--icon${isInsightsActive ? ' is-active' : ''}`}
              onClick={handleOpenInsights}
              aria-label={insightsTooltip}
            >
              <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
                <BarChart3 size={15} className="openharness-nav-panel__footer-btn-icon-swap-default" />
                <LineChart size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
              </span>
            </button>
          </Tooltip>
        </div>

        <div className="openharness-nav-panel__footer-right">
