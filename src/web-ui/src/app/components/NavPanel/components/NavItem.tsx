...
const NavItem: React.FC<NavItemProps> = ({
  ...
  return (
    ...
    >
      <span className="openharness-nav-panel__item-icon" aria-hidden="true">
        <Icon size={15} />
      </span>
      <span className="openharness-nav-panel__item-label">{displayLabel}</span>

      {badge && (
        <span
          ref={badgeRef}
          className={`openharness-nav-panel__item-badge ${onBadgeClick ? 'openharness-nav-panel__item-badge--clickable' : ''}`}
      ...
      {renderActions ? (
        <span className="openharness-nav-panel__item-actions-custom" onClick={e => e.stopPropagation()} onMouseDown={e => e.stopPropagation()}>
          {renderActions()}
        </span>
        ...
        actionTitle ? (
          <Tooltip content={actionTitle} placement="right" followCursor>
            <span
              className="openharness-nav-panel__item-action"
