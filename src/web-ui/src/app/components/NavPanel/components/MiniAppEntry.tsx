...
const MiniAppEntry: React.FC<MiniAppEntryProps> = ({
  ...
  return (
    <div className="openharness-nav-panel__miniapp-entry-wrap">
      <div
        className={[
          'openharness-nav-panel__miniapp-entry',
          isActive && 'is-active',
      ...
      >
        <span className="openharness-nav-panel__miniapp-entry-main">
          <span className="openharness-nav-panel__miniapp-entry-copy">
            <span className="openharness-nav-panel__miniapp-entry-title">{t('scenes.miniApps')}</span>
            <Badge variant="neutral" className="openharness-nav-panel__miniapp-badge">Beta</Badge>
          </span>
        </span>

        <span className="openharness-nav-panel__miniapp-entry-apps">
          {visibleApps.length > 0 ? (
            <>
              {visibleApps.map((app) => {
                ...
                return (
                  <Tooltip key={app.id} content={app.name} placement="right">
                    <span
                      className={[
                        'openharness-nav-panel__miniapp-bubble',
              ...
              {overflowCount > 0 ? (
                <span className="openharness-nav-panel__miniapp-bubble openharness-nav-panel__miniapp-bubble--more">
                  +{overflowCount}
                </span>
              ) : null}
            </>
