...
import { InputDialog, OpenHarnessLoading } from '@/component-library';
...
const FilesPanel: React.FC<FilesPanelProps> = ({
    ...
    if (data.scrollIntoView && finalExpandPath) {
      setTimeout(() => {
        ...
        if (targetElement) {
          ...
          targetElement.classList.add('openharness-file-explorer__node-content--highlighted');
          setTimeout(() => {
            targetElement.classList.remove('openharness-file-explorer__node-content--highlighted');
          }, 2000);
        }
      }, 300);
    }
  ...
  return (
    <div 
      ref={panelRef}
      className="openharness-files-panel"
      tabIndex={-1}
      onFocus={() => {}}
    >
      {!hideHeader && (
        <PanelHeader
          title={t('title')}
          className="openharness-files-panel__header"
          actions={
            workspacePath && (
              <IconButton
                size="xs"
      ...
      <div className="openharness-files-panel__content">
        {workspacePath && viewMode === 'search' && (
          <div className="openharness-files-panel__search">
            <Search
              ...
              loading={isSearching}
            />
            <div className="openharness-files-panel__search-toolbar">
              <div className="openharness-files-panel__search-modes">
                <button
                  type="button"
                  className={`openharness-files-panel__search-mode ${searchMode === 'content' ? 'active' : ''}`}
                ...
                <button
                  type="button"
                  className={`openharness-files-panel__search-mode ${searchMode === 'filenames' ? 'active' : ''}`}
                  onClick={() => setSearchMode('filenames')}
                >
                  {t('search.modeFiles')}
                </button>
              </div>
              <div className="openharness-files-panel__search-options">
                <Tooltip content={t('options.caseSensitive')}>
                  <button
                    type="button"
                    className={`openharness-files-panel__search-option ${searchOptions.caseSensitive ? 'active' : ''}`}
                ...
                <Tooltip content={t('options.wholeWord')}>
                  <button
                    type="button"
                    className={`openharness-files-panel__search-option ${searchOptions.wholeWord ? 'active' : ''}`}
                ...
                <Tooltip content={t('options.useRegex')}>
                  <button
                    type="button"
                    className={`openharness-files-panel__search-option ${searchOptions.useRegex ? 'active' : ''}`}
        ...
        <div
          className={`openharness-files-panel__main-content${
            fileDropHighlight ? ' openharness-files-panel__main-content--drop-target' : ''
          }`}
        >
        {!workspacePath ? (
          <div className="openharness-files-panel__placeholder">
            <div className="openharness-files-panel__placeholder-icon">
          ...
          searchQuery ? (
            <div className="openharness-files-panel__search-content">
              {searchLimitNotice && (
                <div className="openharness-files-panel__search-limit-notice">
                  <span>{searchLimitNotice}</span>
                </div>
              )}
              
              {searchError && (
                <div className="openharness-files-panel__error">
                  <p>❌ {searchError}</p>
                  <button 
                    className="openharness-files-panel__retry-button"
              ...
              {searchResults.length > 0 ? (
                <FileSearchResults
                  ...
                  className="openharness-files-panel__search-results"
                />
              ) : (
          ...
          loading && fileTree.length === 0 ? (
            <div className="openharness-files-panel__loading">
              <OpenHarnessLoading size="medium" text={t('status.loadingFileTree')} />
            </div>
          ) : error ? (
            <div className="openharness-files-panel__error">
              <p>❌ {error}</p>
            ...
            <FileExplorer
              ...
              className="openharness-files-panel__explorer"
              enablePathCompression={true}
      ...
      {transferProgress && (
        <div className="openharness-files-panel__transfer" role="status">
          <div className="openharness-files-panel__transfer-label">
          ...
          <div
            className={`openharness-files-panel__transfer-track${
              transferProgress.indeterminate ? ' openharness-files-panel__transfer-track--indeterminate' : ''
            }`}
          >
            <div
              className="openharness-files-panel__transfer-fill"
              style={
