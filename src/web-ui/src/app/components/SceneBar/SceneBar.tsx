...
const SceneBar: React.FC<SceneBarProps> = ({
  ...
  const sceneBarClassName = `openharness-scene-bar ${!hasWindowControls ? 'openharness-scene-bar--no-controls' : ''} ${className}`.trim();
  ...
  return (
    ...
    >
      <div className="openharness-scene-bar__tabs" style={tabsStyle}>
        {openTabs.map(tab => {
      ...
      {hasWindowControls && (
        <div className="openharness-scene-bar__controls">
          <WindowControls
