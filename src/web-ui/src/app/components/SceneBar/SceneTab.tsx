...
const SceneTab: React.FC<SceneTabProps> = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target.closest('.openharness-scene-tab__action')) return;
    e.preventDefault();
  }, [pinned]);
  ...
  return (
    <div
      ...
      className={[
        'openharness-scene-tab',
        isActive && 'openharness-scene-tab--active',
        pinned && 'openharness-scene-tab--pinned',
      ].filter(Boolean).join(' ')}
      onClick={handleClick}
    ...
    >
      ...
      <div className="openharness-scene-tab__content">
        {Icon && <Icon size={13} className="openharness-scene-tab__icon" aria-hidden="true" />}
        <span className="openharness-scene-tab__label">{label}</span>
        {subtitle && (
          <>
            <span className="openharness-scene-tab__sep" aria-hidden="true">/</span>
            <span className="openharness-scene-tab__subtitle">{subtitle}</span>
          </>
        )}
        {onActionClick && (
          <Tooltip content={actionTitle} placement="bottom" followCursor>
            <span
              className="openharness-scene-tab__action"
      ...
      {!pinned && (
        <button
          type="button"
          className="openharness-scene-tab__close"
