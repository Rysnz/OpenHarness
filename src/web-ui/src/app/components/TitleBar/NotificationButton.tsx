...
const NotificationButton: React.FC<NotificationButtonProps> = ({
  ...
  return (
    ...
    <button
      ref={buttonRef}
      className={[
        'openharness-notification-btn',
        activeNotification ? 'openharness-notification-btn--has-progress' : '',
        activeNotification?.variant === 'loading' ? 'openharness-notification-btn--loading' : '',
        navFooterHoverIconSwap && !activeNotification ? 'openharness-notification-btn--nav-hover-icon' : '',
        className,
      ].filter(Boolean).join(' ')}
    ...
    >
      {activeNotification ? (
        <>
          <div className="openharness-notification-btn__progress">
            {activeNotification.variant === 'loading' ? (
              <div className="openharness-notification-btn__loading-icon">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                  stroke="currentColor" strokeWidth="2.5"
                  className="openharness-notification-btn__spinner">
                  <path d="M12 2 A 10 10 0 0 1 22 12" strokeLinecap="round" />
                </svg>
              </div>
            ) : (
              <div className="openharness-notification-btn__progress-icon">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
            ...
            <span className="openharness-notification-btn__progress-text">
              {activeNotification.variant === 'loading'
          ...
          <div
            className="openharness-notification-btn__tooltip"
            style={{ transform: `translateX(calc(-50% + ${tooltipOffset}px))` }}
          >
            <div
              className="openharness-notification-btn__tooltip-content"
        ...
        unreadCount > 0 ? (
          <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
            <BellDot
              size={15}
              className="openharness-notification-btn__icon--has-message openharness-nav-panel__footer-btn-icon-swap-default"
            />
            <BellRing size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
          </span>
        ) : (
          <span className="openharness-nav-panel__footer-btn-icon-swap" aria-hidden="true">
            <Bell size={15} className="openharness-nav-panel__footer-btn-icon-swap-default" />
            <BellRing size={15} className="openharness-nav-panel__footer-btn-icon-swap-hover" />
          </span>
        )
      ) : (
        unreadCount > 0
          ? <BellDot size={14} className="openharness-notification-btn__icon--has-message" />
          : <Bell size={14} />
      )}
    </button>
    </Tooltip>
