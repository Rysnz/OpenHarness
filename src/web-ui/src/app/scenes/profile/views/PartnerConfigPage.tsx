...
const log = createLogger('PartnerConfigPage');
...
const PartnerConfigPage: React.FC = () => {
  ...
  const { partnerWorkspacesList, currentWorkspace } = useWorkspaceContext();

  const effectiveWorkspaceId = useMemo(() => {
    const inList = (id: string | null | undefined) =>
      id && partnerWorkspacesList.some((w) => w.id === id) ? id : null;

    // Explicit selection from nursery gallery takes highest priority,
    // followed by the selected partner store, then the active workspace.
    return (
      inList(activeWorkspaceId) ??
      inList(selectedPartnerWorkspaceId) ??
      (currentWorkspace?.workspaceKind === WorkspaceKind.Partner ? inList(currentWorkspace.id) : null) ??
      null
    );
  }, [
    activeWorkspaceId,
  ...
  const workspace = useMemo(
    () =>
      effectiveWorkspaceId
        ? partnerWorkspacesList.find((w) => w.id === effectiveWorkspaceId) ?? null
        : null,
    [partnerWorkspacesList, effectiveWorkspaceId],
  );
      ...
      <div className="acp-right-shell">
        ...
        <div className="acp-section acp-section--nested">
          <div className="acp-section__head">
            <span className="acp-section__title">{t('nursery.partner.personaDocsTitle')}</span>
          </div>
            ...
            {PERSONA_DOC_FILES.map((fileName) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="acp-persona-doc-row__label">{t(`nursery.partner.personaDocs.${labelKey}`)}</span>
        ...
        {/* Scheduled tasks — title/toolbar live inside PartnerScheduleView */}
        <div className="acp-section acp-section--nested acp-section--schedule">
          <div className="acp-section__schedule-body">
            {!workspacePath ? (
              <p className="acp-empty">{t('nursery.partner.scheduledSessionsNoWorkspace')}</p>
            ) : (
              <Suspense
                fallback={(
        ...
        <div className="acp-right-shell acp-right-shell--editor">
          <div className="acp-persona-editor">
            <div className="acp-persona-editor__head">
              ...
              <span className="acp-persona-editor__title">{t(`nursery.partner.personaDocs.${docLabelKey}`)}</span>
              <IconButton
                type="button"
                size="xs"
                variant="danger"
            ...
            <div className="acp-persona-editor__body">
              {error && <p className="acp-persona-editor__error">{t('nursery.partner.personaDocLoadFailed')}: {error}</p>}
              {loading ? (
      ...
      <div className="acp-layout">
        ...
        <div className="acp-layout__left">
          ...
          <div className="acp-sessions-area">
            <h2 className="acp-sessions-area__title">{t('nursery.partner.sessionsSectionTitle')}</h2>
            <SessionsSection
              workspaceId={workspace?.id}
              workspacePath={workspacePath}
              partnerLabel={identityName}
              isActiveWorkspace
