...
const NurseryGallery: React.FC = () => {
  ...
  const { partnerWorkspacesList, createPartnerWorkspace, setActiveWorkspace, deletePartnerWorkspace } = useWorkspaceContext();
  const openScene = useSceneStore(s => s.openScene);
  const { switchLeftPanelTab } = useApp();
  const { openTemplate, openPartner } = useNurseryStore();
  ...
  const handleCreatePartner = useCallback(async () => {
    ...
    setCreating(true);
    try {
      const newWorkspace = await createPartnerWorkspace();
      openPartner(newWorkspace.id);
    } catch (e) {
      log.error('Failed to create partner workspace', e);
    } finally {
      setCreating(false);
    }
  }, [creating, createPartnerWorkspace, openPartner]);

  const sortedPartnerWorkspacesList = useMemo(
    () => {
      const primary = partnerWorkspacesList.filter(w => !w.partnerId);
      const secondary = partnerWorkspacesList.filter(w => w.partnerId);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await deletePartnerWorkspace(deleteConfirm.workspaceId);
    } catch (e) {
      log.error('Failed to delete partner workspace', e);
    } finally {
      setDeleting(false);
      setDeleteConfirm(null);
    }
  }, [deleteConfirm, deleting, deletePartnerWorkspace]);

  const handleDeleteCancel = useCallback(() => {
    setDeleteConfirm(null);
  }, []);

  const handleNewPartnerSession = useCallback(
    async (workspace: WorkspaceInfo) => {
      openScene('session');
      switchLeftPanelTab('sessions');
      try {
        await flowChatManager.createChatSession({ workspacePath: workspace.rootPath }, 'Partner');
        await setActiveWorkspace(workspace.id);
      } catch (e) {
        log.error('Failed to create partner session from gallery', e);
      }
    },
    ...
    <GalleryLayout className="nursery-gallery">
      <GalleryPageHeader
        ...
        actions={(
          ...
          >
            <Plus size={15} />
            <span>{t('nursery.gallery.newPartner')}</span>
          </button>
        )}
      />
      ...
      <div className="gallery-zones">
        <GalleryZone
          id="nursery-partners-zone"
          title={t('nursery.gallery.partnersTitle')}
          subtitle={t('nursery.gallery.partnersSubtitle')}
          tools={(
            <span className="gallery-zone-count">{sortedPartnerWorkspacesList.length}</span>
          )}
        >
          <GalleryGrid minCardWidth={360}>
            {sortedPartnerWorkspacesList.map((workspace, i) => {
              const isPrimary = !workspace.partnerId;
              return (
                <PartnerCard
                  key={workspace.id}
                  workspace={workspace}
                  isPrimary={isPrimary}
                  onClick={() => openPartner(workspace.id)}
                  onNewSession={() => { void handleNewPartnerSession(workspace); }}
