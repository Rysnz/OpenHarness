/**
 * PartnerQuickInput — standalone input for the partner detail page.
 *
 * Sends a message by:
 *   1. Creating a new session under the partner workspace
...
const log = createLogger('PartnerQuickInput');

interface PartnerQuickInputProps {
  workspacePath: string;
  workspaceId?: string;
  partnerName?: string;
}

const PartnerQuickInput: React.FC<PartnerQuickInputProps> = ({
  workspacePath,
  workspaceId,
  partnerName,
}) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    setSending(true);
    try {
      // Switch to the partner workspace first
      if (workspaceId) {
...
export default PartnerQuickInput;
