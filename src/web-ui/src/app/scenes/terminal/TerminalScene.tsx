...
const TerminalScene: React.FC<TerminalSceneProps> = ({ isActive = true }) => {
  ...
  if (!isActive) {
    return <div className="openharness-terminal-scene" aria-hidden="true" />;
  }

  return (
    <div className="openharness-terminal-scene">
      {activeSessionId ? (
        <ConnectedTerminal
          key={activeSessionId}
        ...
        <div className="openharness-terminal-scene__empty">
          <SquareTerminal size={32} className="openharness-terminal-scene__empty-icon" />
          <p className="openharness-terminal-scene__empty-hint">{t('emptyState')}</p>
        </div>
      )}
    </div>
  );
