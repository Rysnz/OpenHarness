...
const SessionScene: React.FC<SessionSceneProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={[
        'openharness-session-scene',
        isDragging && 'openharness-session-scene--dragging',
    ...
    >
      ...
      {!isChatHidden && (
        <div
          className={`openharness-session-scene__chat-pane ${isDragging ? 'openharness-session-scene__chat-pane--dragging' : ''}`}
        >
          <ChatPane
            width={0}
            isFullscreen={false}
      ...
      {!isChatHidden && (
        <div
          ref={resizerRef}
          className={[
            'openharness-pane-resizer',
            state.layout.rightPanelCollapsed && 'openharness-pane-resizer--collapsed',
            isDragging && 'openharness-pane-resizer--dragging',
            isHovering && 'openharness-pane-resizer--hovering',
          ].filter(Boolean).join(' ')}
        ...
        >
          <div className="openharness-pane-resizer__line" />
          <div className="openharness-pane-resizer__handle">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="openharness-pane-resizer__icon">
      ...
      <div
        ...
        className={[
          'openharness-session-scene__aux-pane',
          state.layout.rightPanelCollapsed         && 'openharness-session-scene__aux-pane--collapsed',
          isDragging                               && 'openharness-session-scene__aux-pane--dragging',
          isRightAsMain                            && 'openharness-session-scene__aux-pane--editor-mode',
          isAuxPaneExpandingImmediate              && 'openharness-session-scene__aux-pane--no-animation',
        ].filter(Boolean).join(' ')}
        style={{
