...
const ShellNavEntryItem: React.FC<ShellNavEntryItemProps> = ({
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={[
        'openharness-shell-nav__terminal-item',
        isActive && 'is-active',
      ].filter(Boolean).join(' ')}
    ...
    >
      <Tooltip content={entry.name} placement="right">
        <span className="openharness-shell-nav__terminal-item-main">
          {showSavedBadge ? (
            <Bookmark size={14} className="openharness-shell-nav__terminal-icon openharness-shell-nav__terminal-icon--saved" />
          ) : (
            <SquareTerminal size={14} className="openharness-shell-nav__terminal-icon" />
          )}

          <span className="openharness-shell-nav__terminal-label">{entry.name}</span>

          {showSavedBadge ? (
            <span className="openharness-shell-nav__saved-indicator">{savedBadgeLabel}</span>
          ) : null}

          {entry.startupCommand ? (
            <span className="openharness-shell-nav__cmd-indicator">{startupCommandBadgeLabel}</span>
          ) : null}

          <span className={`openharness-shell-nav__terminal-dot${entry.isRunning ? ' is-running' : ' is-stopped'}`} />
        </span>
      </Tooltip>
      ...
      <Tooltip content={quickAction.title} placement="right">
        <button
          type="button"
          className="openharness-shell-nav__terminal-close"
          onClick={(event) => {
