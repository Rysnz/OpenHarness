...
function getWorkspaceDisplayName(workspace: WorkspaceInfo): string {
  return workspace.workspaceKind === WorkspaceKind.Partner
...
const ShellNavWorkspaceSwitcher: React.FC<ShellNavWorkspaceSwitcherProps> = ({
  ...
  return (
    <div className="openharness-shell-nav__workspace-switcher">
      <Tooltip
      ...
      >
        <button
          ...
          type="button"
          className={`openharness-shell-nav__workspace-trigger${workspaceMenuOpen ? ' is-active' : ''}${hasMultipleWorkspaces ? ' is-switchable' : ''}`}
          onClick={onToggle}
          aria-haspopup={hasMultipleWorkspaces ? 'menu' : undefined}
          aria-expanded={hasMultipleWorkspaces ? workspaceMenuOpen : undefined}
        >
          <span className="openharness-shell-nav__workspace-separator">/</span>
          <span className="openharness-shell-nav__workspace-name">{workspaceName}</span>
          {hasMultipleWorkspaces ? (
            <ChevronDown size={12} className="openharness-shell-nav__workspace-trigger-icon" />
          ) : null}
        </button>
      </Tooltip>
      ...
      {workspaceMenuOpen && hasMultipleWorkspaces && workspaceMenuPosition
        ? createPortal(
            <div
              ...
              className="openharness-shell-nav__workspace-menu"
              role="menu"
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`openharness-shell-nav__workspace-menu-item${isActive ? ' is-active' : ''}`}
                      onClick={() => { void onSelectWorkspace(workspace.id); }}
                    >
                      <span className="openharness-shell-nav__workspace-menu-check" aria-hidden="true">
                        {isActive ? <Check size={12} /> : null}
                      </span>
                      <span className="openharness-shell-nav__workspace-menu-text">{label}</span>
                    </button>
                  </Tooltip>
                );
              })}
