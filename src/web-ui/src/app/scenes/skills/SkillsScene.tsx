...
const SkillsScene: React.FC = () => {
  ...
  return (
    <div className="openharness-skills-scene">
                ...
                {market.marketSkills.map((skill, index) => {
                  ...
                  return (
                    <SkillCard
                      ...
                      meta={(
                        <span className="openharness-skills-scene__market-meta">
                          <TrendingUp size={12} />
                          {skill.installs ?? 0}
                        </span>
                      )}
      ...
      >
        {selectedInstalledSkill ? (
          <div className="openharness-skills-scene__detail-row">
            <span className="openharness-skills-scene__detail-label">{t('list.item.pathLabel')}</span>
            {canRevealSkillPath ? (
              <button
                type="button"
                className="openharness-skills-scene__detail-path-btn"
              ...
              <code className="openharness-skills-scene__detail-value">{selectedInstalledSkill.path}</code>
            )}
          </div>
        ) : null}
        ...
        {selectedMarketSkill?.source ? (
          <div className="openharness-skills-scene__detail-row">
            <span className="openharness-skills-scene__detail-label">{t('market.item.sourceLabel')}</span>
            <span className="openharness-skills-scene__detail-value">{selectedMarketSkill.source}</span>
          </div>
        ) : null}

        {selectedMarketSkill ? (
          <div className="openharness-skills-scene__detail-row">
            <span className="openharness-skills-scene__detail-label">{t('market.detail.installsLabel')}</span>
            <span className="openharness-skills-scene__detail-value">{selectedMarketSkill.installs ?? 0}</span>
          </div>
        ) : null}

        {selectedMarketSkill?.url ? (
          <div className="openharness-skills-scene__detail-row">
            <span className="openharness-skills-scene__detail-label">{t('market.detail.linkLabel')}</span>
            <a
              href={selectedMarketSkill.url}
              target="_blank"
              rel="noreferrer"
              className="openharness-skills-scene__detail-link"
            >
              {selectedMarketSkill.url}
            </a>
          </div>
      ...
      <Modal
        ...
        size="small"
      >
        <div className="openharness-skills-scene__modal-form">
          <Select
          ...
          {installed.formLevel === 'project' && installed.hasWorkspace ? (
            <div className="openharness-skills-scene__form-hint">
              {t('form.level.currentWorkspace', { path: installed.workspacePath })}
            </div>
          ) : null}

          <div className="openharness-skills-scene__path-input">
            <Input
          ...
          <div className="openharness-skills-scene__path-hint">
            {t('form.path.hint')}
          </div>

          {installed.isValidating ? (
            <div className="openharness-skills-scene__validating">{t('form.validating')}</div>
          ) : null}

          {installed.validationResult ? (
            <div
              className={[
                'openharness-skills-scene__validation',
                installed.validationResult.valid ? 'is-valid' : 'is-invalid',
              ].filter(Boolean).join(' ')}
            >
              {installed.validationResult.valid ? (
                <>
                  <div className="openharness-skills-scene__validation-name">
                    {installed.validationResult.name}
                  </div>
                  <div className="openharness-skills-scene__validation-desc">
                    {installed.validationResult.description}
                  </div>
                </>
              ) : (
                <div className="openharness-skills-scene__validation-error">
          ...
          <div className="openharness-skills-scene__modal-form-actions">
            <Button
              variant="secondary"
              size="small"
              onClick={() => {
