...
function highlightFirstMatch(text: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="openharness-settings-nav__search-highlight">
...
const SettingsNav: React.FC = () => {
  ...
  return (
    <div className="openharness-settings-nav">
      <div className="openharness-settings-nav__header">
        <span className="openharness-settings-nav__title">
          {t('configCenter.title', { defaultValue: t('title', { defaultValue: 'Settings' }) })}
        </span>
      </div>

      <div className="openharness-settings-nav__search">
        <Search
          ref={searchInputRef}
          className="openharness-settings-nav__search-field"
          size="small"
          value={draftQuery}
      ...
      <div
        ...
        className="openharness-settings-nav__sections"
      ...
      >
        {isSearchMode ? (
          <>
            {results.length === 0 ? (
              <div className="openharness-settings-nav__search-empty" role="status">
                ...
                {results.map((row, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        'openharness-settings-nav__search-result-item',
                        active && 'is-active',
                    ...
                    >
                      <span className="openharness-settings-nav__search-result-line">
                        {highlightFirstMatch(line, displayQuery)}
                      </span>
                      {row.description ? (
                        <span className="openharness-settings-nav__search-result-desc">
          ...
          SETTINGS_CATEGORIES.map((category) => (
            <div key={category.id} className="openharness-settings-nav__category">
              <div className="openharness-settings-nav__category-header">
                <span className="openharness-settings-nav__category-label">
                  {t(category.nameKey, { defaultValue: category.id })}
                </span>
              </div>

              <div className="openharness-settings-nav__items">
                {category.tabs.map((tabDef) => (
                  ...
                  >
                    <span className="openharness-settings-nav__item-label">
                      {t(tabDef.labelKey, { defaultValue: tabDef.id })}
                    </span>
                    {tabDef.beta ? (
                      <Badge variant="warning" className="openharness-settings-nav__item-beta">
                        {t('configCenter.beta')}
                      </Badge>
                    ) : null}
                  </button>
