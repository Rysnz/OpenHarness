...
const WorkingCopyView: React.FC<WorkingCopyViewProps> = ({
  ...
  if (!workspacePath) {
    return (
      <div className="openharness-git-scene-working-copy">
        <div className="openharness-git-scene-working-copy__placeholder">
          <FileCode2 size={48} aria-hidden />
          <p>{t('tabs.changes')}</p>
          <p className="openharness-git-scene-working-copy__hint">Open a workspace to see changes.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="openharness-git-scene-working-copy">
      <div className="openharness-git-scene-working-copy__commit-bar">
        <div className="openharness-git-scene-working-copy__status-row">
          <GitBranch size={12} />
          <span className="openharness-git-scene-working-copy__branch">{status?.current_branch ?? t('common.unknown')}</span>
          {(status?.ahead ?? 0) > 0 && (
            <Tooltip content={t('status.ahead')}>
              <span className="openharness-git-scene-working-copy__badge wcv-badge--ahead">↑{status?.ahead}</span>
            </Tooltip>
          )}
          {(status?.behind ?? 0) > 0 && (
            <Tooltip content={t('status.behind')}>
              <span className="openharness-git-scene-working-copy__badge wcv-badge--behind">↓{status?.behind}</span>
            </Tooltip>
          )}
          <div className="openharness-git-scene-working-copy__sync-actions">
        ...
        <div className="openharness-git-scene-working-copy__commit-input-row">
          <Textarea
            className="openharness-git-scene-working-copy__message"
        ...
        <div className="openharness-git-scene-working-copy__commit-actions">
          <Button
            size="small"
      ...
      <div className="openharness-git-scene-working-copy__main" ref={mainRef}>
        <div className="openharness-git-scene-working-copy__file-list" style={{ width: fileListWidth }}>
          <div className="openharness-git-scene-working-copy__search">
            <SearchComponent
          ...
          {status && (status.unstaged?.length || status.untracked?.length || status.staged?.length) ? (
            <>
              {filteredFiles.unstaged.length > 0 && (
                <>
                  <div
                    className="openharness-git-scene-working-copy__group-header"
                  ...
                  {expandedFileGroups.has('unstaged') &&
                    filteredFiles.unstaged.slice(0, MAX_RENDERED_FILES).map((file: { path: string; status: string }, idx: number) => {
                      ...
                      return (
                        <div
                          key={`u-${idx}`}
                          className={`openharness-git-scene-working-copy__file-row ${isSelected ? 'wcv-file--selected' : ''} ${isLoading ? 'wcv-file--loading' : ''}`}
                          onClick={() => !isLoading && handleOpenFileDiff(file.path, file.status)}
                          title={t('tooltips.viewDiff')}
                        >
                          <button
                            type="button"
                            className="openharness-git-scene-working-copy__file-check"
                            onClick={e => {
                              e.stopPropagation();
                          ...
                          <span className="openharness-git-scene-working-copy__file-name">{fileName}</span>
                          {dirPath && <span className="openharness-git-scene-working-copy__file-dir">{dirPath}</span>}
                          <span className={`openharness-git-scene-working-copy__file-status ${statusInfo.className}`}>{statusInfo.text}</span>
                          <IconButton
                            size="xs"
                            variant="ghost"
                            onClick={e => {
              ...
              {filteredFiles.untracked.length > 0 && (
                <>
                  <div className="openharness-git-scene-working-copy__group-header" onClick={() => toggleFileGroup('untracked')}>
                    ...
                    filteredFiles.untracked.slice(0, MAX_RENDERED_FILES).map((filePath: string, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="openharness-git-scene-working-copy__file-name">{fileName}</span>
                          {dirPath && <span className="openharness-git-scene-working-copy__file-dir">{dirPath}</span>}
                          <span className="openharness-git-scene-working-copy__file-status wcv-status--added">U</span>
                          <IconButton
                            size="xs"
                            variant="ghost"
                            onClick={e => {
              ...
              {filteredFiles.staged.length > 0 && (
                <>
                  <div className="openharness-git-scene-working-copy__group-header" onClick={() => toggleFileGroup('staged')}>
                  ...
                  {expandedFileGroups.has('staged') &&
                    filteredFiles.staged.map((file: { path: string; status: string }, idx: number) => {
                      ...
                      return (
                        <div
                          key={`s-${idx}`}
                          className={`openharness-git-scene-working-copy__file-row ${isLoading ? 'wcv-file--loading' : ''}`}
                          onClick={() => !isLoading && handleOpenFileDiff(file.path, file.status)}
                          title={t('tooltips.viewDiff')}
                        >
                          <span className="openharness-git-scene-working-copy__file-name">{file.path}</span>
                          <span className={`openharness-git-scene-working-copy__file-status ${statusInfo.className}`}>{statusInfo.text}</span>
                          <IconButton
                            size="xs"
                            variant="ghost"
            ...
            <div className="openharness-git-scene-working-copy__empty">{t('empty.noChanges')}</div>
          )}
        </div>
        <div
          className="openharness-git-scene-working-copy__resizer"
        ...
        />
        <div className="openharness-git-scene-working-copy__diff-area">
