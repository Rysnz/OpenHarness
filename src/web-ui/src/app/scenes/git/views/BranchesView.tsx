...
const BranchesView: React.FC<BranchesViewProps> = ({ workspacePath }) => {
  ...
  if (!workspacePath) {
    return (
      <div className="openharness-git-scene-branches">
        <div className="openharness-git-scene-branches__placeholder">
          <GitBranch size={48} />
          <p>{t('tabs.branches')}</p>
          <p className="openharness-git-scene-branches__hint">Open a workspace to see branches.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="openharness-git-scene-branches">
      <div className="openharness-git-scene-branches__left">
        <div className="openharness-git-scene-branches__toolbar">
          <div className="openharness-git-scene-branches__toolbar-search">
            <SearchComponent
          ...
          <div className="openharness-git-scene-branches__toolbar-actions">
            <Button
              size="small"
              variant="primary"
              onClick={() => handleCreateFrom(branches.find(b => b.current)?.name ?? selectedBranchName ?? '')}
              title={t('dialog.createNewBranch.title')}
              className="openharness-git-scene-branches__create-btn"
            >
              <Plus size={14} />
        ...
        <div className="openharness-git-scene-branches__list">
          {branchLoading ? (
            <div className="openharness-git-scene-branches__empty">{t('common.loading')}</div>
          ) : filteredBranches.length === 0 ? (
            <div className="openharness-git-scene-branches__empty">
            ...
            filteredBranches.map((branch, idx) => (
              <div
                key={branch.name ?? idx}
                className={`openharness-git-scene-branches__row ${branch.current ? 'openharness-git-scene-branches__row--current' : ''} ${selectedBranchName === branch.name ? 'openharness-git-scene-branches__row--selected' : ''}`}
                onClick={() => handleSelectBranch(branch.name)}
              >
                <div className="openharness-git-scene-branches__info">
                  <GitBranch size={14} />
                  <span className="openharness-git-scene-branches__name">{branch.name}</span>
                  {branch.current && <span className="openharness-git-scene-branches__current-badge">{t('branch.current')}</span>}
                </div>
                <div className="openharness-git-scene-branches__actions" onClick={e => e.stopPropagation()}>
                  {!branch.current && (
      ...
      <div className="openharness-git-scene-branches__right">
        <div className="openharness-git-scene-branches__history-toolbar">
          <span className="openharness-git-scene-branches__history-title">
        ...
        <div className="openharness-git-scene-branches__history-list">
          {!selectedBranchName ? (
            <div className="openharness-git-scene-branches__empty">{t('empty.noCommits')}</div>
          ) : commitLoading ? (
            <div className="openharness-git-scene-branches__empty">{t('common.loading')}</div>
            ...
            filteredCommits.map((commit, idx) => {
              ...
              return (
                <div
                  key={commit.hash ?? idx}
                  className={`openharness-git-scene-branches__commit ${isExpanded ? 'openharness-git-scene-branches__commit--expanded' : ''}`}
                >
                  <div className="openharness-git-scene-branches__commit-header" onClick={() => toggleCommitExpand(commit.hash)}>
                    <button type="button" className="openharness-git-scene-branches__expand">
                      {isExpanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                    </button>
                    <div className="openharness-git-scene-branches__commit-info">
                      <div className="openharness-git-scene-branches__commit-message">{summary}</div>
                      <div className="openharness-git-scene-branches__commit-meta">
                        {author} · {commit.hash?.substring(0, 7)}
                      </div>
                    </div>
                    <div className="openharness-git-scene-branches__commit-actions" onClick={e => e.stopPropagation()}>
                  ...
                  {isExpanded && (
                    <div className="openharness-git-scene-branches__commit-detail">
                      {body && <pre className="openharness-git-scene-branches__commit-body">{body}</pre>}
                      {files && files.length > 0 && (
                        <div className="openharness-git-scene-branches__files">
                          <span>
