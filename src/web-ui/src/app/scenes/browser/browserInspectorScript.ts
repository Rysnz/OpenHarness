 ...
 *  6. Exposes `window.__openharness_inspector_cancel` so a second eval call can
 *     programmatically cancel an active session.
 */

const INSPECTOR_SCRIPT_BODY = /* js */ `
(function () {
  if (window.__openharness_inspector_active) {
    window.__openharness_inspector_cancel && window.__openharness_inspector_cancel();
    return;
  }
  window.__openharness_inspector_active = true;

  var LABEL = window.__openharness_inspector_label || '';
  ...
  function cleanup() {
    ...
    delete window.__openharness_inspector_active;
    delete window.__openharness_inspector_cancel;
    delete window.__openharness_inspector_label;
  }
  ...
  window.__openharness_inspector_cancel = function () {
    emitTauri(EVENT_CANCELLED, null);
    cleanup();
  };
...
export function createInspectorScript(webviewLabel: string): string {
  ...
  return `window.__openharness_inspector_label = ${JSON.stringify(webviewLabel)};\n${INSPECTOR_SCRIPT_BODY}`;
}

/** Script to cancel an active inspector session without triggering a selection. */
export const CANCEL_INSPECTOR_SCRIPT =
  `if (window.__openharness_inspector_cancel) { window.__openharness_inspector_cancel(); }`;
...
export const BLANK_TARGET_INTERCEPT_SCRIPT = `(function(){
  if(window.__openharness_blank_intercept){return;}
  window.__openharness_blank_intercept=true;
