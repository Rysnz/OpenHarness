...
const WorkspaceBody: React.FC<WorkspaceBodyProps> = ({
  ...
  const handleDividerMouseEnter = useCallback(() => {
    ...
    document.body.classList.add('openharness-divider-hovered');
  }, []);

  const handleDividerMouseLeave = useCallback(() => {
    setIsDividerHovered(false);
    document.body.classList.remove('openharness-divider-hovered');
  }, []);
  ...
  const handleNavCollapseDragStart = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    document.body.classList.add('openharness-is-dragging-nav-collapse');
    document.body.classList.add('openharness-is-resizing-nav');
    ...
    function cleanup() {
      document.body.classList.remove('openharness-is-dragging-nav-collapse');
      document.body.classList.remove('openharness-is-resizing-nav');
      document.body.classList.remove('openharness-divider-hovered');
  ...
  return (
    <div className={`openharness-workspace-body${isEntering ? ' is-entering' : ''}${isExiting ? ' is-exiting' : ''}${isDividerHovered ? ' is-divider-hovered' : ''} ${className}`}>
      {isNavCollapsed && (
        <div className="openharness-workspace-body__collapsed-nav">
      ...
      <div
        className={`openharness-workspace-body__nav-area${isNavCollapsed ? ' is-collapsed' : ''}`}
      ...
      >
        ...
        <NavPanel className="openharness-workspace-body__nav-panel" />
      </div>

      {/* Resize divider — placed at workspace-body level to avoid overflow:hidden clipping */}
      {!isNavCollapsed && (
        <div
          className="openharness-workspace-body__nav-divider"
      ...
      <div className="openharness-workspace-body__scene-area">
        <SceneBar
