...
import {
  agentAPI,
  type EnsurePartnerBootstrapResponse,
...
const log = createLogger('PartnerBootstrap');
...
export function usePartnerBootstrap() {
  ...
  const handleEnsureResponse = useCallback(
    (request: BootstrapRequest, response: EnsurePartnerBootstrapResponse): void => {
      switch (response.status) {
        case 'started':
          if (!response.turnId) {
            log.warn('Partner bootstrap started without turnId', { request, response });
            return;
          }
          ...
          log.info('Partner bootstrap started', {
        ...
        case 'blocked':
          if (
            ...
            notificationService.info(t('info.partnerBootstrapWaitingForModelConfiguration'), {
              duration: 5000,
            });
          }
          log.info('Partner bootstrap blocked', {
            workspacePath: request.workspacePath,
        ...
        case 'skipped':
          ...
          log.debug('Partner bootstrap skipped', {
            workspacePath: request.workspacePath,
            sessionId: request.sessionId,
            reason: response.reason,
          });
  ...
  const requestBootstrap = useCallback(
    async (request: BootstrapRequest): Promise<void> => {
      ...
      try {
        const response = await agentAPI.ensurePartnerBootstrap({
          sessionId: request.sessionId,
          workspacePath: request.workspacePath,
        });
        handleEnsureResponse(request, response);
      } catch (error) {
        log.error('Failed to ensure partner bootstrap', {
          workspacePath: request.workspacePath,
          sessionId: request.sessionId,
          error,
        });
