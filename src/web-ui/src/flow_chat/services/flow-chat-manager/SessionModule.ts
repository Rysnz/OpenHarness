...
type SessionDisplayMode = 'code' | 'cowork' | 'partner';

const isPartnerWorkspace = (workspace?: WorkspaceInfo | null): boolean => {
...
const normalizeSessionDisplayMode = (
  ...
  if (isPartnerWorkspace(workspace)) return 'partner';
  if (!mode) return 'code';
...
const resolveAgentType = (
  ...
  if (isPartnerWorkspace(workspace)) {
    return 'Partner';
  }
  ...
  try {
    ...
    const sessionName =
      sessionMode === 'cowork'
        ...
        : sessionMode === 'partner'
          ? i18nService.t('flow-chat:session.newPartnerWithIndex', { count: sameModeCount })
