...
export class FlowChatManager {
  ...
  async resetWorkspaceSessions(
    ...
    options?: {
      ...
      preferredMode?: string;
      /** After reinit, ask core to run partner bootstrap if BOOTSTRAP.md is present (e.g. workspace reset). */
      ensurePartnerBootstrap?: boolean;
    }
  ): Promise<void> {
    ...
    if (options?.ensurePartnerBootstrap) {
      ...
      if (sid) {
        try {
          ...
          log.warn('ensurePartnerBootstrap after resetWorkspaceSessions failed', {
            workspacePath,
            error,
          });
        }
  ...
  private handleTodoWriteResult(sessionId: string, turnId: string, result: any): void {
    try {
      ...
      window.dispatchEvent(new CustomEvent('openharness:todowrite-update', {
        detail: {
          sessionId,
          turnId,
          todos: incomingTodos,
