...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const isPartnerWorkspace = workspace?.workspaceKind === WorkspaceKind.Partner;
  const currentMode = modeState.current;
  const canSwitchModes = !isPartnerWorkspace && currentMode !== 'Cowork';
  ...
  const switchableModes = useMemo(
    () =>
      modeState.available.filter(mode =>
        mode.enabled &&
        mode.id !== 'Cowork' &&
        (isPartnerWorkspace || mode.id !== 'Partner')
      ),
    [isPartnerWorkspace, modeState.available]
  );
  ...
  React.useEffect(() => {
    const handleSessionSwitched = (event: Event) => {
      ...
      if (sessionId && mode) {
        ...
        try {
          sessionStorage.setItem('openharness:flowchat:lastMode', mode);
        } catch {
          // ignore
        }
      }
    ...
    window.addEventListener('openharness:session-switched', handleSessionSwitched);
    
    return () => {
      window.removeEventListener('openharness:session-switched', handleSessionSwitched);
    };
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (session?.mode) {
      ...
      try {
        sessionStorage.setItem('openharness:flowchat:lastMode', session.mode);
      } catch {
        // ignore
      }
    }
  ...
  React.useEffect(() => {
    if (!isPartnerWorkspace || currentMode === 'Partner') {
      return;
    }

    dispatchMode({ type: 'SET_CURRENT_MODE', payload: 'Partner' });
  }, [currentMode, isPartnerWorkspace]);
  ...
  const applyModeChange = useCallback((modeId: string) => {
    ...
    try {
      sessionStorage.setItem('openharness:flowchat:lastMode', modeId);
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    const dropZone = containerRef.current?.closest('.openharness-chat-input-drop-zone') as HTMLElement | null;
  ...
  const renderActionButton = () => {
    if (!derivedState) return <IconButton className="openharness-chat-input__send-button" disabled size="small"><ArrowUp size={11} /></IconButton>;
    ...
    if (petReplacesStopChrome) {
      ...
      if (sendButtonMode === 'split') {
        return (
          <IconButton
            className="openharness-chat-input__send-button"
    ...
    if (sendButtonMode === 'cancel') {
      return (
        <Tooltip content={t('input.stopGeneration')}>
          <div
            className="openharness-chat-input__send-button openharness-chat-input__send-button--breathing"
            onClick={handleSendOrCancel}
            data-testid="chat-input-cancel-btn"
          >
            <div className="openharness-chat-input__breathing-circle" />
            {hasQueuedInput && <span className="openharness-chat-input__queued-badge">1</span>}
          </div>
        </Tooltip>
      );
    }
    ...
    if (sendButtonMode === 'retry') {
      return (
        <IconButton
          className="openharness-chat-input__send-button openharness-chat-input__send-button--retry"
          onClick={handleSendOrCancel}
    ...
    if (sendButtonMode === 'split') {
      return (
        <div className="openharness-chat-input__split-actions">
          <Tooltip content={t('input.stopGeneration')}>
            <div
              className="openharness-chat-input__send-button openharness-chat-input__send-button--breathing"
              onClick={() => {
  ...
  return (
    <>
      <ContextDropZone
        ...
        className="openharness-chat-input-drop-zone"
      ...
      >
        <div 
          ref={containerRef}
          className={`openharness-chat-input ${inputState.isActive ? 'openharness-chat-input--active' : 'openharness-chat-input--collapsed'} ${inputState.isExpanded ? 'openharness-chat-input--expanded' : ''} ${derivedState?.isProcessing ? 'openharness-chat-input--processing' : ''} ${showCollapsedPet ? 'openharness-chat-input--pet-visible' : ''} ${petReplacesStopChrome ? 'openharness-chat-input--pet-replaces-stop' : ''} ${collapsedPetSplitSend ? 'openharness-chat-input--pet-split-send' : ''} ${className}`}
        ...
        {recommendationContext && (
          <SmartRecommendations
            ...
            className="openharness-chat-input__recommendations"
          />
        )}

        <div className="openharness-chat-input__container">
          <div className={`openharness-chat-input__box ${inputState.isExpanded ? 'openharness-chat-input__box--expanded' : ''}`}>
            {showCollapsedPet && (
              <div
                className={[
                  'openharness-chat-input__pet-wrap',
                  petReplacesStopChrome ? 'openharness-chat-input__pet-wrap--shift' : '',
                  collapsedPetSplitSend ? 'openharness-chat-input__pet-wrap--split' : '',
                ]
                  .filter(Boolean)
                  .join(' ')}
              >
                <div className="openharness-chat-input__pet-inner">
                  {petStopClickable ? (
                    <button
                      type="button"
                      className="openharness-chat-input__pet-stop-btn"
                      onClick={e => {
                        e.stopPropagation();
                        void transition(SessionExecutionEvent.USER_CANCEL);
                      }}
            ...
            {showTargetSwitcher && (
              <div className="openharness-chat-input__target-switcher" data-testid="chat-input-target-switcher">
                <span className="openharness-chat-input__target-switcher-label">{t('chatInput.conversationTarget')}</span>
                <button
                  type="button"
                  tabIndex={-1}
                  className={`openharness-chat-input__target-tab ${inputTarget === 'main' ? 'openharness-chat-input__target-tab--active' : ''}`}
                  onClick={() => setInputTarget('main')}
                >
                  {t('chatInput.targetMain')}
                  {inputTarget === 'main' && currentSessionTitle && (
                    <span className="openharness-chat-input__target-tab-name">{currentSessionTitle}</span>
                  )}
                </button>
                <button
                  type="button"
                  tabIndex={-1}
                  className={`openharness-chat-input__target-tab ${inputTarget === 'btw' ? 'openharness-chat-input__target-tab--active' : ''}`}
                  onClick={() => setInputTarget('btw')}
                >
                  {t('chatInput.targetBtw')}
                  {inputTarget === 'btw' && activeBtwSessionTitle && (
                    <span className="openharness-chat-input__target-tab-name">{activeBtwSessionTitle}</span>
                  )}
                </button>
              </div>
            )}
            <div className="openharness-chat-input__input-area">
              <RichTextInput
              ...
              {!inputState.isActive &&
                ...
                <span className="openharness-chat-input__space-hint">
                  <Trans
                    i18nKey="input.spaceToActivate"
                    t={t}
                    components={{
                      space: <span className="openharness-chat-input__space-key" />,
                    }}
                  />
                </span>
              )}
              ...
              {slashCommandState.isActive && (() => {
                if (slashCommandState.kind === 'actions') {
                  const actions = getFilteredActions();
                  return (
                    <div className="openharness-chat-input__slash-command-picker">
                      <div className="openharness-chat-input__slash-command-header">
                        <span>{t('chatInput.quickAction', { defaultValue: 'Quick action' })}</span>
                        <span className="openharness-chat-input__slash-command-hint">{t('chatInput.selectHint')}</span>
                      </div>
                      <div className="openharness-chat-input__slash-command-list">
                        {actions.length > 0 ? (
                          actions.map((action, index) => (
                            <div
                              key={action.id}
                              className={`openharness-chat-input__slash-command-item ${index === slashCommandState.selectedIndex ? 'openharness-chat-input__slash-command-item--selected' : ''}`}
                              onClick={() => selectSlashCommandAction(action.id)}
                              onMouseEnter={() => setSlashCommandState(prev => ({ ...prev, selectedIndex: index }))}
                            >
                              <span className="openharness-chat-input__slash-command-name">{action.command}</span>
                              <span className="openharness-chat-input__slash-command-label">{action.label}</span>
                            </div>
                          ))
                        ) : (
                          <div className="openharness-chat-input__slash-command-empty">
                            {t('chatInput.noMatchingCommand', { defaultValue: 'No matching command' })}
                          </div>
                        )}
                      </div>
                        ...
                        {mcpPromptCommandsLoading && items.length === 0 ? (
                          ...
                          items.map((item, index) => (
                            <div
                              ...
                              className={`openharness-chat-input__slash-command-item ${index === slashCommandState.selectedIndex ? 'openharness-chat-input__slash-command-item--selected' : ''} ${item.kind === 'mode' && item.id === modeState.current ? 'openharness-chat-input__slash-command-item--active' : ''}`}
                              onClick={() => {
                                if (item.kind === 'mode') {
                                  selectSlashCommandMode(item.id);
                                } else if (item.kind === 'mcpPrompt') {
                              ...
                              onMouseEnter={() => setSlashCommandState(prev => ({ ...prev, selectedIndex: index }))}
                            >
                              <span className="openharness-chat-input__slash-command-name">
                                {item.kind === 'mode' ? `/${item.id}` : item.command}
                              </span>
                              <span className="openharness-chat-input__slash-command-label">
                                {item.kind === 'mode'
                                  ? item.name
                                  : item.kind === 'mcpPrompt'
                              ...
                              {item.kind === 'mode' && item.id === modeState.current && <span className="openharness-chat-input__slash-command-current">{t('chatInput.current')}</span>}
                            </div>
                          ))
                        ) : (
                          <div className="openharness-chat-input__slash-command-empty">
                    ...
                    <div className="openharness-chat-input__slash-command-list">
                      {filteredModes.length > 0 ? (
                        filteredModes.map((mode, index) => (
                          <div
                            key={mode.id}
                            className={`openharness-chat-input__slash-command-item ${index === slashCommandState.selectedIndex ? 'openharness-chat-input__slash-command-item--selected' : ''} ${mode.id === modeState.current ? 'openharness-chat-input__slash-command-item--active' : ''}`}
                            onClick={() => selectSlashCommandMode(mode.id)}
                            onMouseEnter={() => setSlashCommandState(prev => ({ ...prev, selectedIndex: index }))}
                          >
                            <span className="openharness-chat-input__slash-command-name">/{mode.id}</span>
                            <span className="openharness-chat-input__slash-command-label">{mode.name}</span>
                            {mode.id === modeState.current && <span className="openharness-chat-input__slash-command-current">{t('chatInput.current')}</span>}
                          </div>
                        ))
                      ) : (
                        <div className="openharness-chat-input__slash-command-empty">
            ...
            <IconButton
              className="openharness-chat-input__expand-button"
              variant="ghost"
              size="xs"
              shape="circle"
            ...
            <div className="openharness-chat-input__actions">
              <div className="openharness-chat-input__actions-left">
                <div className="openharness-chat-input__agent-boost" ref={agentBoostRef}>
                  <Tooltip content={t('chatInput.addBoostTooltip')}>
                    <IconButton
                      className="openharness-chat-input__agent-boost-add"
                      variant="ghost"
                      size="xs"
                      aria-haspopup="menu"
                  ...
                  {canSwitchModes && modeState.current !== 'agentic' && (
                    <div
                      className={`openharness-chat-input__agent-capsule openharness-chat-input__agent-capsule--${modeState.current === 'debug' ? 'debug' : modeState.current}`}
                    >
                      <span className="openharness-chat-input__agent-capsule-label">
                      ...
                      <button
                        type="button"
                        className="openharness-chat-input__agent-capsule-close"
                  ...
                  {modeState.dropdownOpen && (
                    <div className="openharness-chat-input__mode-dropdown openharness-chat-input__mode-dropdown--agent-boost">
                      {canSwitchModes && (
                        <>
                          <div className="openharness-chat-input__boost-section">
                            {incrementalCodeModes.length > 0 ? (
                              incrementalCodeModes.map(modeOption => {
                                ...
                                return (
                                  <Tooltip key={modeOption.id} content={modeDescription} placement="left">
                                    <div
                                      className={`openharness-chat-input__mode-option ${modeState.current === modeOption.id ? 'openharness-chat-input__mode-option--active' : ''}`}
                                      onClick={e => {
                                        e.stopPropagation();
                                        requestModeChange(modeOption.id);
                                      }}
                                    >
                                      <span className="openharness-chat-input__mode-option-name">{modeName}</span>
                                      {modeState.current === modeOption.id && (
                                        <span className="openharness-chat-input__slash-command-current">{t('chatInput.current')}</span>
                                      )}
                                    </div>
                                  </Tooltip>
                                );
                              ...
                              <div className="openharness-chat-input__agent-boost-empty openharness-chat-input__agent-boost-empty--inline">
                                {t('chatInput.noIncrementalModes')}
                              </div>
                            )}
                          </div>

                          <div className="openharness-chat-input__boost-section-divider" aria-hidden />
                        </>
                      )}
                      ...
                      <div className="openharness-chat-input__boost-section">
                        <div
                          role="button"
                          tabIndex={0}
                          className="openharness-chat-input__boost-context-row"
                          onClick={handleBoostOpenAtContext}
                          onKeyDown={e => e.key === 'Enter' && handleBoostOpenAtContext(e)}
                        >
                          <Files size={14} className="openharness-chat-input__boost-context-icon" aria-hidden />
                        ...
                        >
                          <Image size={14} className="openharness-chat-input__boost-context-icon" aria-hidden />
                          <span>{t('input.addImage')}</span>
                        </div>

                        <div
                          ref={skillsHostRef}
                          className="openharness-chat-input__boost-submenu-host"
                        ...
                        >
                          <div
                            role="button"
                            tabIndex={0}
                            className="openharness-chat-input__boost-submenu-trigger"
                            aria-haspopup="menu"
                            aria-expanded={skillsFlyoutOpen}
                          >
                            <span className="openharness-chat-input__boost-submenu-trigger-main">
                              <Sparkles size={14} className="openharness-chat-input__boost-context-icon" aria-hidden />
                              <span>{t('chatInput.boostSkills')}</span>
                            </span>
                            <ChevronRight size={14} className="openharness-chat-input__boost-submenu-chevron" aria-hidden />
                          </div>
                          <div
                            className={[
                              'openharness-chat-input__boost-submenu-shell',
                              skillsFlyoutOpen ? 'openharness-chat-input__boost-submenu-shell--open' : '',
                              skillsFlyoutLeft ? 'openharness-chat-input__boost-submenu-shell--left' : '',
                              skillsFlyoutUp ? 'openharness-chat-input__boost-submenu-shell--up' : '',
                            ].filter(Boolean).join(' ')}
                            onMouseEnter={openSkillsFlyout}
                            onMouseLeave={closeSkillsFlyout}
                          >
                            <div className="openharness-chat-input__boost-submenu-panel">
                              {boostSkillsLoading ? (
                                <div className="openharness-chat-input__boost-submenu-loading">
                                  <Loader2 size={14} className="openharness-chat-input__boost-submenu-spinner" aria-hidden />
                                  <span>{t('chatInput.boostSkillsLoading')}</span>
                                </div>
                              ) : boostPanelSkills.length === 0 ? (
                                <div className="openharness-chat-input__boost-submenu-empty">{t('chatInput.boostSkillsEmpty')}</div>
                              ) : (
                                <div className="openharness-chat-input__boost-submenu-list">
                                  {boostPanelSkills.map(skill => (
                                    <div
                                      key={skill.name}
                                      role="button"
                                      tabIndex={0}
                                      className="openharness-chat-input__boost-submenu-item"
                                    ...
                                    >
                                      <Sparkles size={12} className="openharness-chat-input__boost-submenu-item-icon" aria-hidden />
                                      <span className="openharness-chat-input__boost-submenu-item-name">{skill.name}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              <div
                                role="button"
                                tabIndex={0}
                                className="openharness-chat-input__boost-submenu-manage"
                        ...
                        {!!currentSessionId && !isBtwSession && (
                          <>
                            ...
                            >
                              <MessageSquarePlus size={14} className="openharness-chat-input__boost-context-icon" aria-hidden />
              ...
              <div className="openharness-chat-input__actions-right">
                {isCollapsedProcessing && !petReplacesStopChrome && (
                  <>
                    <span className="openharness-chat-input__capsule-divider" />
                    <span className="openharness-chat-input__cancel-shortcut">
                      <span className="openharness-chat-input__space-key">Esc</span>
