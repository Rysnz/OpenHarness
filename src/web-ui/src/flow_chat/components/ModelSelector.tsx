...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  return (
    <div
      ref={dropdownRef}
      className={`openharness-model-selector ${className}`}
    >
      <Tooltip content={getModelTooltipText(currentModel, t('modelSelector.autoModelDesc'))}>
        <button
          className={`openharness-model-selector__trigger ${dropdownOpen ? 'openharness-model-selector__trigger--open' : ''}`}
          onClick={() => setDropdownOpen(!dropdownOpen)}
          disabled={loading}
        >
          <Cpu size={10} className="openharness-model-selector__icon" />
          <span className="openharness-model-selector__name">
            {getModelDisplayLabel(currentModel, t('modelSelector.autoModel'))}
          </span>
          {currentModel?.enableThinking && (
            <Sparkles size={9} className="openharness-model-selector__thinking-icon" />
          )}
          {currentModel?.reasoningEffort && (
            <span className="openharness-model-selector__effort-badge">
              {currentModel.reasoningEffort}
            </span>
          )}
          <ChevronDown size={10} className="openharness-model-selector__chevron" />
        </button>
      </Tooltip>

      {dropdownOpen && (
        <div className="openharness-model-selector__dropdown">
          <div className="openharness-model-selector__dropdown-header">
            <span>{t('modelSelector.modelSelection')}</span>
            <span className="openharness-model-selector__dropdown-hint">
          ...
          <Tooltip content={t('modelSelector.autoModelDesc')} placement="right">
            <div
              className={`openharness-model-selector__option openharness-model-selector__option--special ${currentModelId === 'auto' ? 'openharness-model-selector__option--selected' : ''}`}
              onClick={() => handleSelectModel('auto')}
            >
              <div className="openharness-model-selector__option-main">
                <span className="openharness-model-selector__option-name">{t('modelSelector.autoModel')}</span>
              </div>
              {currentModelId === 'auto' && (
          ...
          <div className="openharness-model-selector__list">
            {availableModels.map(model => {
              ...
              return (
                <Tooltip key={model.id} content={buildModelMetaText(model)} placement="right">
                  <div
                    className={`openharness-model-selector__option ${isSelected ? 'openharness-model-selector__option--selected' : ''}`}
                    onClick={() => handleSelectModel(model.id)}
                  >
                    <div className="openharness-model-selector__option-main">
                      <span className="openharness-model-selector__option-name">
                        {model.modelName}
                        {model.enableThinking && (
                          <Sparkles size={10} className="openharness-model-selector__option-thinking" />
                        )}
                      </span>
                    </div>
                    {isSelected && (
