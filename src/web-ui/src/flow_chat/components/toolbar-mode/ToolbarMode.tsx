...
export const ToolbarMode: React.FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (target.closest?.('.openharness-toolbar-mode__session-menu-trigger')) {
        return;
      }
      setShowSessionPicker(false);
    };
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (target.closest?.('.openharness-toolbar-mode__overflow-trigger')) {
        return;
      }
      setShowHeaderOverflowMenu(false);
    };
  ...
  const handleStartDrag = useCallback(async (e: React.MouseEvent) => {
    ...
    if (target.closest?.(
      'button, input, .openharness-toolbar-mode__session-picker, .openharness-toolbar-mode__session-dropdown, .openharness-toolbar-mode__overflow-menu, .openharness-toolbar-mode__stream-content, .openharness-toolbar-mode__session-item, .openharness-toolbar-mode__flowchat-container'
    )) {
      return;
    }
    try {
  ...
  const sessionMenuContent = useMemo(
    () => (
      <div className="openharness-toolbar-mode__session-menu">
        <div className="openharness-toolbar-mode__session-menu-actions">
          <button
            type="button"
            className="openharness-toolbar-mode__session-item openharness-toolbar-mode__session-item--new"
            onMouseDown={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          >
            <span className="openharness-toolbar-mode__session-item-icon" aria-hidden>
              <Plus size={13} strokeWidth={2.25} />
            </span>
            <span className="openharness-toolbar-mode__session-item-label">
          ...
          <div className="openharness-toolbar-mode__session-list-divider" role="separator" />
        </div>
        <div
          className="openharness-toolbar-mode__session-menu-scroll"
          role="listbox"
        ...
        >
          {sessions.map((session) => (
            <button
              ...
              type="button"
              className={`openharness-toolbar-mode__session-item ${
                session.sessionId === flowChatState.activeSessionId ? 'openharness-toolbar-mode__session-item--active' : ''
              }`}
  ...
  const containerClassName = [
    'openharness-toolbar-mode',
    isExpanded && 'openharness-toolbar-mode--expanded',
    currentStreamState.isStreaming && 'openharness-toolbar-mode--processing',
    toolbarState.hasError && 'openharness-toolbar-mode--error',
    toolbarState.hasPendingConfirmation && 'openharness-toolbar-mode--confirm'
  ].filter(Boolean).join(' ');
  
  return (
    <div className={containerClassName} onMouseDown={handleStartDrag}>
      <div className="openharness-toolbar-mode__header">
        <div className="openharness-toolbar-mode__header-left">
          {isExpanded ? (
            <div className="openharness-toolbar-mode__session-menu-root">
              <Tooltip content={t('toolCards.toolbar.openSessionMenu')}>
                <button
                  type="button"
                  className={[
                    'openharness-toolbar-mode__create-btn',
                    'openharness-toolbar-mode__session-menu-trigger',
                    showSessionPicker ? 'openharness-toolbar-mode__session-menu-trigger--open' : '',
                  ].filter(Boolean).join(' ')}
              ...
              {showSessionPicker && (
                <div 
                  className="openharness-toolbar-mode__session-dropdown" 
        ...
        <div className="openharness-toolbar-mode__title-wrapper">
          <div className="openharness-toolbar-mode__title-display" title={sessionTitle}>
            <span className="openharness-toolbar-mode__title-text">{sessionTitle}</span>
          </div>
        </div>

        <div className="openharness-toolbar-mode__header-right">
          <div className="openharness-toolbar-mode__header-drag-area" aria-hidden="true" />
          <div className="openharness-toolbar-mode__header-overflow">
            {isExpanded ? (
              <>
                <Tooltip content={t('toolCards.toolbar.moreMenu')}>
                  <button
                    type="button"
                    className="toolbar-btn toolbar-btn--overflow openharness-toolbar-mode__overflow-trigger"
                ...
                {showHeaderOverflowMenu && (
                  <div
                    ...
                    className="openharness-toolbar-mode__overflow-menu"
                    role="menu"
                    onMouseDown={(e) => e.stopPropagation()}
                  >
                    <button
                      type="button"
                      className="openharness-toolbar-mode__overflow-menu-item"
                      role="menuitem"
                      onClick={() => {
                        void toggleExpanded();
                        setShowHeaderOverflowMenu(false);
              ...
              <div className="openharness-toolbar-mode__header-collapsed-actions">
      ...
      {isExpanded ? (
        <>
          <div className="openharness-toolbar-mode__flowchat-container">
            <ModernFlowChatContainer />
          </div>
          <div className="openharness-toolbar-mode__expanded-input">
            <input
              type="text"
              className="openharness-toolbar-mode__input-field openharness-toolbar-mode__input-field--expanded"
              value={inputValue}
        ...
        <div className="openharness-toolbar-mode__content-row">
          {showInput ? (
            <>
              <input
                type="text"
                className="openharness-toolbar-mode__input-field"
                value={inputValue}
                onChange={(e) => setInputValue(e.target.value)}
                onKeyDown={handleKeyDown}
            ...
            <>
              <div className="openharness-toolbar-mode__stream-content" onClick={toggleExpanded}>
                {currentStreamState.toolName ? (
                  <div className="openharness-toolbar-mode__tool">
                    <span className="openharness-toolbar-mode__tool-name">{currentStreamState.toolName}</span>
                    <span className="openharness-toolbar-mode__tool-summary">{currentStreamState.content || t('toolCards.toolbar.executing')}</span>
                  </div>
                ) : toolbarState.todoProgress && toolbarState.todoProgress.total > 0 ? (
                  <div className="openharness-toolbar-mode__todo">
                    <span className="openharness-toolbar-mode__todo-progress">
                      {toolbarState.todoProgress.completed}/{toolbarState.todoProgress.total}
                    </span>
                    <span className="openharness-toolbar-mode__todo-current">
                      {toolbarState.todoProgress.current || currentStreamState.content}
                    </span>
                  </div>
                ) : (
                  <span className={`openharness-toolbar-mode__text ${currentStreamState.isStreaming ? 'openharness-toolbar-mode__text--streaming' : ''}`}>
              ...
              <div className="openharness-toolbar-mode__controls">
