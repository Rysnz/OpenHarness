...
export const WelcomePanel: React.FC<WelcomePanelProps> = ({
  ...
  const greeting = useMemo(() => {
    ...
    const s = isCoworkSession ? 'Cowork' : isPartnerSession ? 'Partner' : '';
  ...
  }, [t, isCoworkSession, isPartnerSession]);

  const tagline = greeting.subtitle;
  const aiPartnerKey = isCoworkSession ? 'welcome.aiPartnerCowork' : isPartnerSession ? 'welcome.aiPartnerPartner' : 'welcome.aiPartner';
  ...
  useEffect(() => {
    if (isCoworkSession || isPartnerSession || !currentWorkspace?.rootPath) { setGitState(null); return; }
    void loadGitState(currentWorkspace.rootPath);
  }, [currentWorkspace?.rootPath, isCoworkSession, isPartnerSession, loadGitState]);
        ...
        <div className="welcome-panel__greeting">
          <div className="welcome-panel__greeting-inner">
            ...
            <div className={`welcome-panel__greeting-text${isCoworkSession ? ' welcome-panel__greeting-text--mtc' : ''}`}>
              {isCoworkSession ? (
                <h1
                  className="welcome-panel__heading welcome-panel__heading--mtc"
                  aria-label="More Than Coding"
                >
                  <span className="welcome-panel__mtc-title" aria-hidden="true">
                    <span className="welcome-panel__mtc-word welcome-panel__mtc-word--more">
                      <span className="welcome-panel__mtc-letter">M</span>
                      <span className="welcome-panel__mtc-tail">ore</span>
                    </span>
                    <span className="welcome-panel__mtc-word welcome-panel__mtc-word--than">
                      <span className="welcome-panel__mtc-letter">T</span>
                      <span className="welcome-panel__mtc-tail">han</span>
                    </span>
                    <span className="welcome-panel__mtc-word welcome-panel__mtc-word--coding">
                      <span className="welcome-panel__mtc-letter">C</span>
                      <span className="welcome-panel__mtc-tail">oding</span>
                    </span>
                  </span>
                </h1>
              ) : null}
              <h1 className={`welcome-panel__heading${isCoworkSession ? ' welcome-panel__heading--hidden' : ''}`}>
                {greeting.title}，{t(aiPartnerKey)}{isPartnerSession && partnerName ? `，${partnerName}` : ''}
              </h1>
