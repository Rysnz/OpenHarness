...
const CurrentSessionTitle: React.FC<CurrentSessionTitleProps> = ({ onCreateSession }) => {
  ...
  return (
    <div className="openharness-current-session-title">
      <span className="openharness-current-session-title__text">{title}</span>
      <Tooltip content={newSessionLabel} placement="bottom">
        <button
          className="openharness-current-session-title__create-btn"
