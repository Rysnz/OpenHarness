...
export class FlowChatStore {
  ...
  private clearOldStorage(): void {
    try {
      const keysToRemove = [
        'openharness-flow-chat-state',
        'openharness-flow-chat-global',
        'openharness-session-ids'
      ];
      ...
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('openharness-session-')) {
          localStorage.removeItem(key);
        }
      });
    } catch (error) {
  ...
  public switchSession(sessionId: string): void {
    ...
    window.dispatchEvent(new CustomEvent('openharness:session-switched', {
  ...
  public cancelSessionTask(sessionId: string): void {
    this.setState(prev => {
      ...
      window.dispatchEvent(new CustomEvent('openharness:dialog-cancelled', {
        detail: { sessionId }
      }));
    ...
    try {
      ...
      const processSession = async (metadata: any) => {
        ...
        this.setState(prev => {
          ...
          const VALID_AGENT_TYPES = ['agentic', 'debug', 'Plan', 'Cowork', 'Partner'];
