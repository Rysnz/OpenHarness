...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const avatarClass = [
    'openharness-avatar',
    `openharness-avatar--${shape}`,
    typeof size === 'string' && `openharness-avatar--${size}`,
    className
  ].filter(Boolean).join(' ');
  ...
  const renderContent = () => {
    ...
    if (icon) {
      return <span className="openharness-avatar__icon">{icon}</span>;
    }
    if (children) {
      return <span className="openharness-avatar__text">{children}</span>;
    }
    return null;
  };
...
export const AvatarGroup: React.FC<AvatarGroupProps> = ({
  ...
  return (
    <div className={`openharness-avatar-group ${className}`} style={style}>
      {displayChildren}
      {restCount > 0 && (
        <Avatar className="openharness-avatar-group__rest">+{restCount}</Avatar>
      )}
    </div>
  );
};
