...
export const TabPane: React.FC<TabPaneProps> = ({ children, className = '' }) => {
  ...
  return <div className={`openharness-tab-pane ${className}`}>{children}</div>;
};
...
export const Tabs: React.FC<TabsProps> = ({
  ...
  const containerClass = [
    'openharness-tabs',
    `openharness-tabs--${type}`,
    `openharness-tabs--${size}`,
    stretch && 'openharness-tabs--stretch',
    className
  ].filter(Boolean).join(' ');
  ...
  return (
    <TabsContext.Provider value={contextValue}>
      <div className={containerClass} style={style}>
        <div className="openharness-tabs__nav">
          <div className="openharness-tabs__nav-list">
            {tabs.map((tab) => (
              <div
                key={tab.key}
                className={[
                  'openharness-tabs__tab',
                  activeKey === tab.key && 'openharness-tabs__tab--active',
                  tab.disabled && 'openharness-tabs__tab--disabled',
                ].filter(Boolean).join(' ')}
                onClick={() => handleTabClick(tab.key, tab.disabled)}
              >
                {tab.icon && <span className="openharness-tabs__tab-icon">{tab.icon}</span>}
                <span className="openharness-tabs__tab-label">{tab.label}</span>
                {tab.closable && (
                  <span
                    className="openharness-tabs__tab-close"
          ...
          {type === 'line' && <div className="openharness-tabs__ink-bar" />}
        </div>
        <div className="openharness-tabs__content">
          {panes[activeKey]}
        </div>
      </div>
