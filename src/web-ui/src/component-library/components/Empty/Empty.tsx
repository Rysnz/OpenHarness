...
export const Empty: React.FC<EmptyProps> = ({
  ...
  return (
    <div className={`openharness-empty ${className}`} style={style}>
      <div className="openharness-empty__image">
        {image || <DefaultImage size={size} />}
      </div>
      {description && (
        <div className="openharness-empty__description">{description}</div>
      )}
      {children && <div className="openharness-empty__footer">{children}</div>}
    </div>
  );
};
