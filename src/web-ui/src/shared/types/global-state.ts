...
export interface WorkspaceInfo {
   ...
   * Local / partner workspaces use `localhost` (from backend); remote uses SSH config host.
   */
  sshHost?: string;
}
...
export interface GlobalStateAPI {
  ...
  createPartnerWorkspace(): Promise<WorkspaceInfo>;
  deletePartnerWorkspace(workspaceId: string): Promise<void>;
  resetPartnerWorkspace(workspaceId: string): Promise<WorkspaceInfo>;
...
function mapWorkspaceKind(workspaceKind: APIWorkspaceInfo['workspaceKind']): WorkspaceKind {
  switch (workspaceKind) {
    case WorkspaceKind.Partner:
      return WorkspaceKind.Partner;
...
function mapWorkspaceInfo(workspace: APIWorkspaceInfo): WorkspaceInfo {
  return {
    ...
    partnerId: workspace.partnerId ?? undefined,
...
export function createGlobalStateAPI(): GlobalStateAPI {
  return {
    ...
    async createPartnerWorkspace(): Promise<WorkspaceInfo> {
      return mapWorkspaceInfo(await globalAPI.createPartnerWorkspace());
    },

    async deletePartnerWorkspace(workspaceId: string): Promise<void> {
      return await globalAPI.deletePartnerWorkspace(workspaceId);
    },

    async resetPartnerWorkspace(workspaceId: string): Promise<WorkspaceInfo> {
      return mapWorkspaceInfo(await globalAPI.resetPartnerWorkspace(workspaceId));
    },
