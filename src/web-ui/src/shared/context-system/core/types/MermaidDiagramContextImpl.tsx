...
export class MermaidDiagramCardRenderer implements ContextCardRenderer<'mermaid-diagram'> {
  ...
  render(context: MermaidDiagramContext, options?: RenderOptions): React.ReactNode {
    ...
    return (
      <div className={`openharness-context-card openharness-context-card--mermaid ${compact ? 'openharness-context-card--compact' : ''}`}>
  ...
  private renderValidationIndicator(_context: MermaidDiagramContext): React.ReactNode {
    ...
    return (
      <div className="openharness-context-card__status">
        <CheckCircle size={16} className="openharness-context-card__status-icon--success" />
      </div>
    );
  }
}
