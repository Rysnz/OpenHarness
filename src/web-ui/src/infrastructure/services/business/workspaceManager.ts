...
export type WorkspaceSection = 'partners' | 'projects';
...
class WorkspaceManager {
  ...
  private isWorkspaceInSection(workspace: WorkspaceInfo, section: WorkspaceSection): boolean {
    return section === 'partners'
      ? workspace.workspaceKind === 'partner'
      : workspace.workspaceKind !== 'partner';
  }
  ...
  public async createPartnerWorkspace(): Promise<WorkspaceInfo> {
    try {
      this.setLoading(true);
      this.setError(null);

      const workspace = await globalStateAPI.createPartnerWorkspace();
      const [currentWorkspace, recentWorkspaces, openedWorkspaces] = await Promise.all([
        globalStateAPI.getCurrentWorkspace(),
        globalStateAPI.getRecentWorkspaces(),
        globalStateAPI.getOpenedWorkspaces(),
      ...
      log.error('Failed to create partner workspace', { error });
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.updateState({ loading: false, error: errorMessage }, { type: 'workspace:error', error: errorMessage });
      throw error;
    }
  ...
  public async deletePartnerWorkspace(workspaceId: string): Promise<void> {
    try {
      this.setLoading(true);
      this.setError(null);

      log.info('Deleting partner workspace', { workspaceId });
      ...
      log.error('Failed to delete partner workspace', { workspaceId, error });
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.updateState({ loading: false, error: errorMessage }, { type: 'workspace:error', error: errorMessage });
      throw error;
    }
  ...
  public async resetPartnerWorkspace(workspaceId: string): Promise<WorkspaceInfo> {
    try {
      this.setLoading(true);
      this.setError(null);

      log.info('Resetting partner workspace', { workspaceId });

      const workspace = await globalStateAPI.resetPartnerWorkspace(workspaceId);
      ...
      log.error('Failed to reset partner workspace', { workspaceId, error });
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.updateState({ loading: false, error: errorMessage }, { type: 'workspace:error', error: errorMessage });
      throw error;
    }
