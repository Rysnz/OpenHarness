...
export const WorkspaceProvider: React.FC<WorkspaceProviderProps> = ({ children }) => {
    ...
    try {
      ...
      return {
        ...
        normalWorkspacesList: openedWorkspacesList.filter(
          (workspace) => workspace.workspaceKind !== WorkspaceKind.Partner
        ),
        partnerWorkspacesList: openedWorkspacesList.filter(
          (workspace) => workspace.workspaceKind === WorkspaceKind.Partner
        ),
        openWorkspace: async (path: string) => workspaceManager.openWorkspace(path),
        createPartnerWorkspace: async () => workspaceManager.createPartnerWorkspace(),
        ...
        deletePartnerWorkspace: async (workspaceId: string) =>
          workspaceManager.deletePartnerWorkspace(workspaceId),
        resetPartnerWorkspace: async (workspaceId: string) =>
          workspaceManager.resetPartnerWorkspace(workspaceId),
  ...
  const createPartnerWorkspace = useCallback(async (): Promise<WorkspaceInfo> => {
    return await workspaceManager.createPartnerWorkspace();
  }, []);
  ...
  const deletePartnerWorkspace = useCallback(async (workspaceId: string): Promise<void> => {
    return await workspaceManager.deletePartnerWorkspace(workspaceId);
  }, []);

  const resetPartnerWorkspace = useCallback(async (workspaceId: string): Promise<WorkspaceInfo> => {
    return await workspaceManager.resetPartnerWorkspace(workspaceId);
  }, []);
