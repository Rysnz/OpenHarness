...
function RulesPanel() {
  ...
  const renderForm = () => {
    ...
    return (
      <div className="openharness-ai-rules-config__form">
        <div className="openharness-ai-rules-config__form-header">
        ...
        <div className="openharness-ai-rules-config__form-body">
        ...
        <div className="openharness-ai-rules-config__form-footer">
        ...
        {rules.map((rule) => {
          ...
          return (
            <React.Fragment key={ruleKey}>
              <div
                className={`openharness-config-page-row openharness-config-page-row--center openharness-ai-rules-config__rule-row ${!rule.enabled ? 'is-disabled' : ''}`}
                onClick={() => toggleRuleExpanded(ruleKey)}
              >
                <div className="openharness-config-page-row__meta">
                  <p className="openharness-config-page-row__label openharness-ai-rules-config__rule-label">
                    <span className="openharness-ai-rules-config__rule-name">{rule.name}</span>
                    <span className="openharness-ai-rules-config__rule-badge">{AIRulesAPI.getApplyTypeLabel(rule.apply_type as RuleApplyType)}</span>
                  </p>
                </div>
                <div className="openharness-config-page-row__control" onClick={(e) => e.stopPropagation()}>
                  <div className="openharness-ai-rules-config__item-actions">
                    ...
                    <Tooltip content={t('list.item.editTooltip')}>
                      <button type="button" className="openharness-ai-rules-config__action-btn" onClick={(e) => { e.stopPropagation(); handleEdit(rule); }}>
                        <Edit2 size={14} />
                      </button>
                    </Tooltip>
                    <Tooltip content={t('list.item.deleteTooltip')}>
                      <button type="button" className="openharness-ai-rules-config__action-btn openharness-ai-rules-config__action-btn--danger" onClick={(e) => handleDelete(rule, e)}>
                        <Trash2 size={14} />
                      </button>
                    </Tooltip>
                  </div>
              ...
              {isExpanded && (
                <div className="openharness-ai-rules-config__details">
                  {rule.description && (
                    <div className="openharness-ai-rules-config__details-field">
                      <span className="openharness-ai-rules-config__details-label">{t('list.item.descriptionLabel')}</span>
                      <span>{rule.description}</span>
                    </div>
                  )}
                  {rule.globs && (
                    <div className="openharness-ai-rules-config__details-field">
                      <span className="openharness-ai-rules-config__details-label">{t('list.item.globsLabel')}</span>
                      <code className="openharness-ai-rules-config__details-code">{rule.globs}</code>
                    </div>
                  )}
                  <div className="openharness-ai-rules-config__details-content">
                    <div className="openharness-ai-rules-config__details-label">{t('list.item.contentLabel')}</div>
                    <pre className="openharness-ai-rules-config__details-pre">{rule.content}</pre>
                  </div>
                  <div className="openharness-ai-rules-config__details-meta">{t('list.item.filePathPrefix')}{rule.file_path}</div>
                </div>
              )}
            </React.Fragment>
          );
  ...
  return (
    <div className="openharness-ai-rules-memory-config__rules-panel">
      <ConfigPageSection
        title={t('title')}
        description={t('subtitle')}
        extra={addButton}
      >
        <Tabs type="line" size="small" activeKey={scopeTab} onChange={(k) => setScopeTab(k as ScopeTab)} className="openharness-ai-rules-memory-config__scope-tabs">
...
function MemoryPanel() {
  ...
  return (
    <div className="openharness-ai-rules-memory-config__memory-panel">
      <ConfigPageSection
...
const MemoryEditDialog: React.FC<MemoryEditDialogProps> = ({ memory, memoryTypeMap, onClose, onSave }) => {
  ...
  return (
    <Modal isOpen onClose={onClose} title={memory ? t('dialog.titleEdit') : t('dialog.titleCreate')} size="medium">
      <div className="openharness-ai-memory-config__dialog-body">
        ...
        <div className="openharness-ai-memory-config__form-group">
      ...
      <div className="openharness-ai-memory-config__dialog-footer">
...
const AIRulesMemoryConfig: React.FC = () => {
  ...
  return (
    <ConfigPageLayout className="openharness-ai-rules-memory-config">
