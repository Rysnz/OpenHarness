...
const SessionConfig: React.FC = () => {
  ...
  if (isLoading || !settings) {
    return (
      <ConfigPageLayout className="openharness-func-agent-config">
        <ConfigPageHeader title={t('title')} subtitle={t('subtitle')} />
        <ConfigPageContent className="openharness-func-agent-config__content">
    ...
    <ConfigPageLayout className="openharness-func-agent-config">
      ...
      <ConfigPageContent className="openharness-func-agent-config__content">
        ...
        >
          ...
          <ConfigPageRow
            className="openharness-func-agent-config__model-row"
            label={t('model.label')}
            description={enabledModels.length === 0 ? t('models.empty') : undefined}
            align="center"
          >
            <div className="openharness-func-agent-config__row-control openharness-func-agent-config__row-control--model">
              <ModelSelectionRadio
            ...
            <>
              ...
              {computerUseNote ? (
                <ConfigPageRow label={t('computerUse.platformNote')} align="start">
                  <span className="openharness-func-agent-config__hint">{computerUseNote}</span>
                </ConfigPageRow>
              ) : null}
              <ConfigPageRow
              ...
              >
                <div
                  className="openharness-func-agent-config__row-control"
                  style={{
                    display: 'flex',
                    flexDirection: 'row',
                    flexWrap: 'nowrap',
                ...
                >
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    <span className={computerUseAccess ? 'openharness-func-agent-config__perm-status--granted' : undefined}>
                  ...
                  <Button
                    className="openharness-func-agent-config__row-action-btn"
                    size="small"
                    variant="secondary"
                    disabled={computerUseBusy}
              ...
              >
                ...
                >
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    <span className={computerUseScreen ? 'openharness-func-agent-config__perm-status--granted' : undefined}>
        ...
        >
          ...
          >
            <div className="openharness-debug-config__input-group">
              <Input
          ...
          {debugHasChanges && !isTemplatesModalOpen && (
            <ConfigPageRow label={tDebug('actions.save')} align="center">
              <div className="openharness-debug-config__settings-actions">
                <Button
                  variant="primary"
                  size="small"
        ...
        <Modal
          ...
          titleExtra={(
            <IconButton
              ...
              variant="ghost"
              size="xs"
              className="openharness-debug-config__modal-reset-icon"
          ...
          size="large"
        >
          <div className="openharness-debug-config__modal-body">
            {templateEntries.map(([language, template]) => {
              ...
              return (
                <Card
                  ...
                  padding="none"
                  interactive
                  className={`openharness-debug-config__template-card${isExpanded ? ' is-expanded' : ''}`}
                >
                  <div
                    className="openharness-debug-config__template-header"
                    onClick={() => toggleTemplateExpand(language)}
                  >
                    <div className="openharness-debug-config__template-info">
                      ...
                      <span className="openharness-debug-config__template-name">
                        {template.display_name || LANGUAGE_TEMPLATE_LABELS[language] || language}
                      </span>
                    </div>
                    <ChevronDown
                      size={16}
                      className={`openharness-debug-config__template-arrow${isExpanded ? ' is-expanded' : ''}`}
                    />
                  </div>

                  {isExpanded && (
                    <CardBody className="openharness-debug-config__template-content">
                      <div className="openharness-debug-config__template-field">
                        <Textarea
                      ...
                      <div className="openharness-debug-config__template-field">
                        <label className="openharness-debug-config__template-label">
                          {tDebug('templates.region.label')}
                        </label>
                        <div className="openharness-debug-config__region-inputs">
                          <Input
                      ...
                      {template.notes && template.notes.length > 0 && (
                        <div className="openharness-debug-config__template-field">
                          ...
                          <div className="openharness-debug-config__template-notes">
                            {template.notes.map((note, idx) => (
                              <span key={idx} className="openharness-debug-config__template-note">
                                {note}
                              </span>
                            ))}
                          </div>
          ...
          {debugHasChanges && (
            <div className="openharness-debug-config__modal-footer">
              <Button
                variant="primary"
                size="small"
