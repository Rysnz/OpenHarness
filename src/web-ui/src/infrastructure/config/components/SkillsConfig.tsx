...
const SkillsConfig: React.FC = () => {
  ...
  const renderAddForm = (level: SkillLevel) => {
    ...
    return (
      <div className="openharness-collection-form">
        <div className="openharness-collection-form__header">
          <h3>{t('form.title')}</h3>
          <IconButton variant="ghost" size="small" onClick={resetForm} tooltip={t('form.closeTooltip')}>
            <X size={14} />
          </IconButton>
        </div>
        <div className="openharness-collection-form__body">
          <Select
          ...
          {formLevel === 'project' && hasWorkspace && (
            <div className="openharness-skills-config__form-hint">
              {t('form.level.currentWorkspace', { path: workspacePath })}
            </div>
          )}
          <div className="openharness-skills-config__path-input">
            <Input
          ...
          <div className="openharness-skills-config__path-hint">{t('form.path.hint')}</div>
          {isValidating && <div className="openharness-skills-config__validating">{t('form.validating')}</div>}
          {validationResult && (
            <div className={`openharness-skills-config__validation ${validationResult.valid ? 'is-valid' : 'is-invalid'}`}>
              {validationResult.valid ? (
                <>
                  <div className="openharness-skills-config__validation-name">✓ {validationResult.name}</div>
                  <div className="openharness-skills-config__validation-desc">{validationResult.description}</div>
                </>
              ) : (
                <div className="openharness-skills-config__validation-error">✗ {validationResult.error}</div>
              )}
            </div>
          )}
        </div>
        <div className="openharness-collection-form__footer">
  ...
  const renderSkillRow = (skill: SkillInfo) => {
    const badge = (
      <span className="openharness-collection-item__badge">
    ...
    const control = (
      <>
        <button
          type="button"
          className="openharness-collection-btn openharness-collection-btn--danger"
    ...
    const details = (
      <>
        <div className="openharness-collection-details__field">{skill.description}</div>
        <div className="openharness-collection-details__meta">
          <span className="openharness-collection-details__label">{t('list.item.pathLabel')}</span>
          <code className="openharness-skills-config__path-value">{skill.path}</code>
        </div>
      </>
    );
    return (
  ...
  const renderMarketList = () => {
    if (marketLoading) {
      return (
        <div className="openharness-skills-config__market-list" aria-busy="true" aria-label={t('market.loading')}>
          {Array.from({ length: 5 }).map((_, index) => (
            <Card
              key={`market-loading-${index}`}
              variant="elevated"
              padding="none"
              className="openharness-skills-config__market-item is-loading"
            >
              <CardBody className="openharness-skills-config__market-item-body">
                <div className="openharness-skills-config__market-skeleton-main">
                  <div className="openharness-skills-config__market-skeleton-line openharness-skills-config__market-skeleton-line--title" />
                  <div className="openharness-skills-config__market-skeleton-line openharness-skills-config__market-skeleton-line--desc" />
                  <div className="openharness-skills-config__market-skeleton-line openharness-skills-config__market-skeleton-line--desc is-short" />
                  <div className="openharness-skills-config__market-skeleton-chip" />
                </div>
                <div className="openharness-skills-config__market-skeleton-btn" />
              </CardBody>
            </Card>
          ))}
        </div>
    ...
    if (marketError) {
      return <div className="openharness-skills-config__market-state openharness-skills-config__market-state--error">{t('market.errorPrefix')}{marketError}</div>;
    }

    if (marketSkills.length === 0) {
      return (
        <div className="openharness-skills-config__market-state">
    ...
    return (
      <div className="openharness-skills-config__market-list">
        {displayMarketSkills.map((skill) => {
          ...
          return (
            <Card
              ...
              variant="elevated"
              padding="none"
              className={`openharness-skills-config__market-item${isInstalled ? ' is-installed' : ''}`}
            >
              <CardBody className="openharness-skills-config__market-item-body">
                <div className="openharness-skills-config__market-item-main">
                  <div className="openharness-skills-config__market-item-head">
                    <div className="openharness-skills-config__market-item-name-wrap">
                      <div className="openharness-skills-config__market-item-name">{skill.name}</div>
                      {isInstalled ? (
                        <span className="openharness-skills-config__market-item-badge openharness-skills-config__market-item-badge--installed">
                          <CheckCircle2 size={12} />
                          {t('market.item.installed')}
                        </span>
                      ) : null}
                    </div>
                    <span className="openharness-skills-config__market-item-installs">
                      <TrendingUp size={12} />
                      {t('market.item.installs', { count: skill.installs })}
                    </span>
                  </div>
                  <div className="openharness-skills-config__market-item-description">
                    {skill.description?.trim() || t('market.item.noDescription')}
                  </div>
                  <div className="openharness-skills-config__market-item-meta">
                    {skill.source ? (
                      sourceLabel !== skill.source ? (
                        <Tooltip content={skill.source}>
                          <span className="openharness-skills-config__market-item-chip openharness-skills-config__market-item-source">
                ...
                <div className="openharness-skills-config__market-item-action">
  ...
  if (loading) {
    return (
      <ConfigPageLayout className="openharness-skills-config">
  ...
  if (error) {
    return (
      <ConfigPageLayout className="openharness-skills-config">
        <ConfigPageHeader title={t('title')} subtitle={t('subtitle')} />
        <ConfigPageContent>
          <div className="openharness-collection-empty"><p>{t('list.errorPrefix')}{error}</p></div>
        </ConfigPageContent>
      </ConfigPageLayout>
    );
  }
  ...
  return (
    <ConfigPageLayout className="openharness-skills-config">
      ...
      <ConfigPageContent>
        ...
        >
          <div className="openharness-skills-config__market-toolbar">
            <Search
