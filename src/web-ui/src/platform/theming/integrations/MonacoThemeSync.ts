...
import { ThemeConfig } from '@/platform/theming/types';
import { OpenHarnessDarkTheme } from '@/tools/editor/themes/openharness-dark.theme';
import { createLogger } from '@/shared/utils/logger';

const log = createLogger('MonacoThemeSync');


const SEMANTIC_HIGHLIGHTING_RULES = OpenHarnessDarkTheme.rules;

function getOpenHarnessLightMonacoTheme(): monaco.editor.IStandaloneThemeData {
  return {
    base: 'vs',
    inherit: true,
...
export class MonacoThemeSync {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      monaco.editor.defineTheme('openharness-dark', OpenHarnessDarkTheme);
      log.debug('OpenHarness Dark theme registered');
  ...
  syncTheme(theme: ThemeConfig): void {
    try {
      ...
      if (theme.monaco) {
        ...
        if (theme.type === 'dark') {
          targetThemeId = 'openharness-dark';
        } else {
          targetThemeId = 'openharness-light';
          monaco.editor.defineTheme('openharness-light', getOpenHarnessLightMonacoTheme());
        }
  ...
  getTargetMonacoThemeId(theme: ThemeConfig): string {
    ...
    return theme.type === 'dark' ? 'openharness-dark' : 'openharness-light';
  }

  /**
   * Registers OpenHarness built-in and optional custom Monaco themes on the given Monaco instance.
  ...
  registerThemesForEditorInstance(monacoInstance: typeof monaco, theme: ThemeConfig): string {
    try {
      monacoInstance.editor.defineTheme('openharness-dark', OpenHarnessDarkTheme);
      monacoInstance.editor.defineTheme('openharness-light', getOpenHarnessLightMonacoTheme());
