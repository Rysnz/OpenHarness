...
export interface OpenRemoteWorkspaceRequest {
  ...
  connectionId: string;
  connectionName: string;
  /** Passed through to Rust so session files map to ~/.openharness/remote_ssh/{host}/... before/during connect. */
  sshHost?: string;
}

export type CreatePartnerWorkspaceRequest = Record<string, never>;
...
export interface DeletePartnerWorkspaceRequest {
  workspaceId: string;
}

export interface ResetPartnerWorkspaceRequest {
  workspaceId: string;
}
...
export class GlobalAPI {
  ...
  async createPartnerWorkspace(): Promise<WorkspaceInfo> {
    try {
      return await api.invoke('create_partner_workspace', {
        request: {},
      });
    } catch (error) {
      throw createTauriCommandError('create_partner_workspace', error);
    }
  }
  ...
  async deletePartnerWorkspace(workspaceId: string): Promise<void> {
    try {
      await api.invoke('delete_partner_workspace', {
        request: { workspaceId }
      });
    } catch (error) {
      throw createTauriCommandError('delete_partner_workspace', error, { workspaceId });
    }
  }

  async resetPartnerWorkspace(workspaceId: string): Promise<WorkspaceInfo> {
    try {
      return await api.invoke('reset_partner_workspace', {
        request: { workspaceId }
      });
    } catch (error) {
      throw createTauriCommandError('reset_partner_workspace', error, { workspaceId });
    }
  }
