...
const GitDiffView: React.FC<GitDiffViewProps> = ({
  ...
  const renderDiffContent = useCallback((diff: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (!line.startsWith('diff ') && !line.startsWith('index ') && 
        ...
        lineNumber++;
        diffLines.push(
          <div key={index} className={`openharness-git-diff-view__diff-line openharness-git-diff-view__diff-line--${lineType}`}>
            <span className="openharness-git-diff-view__line-number">{lineNumber}</span>
            <span className="openharness-git-diff-view__line-content">{content}</span>
          </div>
        );
      }
    });
  ...
  if (loading) {
    return (
      <div className={`openharness-git-diff-view ${className}`}>
        <div className="openharness-git-diff-view__loading-state">
          <div className="openharness-git-diff-view__loading-spinner" />
  ...
  if (error) {
    return (
      <div className={`openharness-git-diff-view ${className}`}>
        <div className="openharness-git-diff-view__error-state">
          <FileText size={48} />
  ...
  return (
    <div className={`openharness-git-diff-view ${className}`}>
      <div className="openharness-git-diff-view__header">
        <div className="openharness-git-diff-view__header-left">
          {sourceCommit && targetCommit && (
            <span className="openharness-git-diff-view__commit-range">
              {sourceCommit.substring(0, 7)}...{targetCommit.substring(0, 7)}
            </span>
          )}
          {!sourceCommit && !targetCommit && (
            <div className="openharness-git-diff-view__diff-type-switcher">
              <button 
                className={`openharness-git-diff-view__type-btn ${!currentShowStaged ? 'openharness-git-diff-view__type-btn--active' : ''}`}
          ...
          {loading && (
            <span className="openharness-git-diff-view__loading-indicator">
        ...
        <div className="openharness-git-diff-view__header-right">
          <div className="openharness-git-diff-view__view-options">
            <IconButton
      ...
      <div className="openharness-git-diff-view__content">
        {error ? (
          <div className="openharness-git-diff-view__error-state">
          ...
          <div className="openharness-git-diff-view__file-list">
            {diffFiles.map((file, index) => (
              <div key={file.path} className="openharness-git-diff-view__file-item">
                <div 
                  className="openharness-git-diff-view__file-header"
                  onClick={() => toggleFileExpansion(index)}
                >
                  <div className="openharness-git-diff-view__file-info">
                    <span className={`openharness-git-diff-view__expand-icon ${file.expanded ? 'openharness-git-diff-view__expand-icon--expanded' : ''}`}>
                      {file.expanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    </span>
                    
                    <span className="openharness-git-diff-view__file-status-icon">
                      {getFileStatusIcon(file.status)}
                    </span>
                    
                    <span className="openharness-git-diff-view__file-path">{file.path}</span>
                    
                    {file.oldPath && file.oldPath !== file.path && (
                      <span className={`openharness-git-diff-view__file-status openharness-git-diff-view__file-status--${file.status}`}>
                        ← {file.oldPath}
                      </span>
                    )}
                  </div>
                  
                  <div className="openharness-git-diff-view__file-stats">
                    {file.additions > 0 && (
                      <span className="openharness-git-diff-view__additions">
                        <Plus size={12} />
                        {file.additions}
                      </span>
                    )}
                    {file.deletions > 0 && (
                      <span className="openharness-git-diff-view__deletions">
                        <Minus size={12} />
                        {file.deletions}
                      </span>
                    )}
                ...
                {file.expanded && (
                  <div className="openharness-git-diff-view__diff-content">
          ...
          <div className="openharness-git-diff-view__empty-state">
            <FileText size={48} />
