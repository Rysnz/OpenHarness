...
const RenameInput: React.FC<RenameInputProps> = ({ node, onRename, onCancel }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const input = document.querySelector('.openharness-file-explorer__rename-input-wrapper input') as HTMLInputElement | null;
      if (!input) {
        return;
      }
  ...
  return (
    <div className="openharness-file-explorer__rename-input-wrapper" onClick={(event) => event.stopPropagation()}>
      <Input
        type="text"
        variant="filled"
        inputSize="small"
...
export const FileTreeItem: React.FC<FileTreeItemProps> = ({
  ...
  return (
    <div 
      className={`openharness-file-explorer__node-content ${isSelected ? 'openharness-file-explorer__node-content--selected' : ''} ${node.isDirectory ? 'openharness-file-explorer__node-content--directory' : ''} ${isCompressed ? 'openharness-file-explorer__node-content--compressed' : ''} ${className}`}
    ...
    >
      {node.isDirectory ? (
        <span className={`openharness-file-explorer__expand-icon ${isExpanded ? 'openharness-file-explorer__expand-icon--expanded' : ''}`} onClick={handleExpandClick}>
          {isLoading ? (
            <Loader2 size={16} className="openharness-file-explorer__loading-icon" />
          ) : isExpanded ? (
            <ChevronDown size={16} />
          ) : (
      ...
      {isRenaming ? (
        ...
        <span className={`openharness-file-explorer__node-name ${isCompressed ? 'openharness-file-explorer__compressed-path' : ''}`}>
          {node.name}
        </span>
      )}

      {renderActions ? (
        <div className="openharness-file-explorer__node-actions" onClick={(event) => event.stopPropagation()}>
          {renderActions(node)}
        </div>
      ) : null}
    </div>
