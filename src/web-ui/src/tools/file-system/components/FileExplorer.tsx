...
const ScrollBreadcrumb: React.FC<ScrollBreadcrumbProps> = ({ containerRef, workspacePath, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const detectCurrentDirectory = () => {
      const treeContainer = container.querySelector('.openharness-file-explorer__tree');
      ...
      expandedDirNodes.forEach((node) => {
        ...
        const nodeElement = node.closest('.openharness-file-explorer__node');
        const childrenContainer = nodeElement?.querySelector(':scope > .openharness-file-explorer__node-children');
  ...
  return (
    <div className="openharness-file-explorer__breadcrumb">
      {displaySegments.map((segment, index) => (
        <React.Fragment key={segment.fullPath || index}>
          {index > 0 && (
            <ChevronRight size={10} className="openharness-file-explorer__breadcrumb-separator" />
          )}
          <span 
            className={`openharness-file-explorer__breadcrumb-item ${segment.fullPath ? 'openharness-file-explorer__breadcrumb-item--clickable' : ''}`}
...
export const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const renderNodeContent = useCallback((node: FileSystemNode, _level: number) => {
    return (
      <div className="openharness-file-explorer__node-wrapper">
        <span className={`openharness-file-explorer__node-name ${node.isCompressed ? 'openharness-file-explorer__compressed-path' : ''}`}>
          {node.name}
        </span>
        
        {showFileSize && !node.isDirectory && node.size && (
          <span className="openharness-file-explorer__node-size">
            {formatFileSize(node.size)}
          </span>
        )}
        
        {showLastModified && node.lastModified && (
          <span className="openharness-file-explorer__node-modified">
  ...
  const handleBlur = useCallback((e: React.FocusEvent) => {
    const toolbar = e.currentTarget.querySelector('.openharness-file-explorer__toolbar');
  ...
  const handleContainerClick = useCallback((e: React.MouseEvent) => {
    ...
    if (target.closest('.openharness-file-explorer__toolbar')) {
      return;
    }
    setIsFocused(true);
    setIsToolbarVisible(true);
  ...
  if (filteredFileTree.length === 0) {
    return (
      <div 
        className={`openharness-file-explorer openharness-file-explorer--empty ${className}`}
        data-area="file-explorer"
        data-workspace-root={workspacePath}
        data-shortcut-scope="filetree"
        tabIndex={0}
      >
        <div className="openharness-file-explorer__empty">
          <Folder size={48} className="openharness-file-explorer__empty-icon" />
  ...
  return (
    <div 
      ref={containerRef}
      className={`openharness-file-explorer ${className}`}
      data-area="file-explorer"
      data-workspace-root={workspacePath}
      data-shortcut-scope="filetree"
      tabIndex={0}
    ...
    >
      {(onNewFile || onNewFolder || onRefresh) && !hideToolbar && (
        <div 
          className={`openharness-file-explorer__toolbar ${isToolbarVisible ? 'openharness-file-explorer__toolbar--visible' : ''}`}
      ...
      {useVirtualScroll ? (
        <VirtualFileTree
          ...
          className="openharness-file-explorer__tree"
          workspacePath={workspacePath}
