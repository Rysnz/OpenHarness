...
const HighlightedText = memo<HighlightedTextProps>(({ text, query }) => {
  ...
  while (matchIndex !== -1) {
    ...
    parts.push(
      <mark key={keyIndex++} className="openharness-search-results__highlight">
...
const MatchItem = memo<MatchItemProps>(({ match, target, searchQuery, onLineClick }) => {
  ...
  return (
    <button
      type="button"
      className="openharness-search-results__match"
      onClick={() => onLineClick(target, match.lineNumber)}
    >
      <span 
        className="openharness-search-results__match-content"
        title={match.matchedContent || ''}
      >
        <code>
          {preview.before && (
            <span className="openharness-search-results__match-before">{preview.before}</span>
          )}
          {preview.inside ? (
            <mark className="openharness-search-results__highlight openharness-search-results__match-highlight">
              {preview.inside}
            </mark>
          ) : null}
          {preview.after && (
            <span className="openharness-search-results__match-after">{preview.after}</span>
          )}
        </code>
      </span>
    </button>
...
const FileGroup = memo<FileGroupProps>(({ 
  ...
  return (
    <div className="openharness-search-results__group">
      <div className="openharness-search-results__file">
        <button
          type="button"
          className="openharness-search-results__file-main"
          onClick={() => onFileClick(target)}
          onContextMenu={(event) => onFileContextMenu(event, target)}
        >
          <span
            className={`openharness-search-results__file-icon${
              group.isDirectory ? ' openharness-search-results__file-icon--directory' : ''
            }`}
          >
            {group.isDirectory ? (
              <Folder size={16} />
          ...
          <span className="openharness-search-results__file-info">
            <span className="openharness-search-results__file-name">
              <HighlightedText text={group.name} query={searchQuery} />
            </span>
            <span className="openharness-search-results__file-path">
              {group.path}
            </span>
          </span>
        </button>
        ...
        {hasContentMatches && (
          <button
            type="button"
            className="openharness-search-results__file-toggle"
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            ...
            <span className="openharness-search-results__file-toggle-count">
      ...
      {hasContentMatches && isExpanded && (
        <div className="openharness-search-results__matches">
...
export const FileSearchResults: React.FC<FileSearchResultsProps> = ({
  ...
  if (results.length === 0) {
    return (
      <div className={`openharness-search-results openharness-search-results--empty ${className}`}>
        <div className="openharness-search-results__empty">
          <div className="openharness-search-results__empty-icon">
            <FileText size={48} />
          </div>
          <p>{t('search.noResults')}</p>
          <p className="openharness-search-results__empty-hint">
  ...
  return (
    <div className={`openharness-search-results ${className}`}>
      <div className="openharness-search-results__header">
        <span className="openharness-search-results__count">
          {t('search.resultsSummary', { files: results.length, matches: totalMatches })}
          {hasMore && <span className="openharness-search-results__showing">{t('search.resultsShowing', { count: displayCount })}</span>}
        </span>
      </div>

      <div
        ref={listRef}
        className="openharness-search-results__list"
