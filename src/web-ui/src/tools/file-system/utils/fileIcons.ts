...
export function getFileIconClass(node: FileSystemNode, isExpanded?: boolean): string {
  if (node.isDirectory) {
    return `openharness-file-explorer__icon openharness-file-explorer__icon--folder${isExpanded ? ' openharness-file-explorer__icon--folder-open' : ''}`;
  }
  
  const iconType = getFileIconType(node);
  return `openharness-file-explorer__icon openharness-file-explorer__icon--${iconType}`;
}
