...
import { Tooltip, OpenHarnessLoading } from '@/component-library';
...
export const DiffEditor: React.FC<DiffEditorProps> = ({
  ...
  const [_currentThemeId, setCurrentThemeId] = useState<string>(OpenHarnessDarkThemeMetadata.id);
  ...
  useEffect(() => {
    ...
    const initDiffEditor = async () => {
      try {
        ...
        forceRegisterTheme(OpenHarnessDarkThemeMetadata.id, OpenHarnessDarkTheme);
    ...
    <div className={`diff-editor-container ${className}`}>
      ...
      {loading && (
        <div className="diff-editor-loading-overlay">
          <OpenHarnessLoading size="medium" text={t('editor.diffEditor.loading')} />
        </div>
      )}
