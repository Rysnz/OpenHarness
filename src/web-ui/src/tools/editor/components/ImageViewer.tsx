...
export const ImageViewer: React.FC<ImageViewerProps> = ({
  ...
  return (
    <div className={`openharness-image-viewer ${className} ${isFullscreen ? 'fullscreen' : ''}`}>
      <div className="openharness-image-viewer__toolbar">
        <div className="openharness-image-viewer__info">
          <span className="openharness-image-viewer__filename">{fileName || filePath.split(/[/\\]/).pop()}</span>
          {imageDimensions && (
            <span className="openharness-image-viewer__dimensions">
              {imageDimensions.width} × {imageDimensions.height}
            </span>
          )}
          {fileSize > 0 && (
            <span className="openharness-image-viewer__filesize">
              {formatFileSize(fileSize)}
            </span>
          )}
        </div>
        <div className="openharness-image-viewer__controls">
          <Tooltip content={t('editor.imageViewer.zoomOut')} placement="top">
            <button
              className="openharness-image-viewer__btn"
          ...
          <Tooltip content={t('editor.imageViewer.zoomReset')} placement="top">
            <button
              className="openharness-image-viewer__btn openharness-image-viewer__btn--zoom-display"
          ...
          <div className="openharness-image-viewer__divider" />
      ...
      <div className="openharness-image-viewer__container">
        {loading && (
          <div className="openharness-image-viewer__loading">
            <div className="openharness-image-viewer__spinner" />
            <p>{t('editor.common.loading')}</p>
          </div>
        )}

        {error && (
          <div className="openharness-image-viewer__error">
            <p>{error}</p>
            <p className="openharness-image-viewer__error-path">{filePath}</p>
          </div>
        )}

        {!loading && !error && imageUrl && (
          <div className="openharness-image-viewer__image-wrapper">
            <img
              src={imageUrl}
              alt={fileName || filePath}
              className="openharness-image-viewer__image"
              style={{
