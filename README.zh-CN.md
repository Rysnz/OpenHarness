...
OpenHarness 内置 Agent 记忆系统。它不是简单的聊天记录存储，而是让 Agent 能够自动捕获工作中的关键信息，将碎片化经验整合为可检索的长期记忆。

在配置页面中，你可以设定记忆规则，控制 Agent 应该记住什么、如何整理、何时回顾。记忆系统支持语义搜索，让 Agent 在新会话中也能快速调取相关经验，而不是每次从零开始。
...
| 项目工作 | 文件浏览、代码编辑、终端、Git、项目洞察、增强 Bash 与任务工具 |
| 智能体管理 | 专业智能体、伙伴实例、自定义 Agent 与 Sub-Agent、权限管控、MCP 服务管理 |
| 技能扩展 | 技能安装、技能市场、工具链能力沉淀 |
| Agent 记忆 | 自动记忆捕获、经验整合、语义搜索、记忆规则配置 |
