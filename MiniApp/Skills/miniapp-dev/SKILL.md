---
name: miniapp-dev
description: Develops and maintains the OpenHarness MiniApp system (Zero-Dialect Runtime). Use when working on miniapp modules, Mini Apps gallery, bridge scripts, agent tool (InitMiniApp), permission policy, or any code under src/crates/core/src/miniapp/ or src/web-ui/src/app/scenes/miniapps/. Also use when the user mentions MiniApp, miniapps, bridge, or zero-dialect.
---

# OpenHarness MiniApp V2 开发指南

## 核心哲学：Zero-Dialect Runtime

MiniApp 使用 **标准 Web API + window.app**：UI 侧为 ESM 模块（`ui.js`），后端逻辑在独立 JS Worker 进程（Bun 优先 / Node 回退）中执行。Rust 负责进程管理、权限策略和 Tauri 独占 API；Bridge 从旧的 `require()` shim + `__OPENHARNESS__` 替换为统一的 **window.app** Runtime Adapter。
...
宿主会将主应用主题映射为以下 CSS 变量并注入 iframe 的 `:root`。在 MiniApp 的 CSS 中建议用 `var(--openharness-*, <fallback>)` 引用，以便在 OpenHarness 内与主应用一致，导出为独立应用时 fallback 生效。

**背景**

- `--openharness-bg` — 主背景
- `--openharness-bg-secondary` — 次级背景（如工具栏、面板）
- `--openharness-bg-tertiary` — 第三级背景
- `--openharness-bg-elevated` — 浮层/卡片背景

**文字**

- `--openharness-text` — 主文字
- `--openharness-text-secondary` — 次要文字
- `--openharness-text-muted` — 弱化文字

**强调与语义**

- `--openharness-accent`、`--openharness-accent-hover` — 强调色及悬停
- `--openharness-success`、`--openharness-warning`、`--openharness-error`、`--openharness-info` — 语义色

**边框与元素**

- `--openharness-border`、`--openharness-border-subtle` — 边框
- `--openharness-element-bg`、`--openharness-element-hover` — 控件背景与悬停

**圆角与字体**

- `--openharness-radius`、`--openharness-radius-lg` — 圆角
- `--openharness-font-sans`、`--openharness-font-mono` — 无衬线与等宽字体

**滚动条**

- `--openharness-scrollbar-thumb`、`--openharness-scrollbar-thumb-hover` — 滚动条滑块

示例（在 `style.css` 中）：

```css
:root {
  --bg: var(--openharness-bg, #121214);
  --text: var(--openharness-text, #e8e8e8);
  --accent: var(--openharness-accent, #60a5fa);
}
body {
  font-family: var(--openharness-font-sans, system-ui, sans-serif);
  color: var(--text);
  background: var(--bg);
}
```
...
- iframe 加载后 bridge 会向宿主发送 `openharness/request-theme`，宿主回推当前主题变量，iframe 内 `_applyThemeVars` 写入 `:root`。
