...
> **实际全局对象为 `window.app`**（非 `window.__OPENHARNESS__`），以下各节均基于 `window.app`。
...
- DOM、CSS（含 CSS 变量 `--openharness-bg`, `--openharness-text`, `--openharness-accent` 等）
- Canvas 2D / WebGL
- Web Audio
...
app.onThemeChange((payload) => {
  // payload: { type: 'dark'|'light', vars: { '--openharness-bg': '...', ... } }
});
```
