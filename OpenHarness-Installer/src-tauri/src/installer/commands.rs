...
const INSTALL_MANIFEST_FILE: &str = ".openharness-install-manifest.json";
...
pub fn get_default_install_path() -> String {
    ...
    base.join("OpenHarness").to_string_lossy().to_string()
}
    ...
    let result: Result<(), String> = (|| {
        ...
        if !extracted {
            if cfg!(debug_assertions) {
                ...
                let placeholder = install_path.join("OpenHarness.exe");
                if !placeholder.exists() {
                    std::fs::write(&placeholder, "placeholder")
                        .map_err(|e| format!("Failed to write placeholder: {}", e))?;
                }
                installed_files.push("OpenHarness.exe".to_string());
        ...
        // Step 4: Save first-launch language preference for OpenHarness app.
...
/// Uninstall OpenHarness (for the uninstaller companion).
#[tauri::command]
...
fn schedule_windows_self_uninstall_cleanup(uninstall_exe_path: &Path) -> Result<(), String> {
    ...
    let script_path = temp_dir.join(format!("openharness-uninstall-{}.cmd", pid));
    let log_path = temp_dir.join(format!("openharness-uninstall-cleanup-{}.log", pid));
...
set "LOG=%~2"
if "%TARGET%"=="" exit /b 2
if "%LOG%"=="" set "LOG=%TEMP%\openharness-uninstall-cleanup.log"
...
fn append_uninstall_runtime_log(message: &str) {
    let ts = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let log_path = std::env::temp_dir().join("openharness-uninstall-runtime.log");
...
pub fn launch_application(install_path: String) -> Result<(), String> {
    let exe = if cfg!(target_os = "windows") {
        PathBuf::from(&install_path).join("OpenHarness.exe")
    } else if cfg!(target_os = "macos") {
        PathBuf::from(&install_path).join("OpenHarness")
    } else {
        PathBuf::from(&install_path).join("openharness")
    };
    ...
    std::process::Command::new(&exe)
        ...
        .spawn()
        .map_err(|e| format!("Failed to launch OpenHarness: {}", e))?;
...
pub fn set_theme_preference(theme_preference: String) -> Result<(), String> {
    let allowed = [
        ...
        "openharness-dark",
        "openharness-light",
        "openharness-midnight",
        "openharness-china-style",
        "openharness-china-night",
        "openharness-cyber",
        "openharness-slate",
    ];
...
/// Stored `request_url` aligned with settings `resolveRequestUrl` (no openharness_core).
...
/// Actual install root is always under a `OpenHarness` directory: `{user choice}/OpenHarness`.
/// If the user already chose a path whose last segment is `OpenHarness`, do not append again.
fn with_openharness_install_subdir(path: PathBuf) -> PathBuf {
    let already_openharness = path
        .file_name()
        .and_then(|n| n.to_str())
        .map(|s| s.eq_ignore_ascii_case("OpenHarness"))
        .unwrap_or(false);
    if already_openharness {
        path
    } else {
        path.join("OpenHarness")
    }
}
...
fn prepare_install_target(requested_path: &Path) -> Result<PathBuf, String> {
    ...
    let install_path = with_openharness_install_subdir(requested_path.to_path_buf());
    ...
    if install_path.exists() {
        ...
        if directory_has_entries(&install_path)?
            ...
            && !install_path.join("OpenHarness.exe").exists()
        {
            return Err(format!(
                "{}directory_must_be_empty_or_openharness",
    ...
    let test_file = writable_dir.join(".openharness_install_test");
...
fn ensure_app_config_path() -> Result<PathBuf, String> {
    ...
    std::fs::create_dir_all(&config_root)
        .map_err(|e| format!("Failed to create OpenHarness config directory: {}", e))?;
...
fn preflight_validate_payload_zip_archive<R: std::io::Read + std::io::Seek>(
    ...
    for i in 0..archive.len() {
        ...
        if file_name.eq_ignore_ascii_case("OpenHarness.exe") {
...
fn validate_payload_exe_size(size: u64, source_label: &str) -> Result<(), String> {
    if size < MIN_WINDOWS_APP_EXE_BYTES {
        return Err(format!(
            "Payload OpenHarness.exe from {source_label} is too small ({size} bytes)"
        ));
    }
    Ok(())
}
...
fn collect_uninstall_targets(install_path: &Path) -> Result<Vec<PathBuf>, String> {
    let mut relative_paths = match read_installed_manifest(install_path)? {
        ...
        None => vec!["OpenHarness.exe".to_string(), "uninstall.exe".to_string()],
    };
...
fn verify_installed_payload(install_path: &Path) -> Result<(), String> {
    let app_exe = install_path.join("OpenHarness.exe");
    let app_meta = std::fs::metadata(&app_exe)
        .map_err(|_| "Installed OpenHarness.exe is missing after extraction".to_string())?;
    if app_meta.len() < MIN_WINDOWS_APP_EXE_BYTES {
        return Err(format!(
            "Installed OpenHarness.exe is too small ({} bytes). Payload is likely invalid.",
            app_meta.len()
        ));
    }
