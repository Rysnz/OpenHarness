...
/// Create a desktop shortcut for OpenHarness.
pub fn create_desktop_shortcut(install_path: &Path) -> Result<()> {
    ...
    let shortcut_path = desktop.join("OpenHarness.lnk");
...
/// Create a Start Menu shortcut for OpenHarness.
pub fn create_start_menu_shortcut(install_path: &Path) -> Result<()> {
    let start_menu = get_start_menu_dir()?;
    let openharness_folder = start_menu.join("OpenHarness");
    std::fs::create_dir_all(&openharness_folder)?;

    let shortcut_path = openharness_folder.join("OpenHarness.lnk");
    let exe_path = install_path.join("OpenHarness.exe");
...
pub fn remove_start_menu_shortcut() -> Result<()> {
    let start_menu = get_start_menu_dir()?;
    let openharness_folder = start_menu.join("OpenHarness");
    if openharness_folder.exists() {
        std::fs::remove_dir_all(&openharness_folder)?;
    }
    Ok(())
}
