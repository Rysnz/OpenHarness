...
//! - Context menu integration ("Open with OpenHarness")
...
const APP_NAME: &str = "OpenHarness";
const UNINSTALL_KEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenHarness";
...
/// Register the right-click context menu "Open with OpenHarness" for directories.
pub fn register_context_menu(install_path: &Path) -> Result<()> {
    let exe_path = install_path.join("OpenHarness.exe");
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);

    // Directory background context menu (right-click on empty area)
    let bg_key_path = r"Software\Classes\Directory\Background\shell\OpenHarness";
    let (bg_key, _) = hkcu.create_subkey(bg_key_path)?;
    bg_key.set_value("", &"Open with OpenHarness")?;
    ...
    let dir_key_path = r"Software\Classes\Directory\shell\OpenHarness";
    let (dir_key, _) = hkcu.create_subkey(dir_key_path)?;
    dir_key.set_value("", &"Open with OpenHarness")?;
...
pub fn remove_context_menu() -> Result<()> {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\Background\shell\OpenHarness");
    let _ = hkcu.delete_subkey_all(r"Software\Classes\Directory\shell\OpenHarness");
    Ok(())
}
