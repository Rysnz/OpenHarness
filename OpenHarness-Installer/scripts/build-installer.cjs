/**
 * OpenHarness Installer build script.
 *
 * Steps:
 * 1. Build OpenHarness main app (optional).
...
const OPENHARNESS_ROOT = path.resolve(ROOT, "..");
...
Options:
  ...
  --skip-app-build       Skip building main OpenHarness app
...
function getCandidateAppExePaths(mode) {
  ...
  for (const profile of preferredProfiles) {
    candidates.push(
      ...
      path.join(OPENHARNESS_ROOT, "target", profile, "openharness-desktop.exe"),
      path.join(OPENHARNESS_ROOT, "target", profile, "OpenHarness.exe")
    );
  }
...
// Step 1: Build main OpenHarness app.
if (!skipAppBuild) {
  log("Step 1: Building OpenHarness main application...");
  run(getMainAppBuildCommand(buildMode), OPENHARNESS_ROOT);
} else {
...
if (!appExePath && STRICT_PAYLOAD_VALIDATION) {
  error(
    "Could not find built OpenHarness executable for payload. Build the desktop app first or run with --dev for local debug."
  );
}
...
if (appExePath) {
  ...
  const destExe = path.join(PAYLOAD_DIR, "OpenHarness.exe");
  ...
  if (STRICT_PAYLOAD_VALIDATION && exeSize < MIN_APP_EXE_BYTES) {
    error(
      `OpenHarness.exe in payload is unexpectedly small (${exeSize} bytes). Refusing to continue.`
    );
  }
