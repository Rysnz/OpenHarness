...
A fully custom, branded installer for OpenHarness — built with **Tauri 2 + React** for maximum UI flexibility.
...
```bash
npx tauri dev -- -- --uninstall "D:\\tmp\\openharness-uninstall-test"
```
...
```
src-tauri/target/release/openharness-installer.exe
```

Fast mode output path:

```
src-tauri/target/release-fast/openharness-installer.exe
```
...
Place the built OpenHarness application files in `src-tauri/payload/` before building the installer. The build script handles this automatically.
During `cargo build`, the payload directory is packed into an embedded zip inside `openharness-installer.exe`.
...
- name: Upload Installer
  ...
  with:
    name: OpenHarness-Installer-Exe
    path: OpenHarness-Installer/src-tauri/target/release/openharness-installer.exe
```
