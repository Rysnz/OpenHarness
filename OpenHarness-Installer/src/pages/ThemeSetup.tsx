...
export function ThemeSetup({ options, setOptions, onLaunch, onClose }: ThemeSetupProps) {
  ...
  const lightPreview = findInstallerThemeById('openharness-light');
  const darkPreview = findInstallerThemeById('openharness-dark');
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      setFinishError(typeof err === 'string' ? err : (err as Error)?.message || 'Failed to launch OpenHarness');
    } finally {
