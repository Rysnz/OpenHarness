...
/** Same rule as main app `getSystemPreferredDefaultThemeId`: dark -> openharness-dark, else openharness-light. */
export function getSystemPreferredBuiltinThemeId(): ThemeId {
  if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') {
    return 'openharness-light';
  }
  return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'openharness-dark' : 'openharness-light';
}
 ...
 * When preference is `system`, follows `prefers-color-scheme` like the main OpenHarness ThemeService.
 */
