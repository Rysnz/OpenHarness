...
export class Header extends BasePage {
  private selectors = {
    container: '.openharness-nav-panel, .openharness-scene-bar, .openharness-nav-bar, [data-testid="header-container"], .openharness-header, header',
    homeBtn: '[data-testid="header-home-btn"], .openharness-nav-bar__logo-button, .openharness-header__home',
    minimizeBtn: '[data-testid="header-minimize-btn"], .openharness-title-bar__minimize, .window-controls__btn--minimize, button[aria-label*="Minimize"], button[aria-label*="最小化"]',
    maximizeBtn: '[data-testid="header-maximize-btn"], .openharness-title-bar__maximize, .window-controls__btn--maximize, button[aria-label*="Maximize"], button[aria-label*="最大化"], button[aria-label*="Restore"], button[aria-label*="还原"]',
    closeBtn: '[data-testid="header-close-btn"], .openharness-title-bar__close, .window-controls__btn--close, button[aria-label*="Close"], button[aria-label*="关闭"]',
    leftPanelToggle: '[data-testid="header-left-panel-toggle"], .openharness-nav-bar__panel-toggle',
    rightPanelToggle: '[data-testid="header-right-panel-toggle"]',
    newSessionBtn: '[data-testid="header-new-session-btn"]',
    title: '[data-testid="header-title"], .openharness-nav-bar__menu-item-main, .openharness-header__title',
    configBtn: '[data-testid="header-config-btn"], .openharness-header-right button',
  };
  ...
  async isVisible(): Promise<boolean> {
    const selectors = ['.openharness-nav-panel', '.openharness-scene-bar', '.openharness-nav-bar', '[data-testid="header-container"]', '.openharness-header', 'header'];
    for (const selector of selectors) {
      try {
        const element = await $(selector);
  ...
  async clickMinimize(): Promise<void> {
    const element = await this.findExistingElement([
      ...
      '.openharness-title-bar__minimize',
  ...
  async clickMaximize(): Promise<void> {
    const element = await this.findExistingElement([
      ...
      '.openharness-title-bar__maximize',
  ...
  async areWindowControlsVisible(): Promise<boolean> {
    const controlsContainer = await this.findExistingElement([
      '.window-controls',
      '.openharness-header-right .window-controls',
    ]);
