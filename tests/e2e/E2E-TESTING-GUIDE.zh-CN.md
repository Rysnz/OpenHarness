...
# OpenHarness E2E 测试指南

使用 WebDriverIO + OpenHarness 内嵌 WebDriver 进行 OpenHarness 项目的端到端测试完整指南。
...
OpenHarness E2E 测试专注于**用户旅程**和**关键路径**，确保桌面应用从用户角度正常工作。我们使用分层测试方法来平衡覆盖率和执行速度。
...
#### Debug 模式（默认）
- **应用路径**: `target/debug/openharness-desktop.exe`
...
**核心原理**: E2E 只使用 `target/debug/openharness-desktop.exe`。如果 debug 二进制不存在，应直接失败，而不是回退到 `release`。
...
6. **不要测试第三方代码** - 只测试 OpenHarness 功能
...
# 用 debug 模式运行测试，OpenHarness 会在进程内启动 WebDriver
...
**症状**: `Application not found at target/debug/openharness-desktop.exe`
