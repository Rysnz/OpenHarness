...
# OpenHarness E2E Testing Guide

Complete guide for E2E testing in OpenHarness project using WebDriverIO + OpenHarness embedded WebDriver.
...
OpenHarness E2E tests focus on **user journeys** and **critical paths** to ensure the desktop application works correctly from the user's perspective. We use a layered testing approach to balance coverage and execution speed.
...
OpenHarness uses a 3-tier test classification system:
...
cd ../..
cargo build -p openharness-desktop
```
...
**Windows**: `target/debug/openharness-desktop.exe`
**Linux/macOS**: `target/debug/openharness-desktop`
...
#### Debug Mode (Default)
- **Application Path**: `target/debug/openharness-desktop.exe`
...
```bash
# Debug Mode Output
application: <PROJECT_ROOT>\target\debug\openharness-desktop.exe
Debug build detected, checking dev server...
```

**Core Principle**: E2E uses `target/debug/openharness-desktop.exe` only. If the debug binary is missing, the run should fail instead of falling back to `release`.
...
6. **Don't test third-party code** - Only test OpenHarness functionality
...
# Run tests in debug mode so the embedded driver starts inside OpenHarness
OPENHARNESS_E2E_APP_MODE=debug pnpm --dir tests/e2e run test:l0:protocol
...
**Symptom**: `Application not found at target/debug/openharness-desktop.exe`
...
# Windows
dir target\debug\openharness-desktop.exe
# Linux/macOS
ls -la target/debug/openharness-desktop
```
...
jobs:
  l0-tests:
    ...
    steps:
      ...
      - name: Build app
        run: cargo build -p openharness-desktop
      - name: Install test dependencies
        run: cd tests/e2e && pnpm install
      - name: Run L0 tests
        run: cd tests/e2e && OPENHARNESS_E2E_APP_MODE=debug pnpm run test:l0:all
  ...
  l1-tests:
    ...
    steps:
      ...
      - name: Build app
        run: cargo build -p openharness-desktop
      - name: Run L1 tests
        run: cd tests/e2e && OPENHARNESS_E2E_APP_MODE=debug pnpm run test:l1
```
...
- [OpenHarness Project Structure](../../AGENTS.md)
