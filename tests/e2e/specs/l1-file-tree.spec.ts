...
describe('L1 File Tree', () => {
  ...
  before(async () => {
    ...
    if (hasWorkspace) {
      ...
      const fileNavSelectors = [
        '//button[contains(@class, "openharness-nav-panel__item")]//span[contains(text(), "Files")]/..',
        '//button[contains(@class, "openharness-nav-panel__item")]//span[contains(text(), "文件")]/..',
        '.openharness-nav-panel__item[aria-label*="Files"]',
        '.openharness-nav-panel__item[aria-label*="文件"]',
        'button.openharness-nav-panel__item:first-child', // Files is usually first
      ];
  ...
  describe('File tree existence', () => {
    it('file tree container should be visible', async function () {
      ...
      const selectors = [
        '.openharness-file-explorer__tree',
        '[data-file-tree]',
        '.file-tree',
        '[class*="file-tree"]',
        '[class*="FileTree"]',
        '.openharness-file-explorer',
        '[class*="file-explorer"]',
      ];
      ...
      if (!treeFound) {
        ...
        const fileExplorer = await $('.openharness-file-explorer, .openharness-explorer-scene, [class*="Explorer"]');
  ...
  describe('File selection', () => {
    it('clicking file should select it', async function () {
      ...
      const content = await firstFile.$('.openharness-file-explorer__node-content');
    ...
    it('selected file should have selected class', async function () {
      ...
      const selectedNodes = await browser.$$('.openharness-file-explorer__node-content--selected');
