...
describe('L0 Smoke Tests', () => {
  ...
  describe('Core UI components', () => {
    it('Header should be visible', async () => {
      ...
      const header = await $('.openharness-nav-panel, .openharness-scene-bar, .openharness-nav-bar, [data-testid="header-container"]');
      ...
      if (exists) {
        ...
        const selectors = [
          '.openharness-nav-panel',
          '.openharness-scene-bar',
          '.openharness-nav-bar',
          'header',
          '.header',
          '[class*="header"]',
    ...
    it('should have either startup page or workspace UI', async () => {
      ...
      const welcomeSelectors = [
        ...
        '.welcome-scene',
        '.openharness-scene-viewport--welcome',
      ];
      ...
      if (!welcomeExists) {
        ...
        const sceneViewport = await $('.openharness-scene-viewport');
