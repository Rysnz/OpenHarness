...
describe('L1 Mermaid Export', () => {
  ...
  beforeEach(async function () {
    ...
    await browser.execute(() => {
      (window as Window & { __OPENHARNESS_E2E_PNG_EXPORT_DELAY_MS__?: number }).__OPENHARNESS_E2E_PNG_EXPORT_DELAY_MS__ = 1200;
    });
  ...
  afterEach(async () => {
    await browser.execute(() => {
      delete (window as Window & { __OPENHARNESS_E2E_PNG_EXPORT_DELAY_MS__?: number }).__OPENHARNESS_E2E_PNG_EXPORT_DELAY_MS__;
    });
  });
  ...
  it('png export should show progress and finish successfully', async function () {
    ...
    await browser.waitUntil(async () => {
      ...
      return !!className && className.includes('openharness-notification-btn--loading');
    }, {
      timeout: 5000,
    ...
    await browser.waitUntil(async () => {
      return browser.execute(() => {
        ...
        return !(button?.classList.contains('openharness-notification-btn--loading'));
      });
    }, {
      timeout: 20000,
