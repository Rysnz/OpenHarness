...
function requireWorkspace(context: { skip: () => void }, hasWorkspace: boolean): boolean {
  if (hasWorkspace) {
    return true;
  }
...
async function typeAndRead(chatInput: ChatInput, message: string): Promise<string> {
  await chatInput.typeMessage(message);
  return chatInput.getValue();
}
...
describe('L1 Chat Input Validation', () => {
  ...
  describe('Input interaction', () => {
    ...
    it('should type single line message', async function () {
      ...
      const value = await typeAndRead(chatInput, testMessage);
    ...
    it('should type multiline message', async function () {
      ...
      const value = await typeAndRead(chatInput, multilineMessage);
    ...
    it('should handle special characters', async function () {
      ...
      const value = await typeAndRead(chatInput, specialChars);
